...
async function sendEmbed() {
  ...
  if (isForum && !forum_title) { toast('This is a forum channel — give the post a title', false); return; }

  const payload = { channel_id, mode, forum_title: isForum ? forum_title : null };
  if (mode === 'raw') {
...
def api_embed_builder_send(guild_id):
    ...
    if is_forum and not forum_title:
        return {"ok": False, "error": "Give the forum post a title."}, 400
    ...
    try:
        if mode == "raw":
            ...
            if is_forum:
                ...
                for part in parts:
                    run_on_bot_loop(channel.send(content=part), timeout=30)
        else:
            e = data.get("embed") or {}
    ...
    return {"ok": True, "message": f"Sent to {'a new post in ' if is_forum else '#'}{channel.name} ✅"}
