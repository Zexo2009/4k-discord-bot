...
async def log_points_action(guild: discord.Guild, actor: discord.Member, target_id: int, amount: int, new_total: int, action: str):
    """Posts an audit-log entry whenever someone gives/removes points, so it's always visible who did it."""
    log_channel = find_channel(guild, LOG_CHANNEL_ID, LOG_CHANNEL_NAME)
    if not log_channel:
        return
    ...
    try:
        ...
        print("⚠️ Missing permission to post in the points log channel.")
...
async def build_leaderboard_embed(guild: discord.Guild):
    ...
    for i, (uid, score) in enumerate(sorted_entries):
        ...
        rank_label = current_rank_label(score) or UNRANKED_LABEL
        lines.append(f"{position} **{name}** — `{score}` pts {rank_emoji} *{rank_label}*")
    embed.description = "\n".join(lines)
    embed.set_footer(text="Keep submitting daily to climb the board!")
    return embed
...
async def addpoints(ctx, member: discord.Member = None, amount: int = None):
    ...
    await log_points_action(ctx.guild, ctx.author, member.id, amount, new_total, "add")
...
async def removepoints(ctx, member: discord.Member = None, amount: int = None):
    ...
    if member is None or amount is None:
        ...
        return
    new_total = await award_points(ctx.guild, member.id, -amount)
    await log_points_action(ctx.guild, ctx.author, member.id, amount, new_total, "remove")
...
async def slash_addpoints(interaction: discord.Interaction, member: discord.Member, amount: int):
    ...
    await log_points_action(interaction.guild, interaction.user, member.id, amount, new_total, "add")
...
async def slash_removepoints(interaction: discord.Interaction, member: discord.Member, amount: int):
    if not has_points_permission(interaction.user):
        ...
        return
    new_total = await award_points(interaction.guild, member.id, -amount)
    await log_points_action(interaction.guild, interaction.user, member.id, amount, new_total, "remove")
