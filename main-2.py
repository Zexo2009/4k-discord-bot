...
async def purge_messages_with_word(channel: discord.TextChannel, word: str, scan_limit: int = None) -> int:
    """Deletes every message in `channel` whose content contains `word` as a whole word.
    scan_limit=None means the ENTIRE channel history is scanned, no matter how old.
    Handles both recent messages (fast bulk delete) and messages older than 14 days
    (Discord requires deleting those one by one, so it's slower)."""
    ...
    for msg in to_delete:
        if now - msg.created_at < timedelta(days=14):
            ...
            try:
                await msg.delete()
                deleted_total += 1
                await asyncio.sleep(0.7)  # avoid rate limits on single deletes
            except discord.HTTPException:
                pass
...
async def purgeword(ctx, channel: discord.TextChannel = None, *, word: str = None):
    ...
    if not channel.permissions_for(ctx.guild.me).manage_messages:
        ...
        return
    status = await ctx.send(f"⏳ Deleting every message containing **{word}** in {channel.mention}... scanning the ENTIRE channel history, so this can take a while for old/active channels.")
    deleted = await purge_messages_with_word(channel, word)
    await status.edit(content=f"✅ Deleted **{deleted}** message(s) containing '{word}' in {channel.mention}.")
...
async def slash_purgeword(interaction: discord.Interaction, channel: discord.TextChannel, word: str):
    ...
    if not channel.permissions_for(interaction.guild.me).manage_messages:
        ...
        return
    await interaction.response.send_message(f"⏳ Deleting every message containing **{word}** in {channel.mention}... scanning the ENTIRE channel history, so this can take a while for old/active channels.")
    deleted = await purge_messages_with_word(channel, word)
    await interaction.followup.send(f"✅ Deleted **{deleted}** message(s) containing '{word}' in {channel.mention}.")
