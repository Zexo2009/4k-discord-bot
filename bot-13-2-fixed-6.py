...
async def handle_nuke_bot_detected(guild: discord.Guild, bot_member: discord.Member, reason: str):
    ...
    else even if the kick call is briefly delayed — then kick it, then punish whoever added it."""
    ...
    try:
        await bot_member.kick(reason=reason)
    except discord.Forbidden:
        print(f"⚠️ Anti-raid: missing 'Kick Members' permission in {guild.name} — could not kick {bot_member}.")
    except discord.HTTPException as e:
        print(f"⚠️ Anti-raid: failed to kick {bot_member}: {e}")
...
async def on_guild_channel_create(channel):
    ...
    # flood the server with junk channels. Same detection approach as deletion above.
    guild = channel.guild
    creator = None
    try:
    ...
    if not creator or not creator.bot or not bot.user or creator.id == bot.user.id:
        return
    if _record_and_check_nuke_pattern(guild.id, creator.id, "channel_create", NUKE_ACTION_THRESHOLD, NUKE_ACTION_WINDOW_SECONDS):
        creator_member = guild.get_member(creator.id)
        if creator_member:
            await handle_nuke_bot_detected(guild, creator_member, reason="mass channel creation (nuke pattern)")
...
async def on_guild_channel_delete(channel):
    ...
    # way as an unauthorized bot add.
    guild = channel.guild
