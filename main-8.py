            ...
            if not message_pings_role(message, picker_role):
                ...
                if sources:
                    ...
                    if message.author.id not in reminded:
                        ...
                        try:
                            await message.reply(
                                f"👋 Heads up {message.author.mention} — to have your edit count for "
...
async def collect_pending_entries(guild: discord.Guild):
    ...
    async for msg in submit_channel.history(after=since, limit=None):
        ...
        if not message_pings_role(msg, picker_role):
            continue
        for video_source in await extract_video_sources(msg):
...
async def run_collect_job():
    ...
    for guild in bot.guilds:
        ...
        for batch_index, batch in enumerate(batches):
            question = "Vote for today's top edit!" if len(batches) == 1 else (
                f"Vote for today's top edit! (part {batch_index + 1}/{len(batches)})"
            )
            ...
            ping = f"{picker_role.mention} 📊 Vote for today's top edit!" if batch_index == 0 else (
