...
async def vhq(ctx, url: str = None):
    """
    Nutzung: !vhq <Video-URL>
    Lädt ein Video herunter und kodiert es in guter Qualität (1080p) um.
    """
    if url is None:
    ...
    try:
        ...
        dl_cmd = [
            "yt-dlp",
            # Quelle auf max. 1080p begrenzen -> weniger Rohdaten im Speicher
            # beim späteren Hochskalieren auf 4K
        ...
        try:
            ...
            proc.kill()
            await status_msg.edit(content="❌ Download hat zu lange gedauert (Timeout). Versuch ein kürzeres Video.")
            return
        ...
        await status_msg.edit(
            content=(
                "⏳ Kodiere in 1080p... "
                "Hinweis: Auf dem kostenlosen Server wird das Video auf max. 60 Sekunden "
                "gekürzt, damit der Speicher (RAM) nicht überläuft."
            )
        )

        MAX_SECONDS = 60  # Grenze wegen 512 MB RAM auf dem Free-Tier

        ff_cmd = [
            "ffmpeg", "-y",
            "-i", actual_input,
            "-t", str(MAX_SECONDS),          # Video auf max. Länge kürzen
            "-vf", "scale=-2:1080",
            "-c:v", "libx264",
            "-preset", "ultrafast",          # deutlich weniger RAM als "medium"
            "-crf", "23",
            "-threads", "1",                 # keine parallelen Speicherspitzen
            "-c:a", "aac",
            output_path,
        ]
        ...
        try:
            ...
            proc2.kill()
            await status_msg.edit(content="❌ Kodierung hat zu lange gedauert (Timeout). Versuch ein kürzeres Video.")
            return
