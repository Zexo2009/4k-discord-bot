...
async def on_message(message: discord.Message):
    ...
    if message.guild and not message.author.bot:
        ...
        if submit_channel and picker_role and message.channel.id == submit_channel.id:
            ...
            # Ping reminder — pinging is required for the edit to count at all, so it fires
            # every single time it's missing (not just once ever) — otherwise someone who forgets
            # again next week gets no nudge at all.
        ...
        try:
            async for prior in msg.channel.history(before=msg, limit=5):
                if prior.author.id != msg.author.id:
                    continue
                prior_sources = [a.url for a in prior.attachments]
                prior_sources += [w for w in prior.content.split() if w.startswith("http://") or w.startswith("https://")]
                for snapshot in (getattr(prior, "message_snapshots", None) or []):
                    prior_sources += [a.url for a in (getattr(snapshot, "attachments", []) or [])]
                    prior_sources += [w for w in (getattr(snapshot, "content", "") or "").split() if w.startswith("http://") or w.startswith("https://")]
                if prior_sources:
...
async def extract_simple_sources(msg: discord.Message):
    ...
    sources = [a.url for a in msg.attachments]
    sources += [w for w in msg.content.split() if w.startswith("http://") or w.startswith("https://")]
    ...
    if not sources:
        ...
        if replied is None and msg.reference and msg.reference.message_id:
            ...
            try:
                ...
                replied = None
        if isinstance(replied, discord.Message):
            sources = [a.url for a in replied.attachments]
            sources += [w for w in (replied.content or "").split() if w.startswith("http://") or w.startswith("https://")]
            if not sources:
                ...
                for snapshot in (getattr(replied, "message_snapshots", None) or []):
                    sources += [a.url for a in (getattr(snapshot, "attachments", []) or [])]
                    sources += [w for w in (getattr(snapshot, "content", "") or "").split() if w.startswith("http://") or w.startswith("https://")]
            if not sources:
                ...
                for raw_snap in await _raw_message_snapshots(replied):
                    snap_msg = raw_snap.get("message", {}) or {}
                    sources += [a.get("url") for a in (snap_msg.get("attachments") or []) if a.get("url")]
                    sources += [w for w in (snap_msg.get("content") or "").split() if w.startswith("http://") or w.startswith("https://")]
            if sources:
...
async def airating_cmd(ctx, *, video_url: str = None):
    ...
    async with ctx.typing():
        ...
        frame_paths = []
        try:
            # 8 frames instead of 4 — more evenly-spaced samples across the clip give the model
            # a real basis for judging grading *consistency* and cut frequency, not just a couple
            # of lucky/unlucky stills. Directly improves rating accuracy.
            frame_paths = await asyncio.get_running_loop().run_in_executor(None, _extract_frames, video_path, 8)
            if not frame_paths:
...
RATE_EDIT_PROMPT = (
    "You are a blunt, no-mercy video editor giving cold, fact-only feedback on 8 evenly-spaced "
    ...
    "CALIBRATION — read this carefully, this is the part you tend to get wrong: your default "
    "instinct is to round everything up toward a 'safe' 6-7 out of politeness. Fight that. Most "
    "casual submissions you'll see are genuinely average-to-weak: default CapCut/phone-app cuts, "
    "no real color grading (or a single slapped-on filter), no deliberate sync-to-beat framing, "
    "nothing distinctive — that is a 3-5, not a 6-7, and you should default there unless the frames "
    "show clear deliberate technique. Reserve 6-7 for edits with actual intentional grading AND "
    "clean composition. Reserve 8-10 for edits that would hold up next to professional work — "
    "genuinely rare. If you're unsure between two scores, take the lower one. That said, don't "
    "swing to the opposite failure mode either: don't punish an edit for being simple if what IS "
    "there is genuinely well executed, and don't invent flaws that aren't visible just to look "
    "tough — an excellent edit still gets a 9-10, stated as fact, not talked down.\n\n"
    ...
    "TIPS:\n"
    "- <one short, concrete, fact-based criticism or note — under 15 words>\n"
    "- <a second one — under 15 words>\n\n"
    ...
    "transition, text-animation look) — otherwise say it's not identifiable. Tips are flat "
    "statements of fact, no hedging language ('maybe', 'consider', 'might help') — say what's "
    "wrong or, if it's genuinely strong, what's working, as fact either way. You're judging still "
    "frames, not motion/timing/sync — don't claim to assess smoothness or transitions you can't "
    "see, and you have no access to other submissions or a live leaderboard to compare against."
)
...
async def _run_rate_edit(interaction: discord.Interaction, source_message: discord.Message):
    ...
    frame_paths = []
    try:
        # Same accuracy bump as .airating — 8 evenly-spaced frames instead of 4.
        frame_paths = await asyncio.get_running_loop().run_in_executor(None, _extract_frames, video_path, 8)
        if not frame_paths:
