...
@app_commands.describe(
    ...
    ping_here="Ping @here when the giveaway starts? (default: no)",
)
...
async def _create_giveaway_and_post(
    ...
    if ping_here:
        send_kwargs["content"] = "@here 🎉 New giveaway!"
...
class GiveawayCreateModal(discord.ui.Modal, title="Create a Giveaway"):
    ...
    Description) — the advanced options (channel, roles, attachment, ping) are already
    picked via the /giveaway start slash command's own parameters before this modal opens."""
    ...
    def __init__(self, target_channel, requirement_role, bonus_role, bonus_entries, attachment, ping_here):
        super().__init__()
