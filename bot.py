...
async def on_raw_message_delete(payload: discord.RawMessageDeleteEvent):
    ...
    await _log_deleted_message(payload.guild_id, payload.channel_id, payload.cached_message)
...
def extract_simple_sources(msg: discord.Message) -> list[str]:
    """Fast, local-only edit fingerprint for a message: attachment URLs and http(s) links in
    its own content. Unlike extract_video_sources, this never calls Discord's API (no reply
    or forward-snapshot lookups) — it's for deduplication in /previewvotes, where speed matters
    more than catching every edge case. A message with no attachment or link falls back to its
    own message ID, so it still counts as one entry rather than being silently dropped."""
    sources = [a.url for a in msg.attachments]
    sources += [w for w in msg.content.split() if w.startswith("http://") or w.startswith("https://")]
    return sources or [f"msg:{msg.id}"]
        ...
        async for msg in submit_channel.history(after=since, limit=None):
            ...
            try:
                ...
                if not message_pings_role(msg, picker_role):
                    continue
                sources = extract_simple_sources(msg)
                ...
                entries.append({"author_id": msg.author.id, "author_name": msg.author.display_name, "message_id": msg.id, "posted_at": msg.created_at})
            except Exception:
...
async def _log_deleted_message(guild_id: int, channel_id: int, cached_message):
    ...
    message was still in discord.py's internal cache when it got deleted — otherwise we still
    log the bare fact that *something* got deleted, just without content details."""
    config = load_guild_config(guild_id)
    log_channel_id = config.get("msglog_channel_id")
    if not log_channel_id:
        return
