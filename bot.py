...
RATE_EDIT_PROMPT = (
    ...
    "encouragement-for-its-own-sake — every claim you make has to be backed by something actually "
    "visible in these frames, nothing more, nothing less. Use the fact that these frames span the "
    "whole clip: compare them against each other to judge whether the color grading, exposure, "
    "and visual style stay CONSISTENT throughout, or whether quality drops off in places (e.g. "
    "strong grading early on that flattens out later, or a polished intro followed by a rushed-"
    "looking back half) — that consistency read is real signal you have here and should factor "
    "into the score, not just a single frame's look. Judge purely on what's actually visible — "
    "score based on genuine merit, not on hitting a preset distribution. Most casual submissions "
    "are genuinely mediocre — basic cuts, no real color grading, no sync/impact framing, generic "
    "look — call that out plainly and score it low, don't soften it or round it up out of pity. "
    "That said, being harsh is not the same as being wrong: don't invent flaws that aren't there "
    "and don't deflate a genuinely strong edit just to sound tough — if it's excellent, the facts "
    "say so, score it that way. Based only on what's visible in these frames, write a short, "
    "direct, fact-only review covering:\n"
    ...
    "torn between two scores, pick the lower one — err toward accuracy over kindness, always.\n"
    ...
    "guessing.\n"
    "4. 2-3 concrete, specific criticisms stated as flat fact, no hedging or softening language "
    "(skip phrases like 'maybe consider' or 'it might help to') — call out every real weakness "
    "visible (flat/unbalanced color grading, muddy composition, low-effort framing, generic look, "
    "etc.), but don't invent flaws in an edit that's genuinely well done — state plainly what's "
    "working instead, as a fact, not a compliment.\n"
    "No mercy, no pity, no encouragement fluff, no disclaimers about being AI or subjective — just "
    "the facts as they appear in the frames, stated directly. Keep it under 170 words. "
...
<div class="wrap">
  ...
  <form method="POST">
    ...
    <div class="card">
      ...
      <div id="bonusRows">
        ...
        <div class="tier-edit bonus-row">
          <div class="row4">
            ...
            <div>
              <label>Role/User ID</label>
              <input type="text" name="bonus_target[]" value="{{ row.target_id }}" placeholder="Paste ID">
            </div>
            <div>
              <label>Bonus %</label>
              <input type="number" name="bonus_percent[]" min="0" max="1000" value="{{ row.percent }}">
            </div>
      ...
      <template id="bonusRowTemplate">
        <div class="tier-edit bonus-row">
          <div class="row4">
            ...
            <div>
              <label>Role/User ID</label>
              <input type="text" name="bonus_target[]" value="" placeholder="Paste ID">
            </div>
            <div>
              <label>Bonus %</label>
              <input type="number" name="bonus_percent[]" min="0" max="1000" value="0">
            </div>
            <div style="display:flex; align-items:flex-end;">
              <button type="button" class="btn-remove-rank" onclick="this.closest('.bonus-row').remove()">✕ Remove</button>
            </div>
      ...
      <div class="hint">💡 Right-click a user/role → "Copy ID" (Discord Developer Mode must be on). Bonus stacks with every matching row plus the automatic booster bonus.</div>
...
def web_settings(guild_id):
    ...
    if request.method == 'POST':
        ...
        bonus_targets = request.form.getlist("bonus_target[]")
        ...
        for i in range(len(bonus_types)):
            ...
            raw_target = (bonus_targets[i] if i < len(bonus_targets) else "").strip()
            if kind not in ("user", "role") or not raw_target:
                continue
            digits = re.sub(r"[^0-9]", "", raw_target)
            if not digits:
