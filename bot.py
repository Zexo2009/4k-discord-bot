...
AI_CAPTION_PLATFORM_CHOICES = [
    app_commands.Choice(name="TikTok", value="TikTok"),
    app_commands.Choice(name="Instagram", value="Instagram"),
    app_commands.Choice(name="YouTube Shorts", value="YouTube Shorts"),
    app_commands.Choice(name="X (Twitter)", value="X (Twitter)"),
    app_commands.Choice(name="Allgemein / andere", value="social media"),
]
...
@bot.tree.command(name="ai-caption", description="AI-generated title, hashtags, and caption for a topic on your chosen platform")
@app_commands.describe(thema="Worum geht's? (Thema/Idee für den Post)", plattform="Für welche Plattform ist der Post?")
@app_commands.choices(plattform=AI_CAPTION_PLATFORM_CHOICES)
async def slash_ai_caption(interaction: discord.Interaction, thema: str, plattform: app_commands.Choice[str]):
    await interaction.response.defer()
    try:
        prompt = AI_CAPTION_PROMPT_TEMPLATE.format(platform=plattform.value, topic=thema)
        raw = await asyncio.get_running_loop().run_in_executor(
            None, _call_gemini_vision, [], prompt
        )
        parsed = _parse_content_info(raw)
        view = AICaptionView(thema, plattform.value)
        await interaction.followup.send(embed=_build_ai_caption_embed(parsed, plattform.value), view=view)
    except RuntimeError as e:
        await interaction.followup.send(f"⚠️ {e}")
    except Exception as e:
