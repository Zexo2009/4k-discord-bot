        ...
        async for msg in submit_channel.history(after=since, limit=None):
            ...
            try:
                ...
                for video_source, video_author in await extract_video_sources(msg):
                    ...
                    entries.append({"author_id": video_author.id, "author_name": video_author.display_name, "video_url": video_source, "message_id": msg.id})
            except Exception:
                # A single message failing to parse (e.g. a forward-snapshot fetch hiccup) must
                # never abort the whole scan — every other message, including ones after this
                # one in history, still needs to be checked.
                traceback.print_exc()
