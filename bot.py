...
HELP_CATEGORIES = {
    ...
    "editing": {
        ...
        "description": (
            ...
            "a link, original audio is kept. Free & self-hosted, so it runs a bit slow (CPU).\n\n"
...
@app_commands.describe(
    ...
    method="Removal algorithm — Hybrid works best for most edits",
    quality="Processing quality vs. speed — this runs on the bot's own CPU, so lower = faster",
)
...
async def slash_removesubs(
    ...
    await interaction.response.defer()
    chosen_method = method.value if method else "hybrid"
    chosen_quality = quality.value if quality else "480p"
    ...
    try:
        ...
        if duration and duration > MAX_REMOVESUBS_SECONDS:
            await interaction.followup.send(
                f"⚠️ That video is {int(duration)}s long — subtitle removal is capped at "
                f"{MAX_REMOVESUBS_SECONDS}s since this runs on the bot's own CPU (no paid GPU service)."
            )
            return
