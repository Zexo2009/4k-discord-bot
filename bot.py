...
async def on_ready():
    ...
    # failure anywhere earlier in on_ready can NEVER prevent this from running. Before this
    # fix, any exception above (e.g. a blocking DB call in migrate_legacy_data_once) silently
    # aborted the rest of on_ready and newly-added commands (like /giveaway) never got synced.
    try:
        synced = await asyncio.wait_for(bot.tree.sync(), timeout=30)
        print(f"Synced {len(synced)} slash command(s) globally")
    except asyncio.TimeoutError:
        print("Global slash command sync TIMED OUT after 30s (Discord API hung or unreachable)")
    except Exception as e:
        print(f"Global slash command sync failed: {e}")
    # Global sync can take up to an hour to actually show up for everyone — copying the
    # same command set as a guild-specific sync for every server the bot is in makes them
    # appear instantly instead (Discord de-dupes by name, so nothing shows twice).
    for guild in bot.guilds:
        try:
