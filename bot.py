...
async def _recreate_channel_from_snapshot(guild: discord.Guild, snapshot: dict):
    reason = "Automatic anti-raid: restoring a channel deleted during a raid"
    try:
        kind = snapshot["type"]
        if kind == "TextChannel":
            new_channel = await guild.create_text_channel(
                ...
                slowmode_delay=snapshot["slowmode_delay"], reason=reason,
            )
            try:
                await new_channel.send("🛡️ This channel was deleted during a raid and has been automatically restored by Zexo's anti-raid protection.")
            except discord.HTTPException:
                pass
        elif kind == "VoiceChannel":
            await guild.create_voice_channel(
                name=snapshot["name"], category=snapshot["category"], position=snapshot["position"],
                overwrites=snapshot["overwrites"], bitrate=snapshot["bitrate"], user_limit=snapshot["user_limit"], reason=reason,
            )
        elif kind == "CategoryChannel":
            await guild.create_category(
                name=snapshot["name"], position=snapshot["position"], overwrites=snapshot["overwrites"], reason=reason,
            )
        elif kind == "ForumChannel":
            await guild.create_forum(
                name=snapshot["name"], category=snapshot["category"], position=snapshot["position"],
                overwrites=snapshot["overwrites"], topic=snapshot["topic"] or None, reason=reason,
            )
        elif kind == "StageChannel":
            await guild.create_stage_channel(
                name=snapshot["name"], category=snapshot["category"], position=snapshot["position"],
                overwrites=snapshot["overwrites"], reason=reason,
            )
    except discord.Forbidden:
        print(f"⚠️ Anti-raid: missing permission to restore channel '{snapshot['name']}' in {guild.name}.")
    except discord.HTTPException as e:
        print(f"⚠️ Anti-raid: failed to restore channel '{snapshot['name']}': {e}")
...
async def on_guild_channel_delete(channel):
    ...
    # get restored; a mod deleting one channel on purpose should never see it pop back.
    # EXCEPT for channels we deleted ourselves as raid cleanup (see on_guild_channel_create)
    # — recreating a junk channel we just removed on purpose would defeat the whole point.
    ...
    if not deleter or not deleter.bot or not bot.user or deleter.id == bot.user.id:
        return  # not a bot deletion — never acted on, no matter how many happen
