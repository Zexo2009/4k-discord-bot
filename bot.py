...
def extract_simple_sources(msg: discord.Message):
    ...
    credited to ITS author rather than whoever replied. Unlike extract_video_sources, this
    never makes a network call: it only uses msg.reference.resolved, which Discord already
    includes for free on same-channel replies (no fetch needed) — it does NOT chase down a
    reference Discord didn't hand us locally, since that's exactly the slow path this avoids.

    Returns (sources, credited_author). A message with no attachment/link anywhere (its own or
    the original's) falls back to its own message ID as the fingerprint, so it still counts as
    one entry rather than being silently dropped."""
        ...
        async for msg in submit_channel.history(after=since, limit=None):
            ...
            try:
                ...
                if not message_pings_role(msg, picker_role):
                    continue
                sources, credited_author = extract_simple_sources(msg)
