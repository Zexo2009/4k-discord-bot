...
def default_guild_config() -> dict:
    ...
    return {
        ...
        "rankup_channel_id": RANKUP_CHANNEL_ID,
...
async def extract_simple_sources(msg: discord.Message):
    ...
    return (sources or [f"msg:{msg.id}"]), credited_author
      ...
      if results_channel:
        ...
        if winner_use_embed:
            ...
            plain_text = (
                ...
                f"{win_line}\n"
                f"🗳️ Votes: **{best_votes}** {vote_word} · 💰 Points earned: **+{winner_points}** (total: **{new_total}**)"
            )
...
HELP_CATEGORIES = {
    ...
    "moderation": {
        ...
        "description": (
            ...
            "**`/modlog ban #channel`** / **`/modlog kick #channel`** / **`/modlog timeout #channel`**\n"
            "Each gets logged automatically (moderator + reason) — point them at the same channel "
            "or different ones, your call.\n\n"
            "**`/modlog messages #channel`**\n"
            "Every edited or deleted message gets logged here (author, before/after, attachments "
            "like images/videos/gifs).\n\n"
            "**`/modlog off <ban|kick|timeout|messages>`**\n"
            "Turns one of the four logs back off.\n\n"
            "**`/timeout @user <duration> [reason]`** / **`/untimeout @user`**\n"
            "Times a member out (or removes one) — logged automatically.\n\n"
            "**`/partnership setup @role #category`**\n"
            "One-time setup: this role can post links, tickets get created in this category. "
            "Add more roles anytime with **`/partnership addrole @role`** / **`/partnership removerole @role`**, "
            "or manage the full list on the dashboard.\n\n"
            "**`/linkfilter toggle <on/off>`** / **`/linkfilter bypass @role`**\n"
            "Turns automatic link-deletion on/off, or lets an extra role post links too.\n\n"
            "**`/channelaccess add/remove/clear/list [@user] [@role] [#channel]`**\n"
            "Locks a specific user or role to only being able to type in certain channels."
        ),
    },
    "utility": {
        "label": "⚙️ Utility",
...
def build_help_home_embed():
    ...
    embed.add_field(
        ...
        value=(
            "🏅 Points, ranks & daily Top-Edit voting\n"
...
class PartnershipTicketView(discord.ui.View):
    ...
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not category:
            await interaction.response.send_message(
                "⚠️ The partnership system hasn't been set up yet (Staff: `/partnership setup`).", ephemeral=True
            )
            return
        tickets = load_tickets(interaction.guild.id)
...
partnership_group = app_commands.Group(name="partnership", description="Partnership ticket system")


@partnership_group.command(name="setup", description="Set up the partnership system (role + ticket category)")
@app_commands.describe(
    role="The role allowed to post promo/links (e.g. Partnership Recruiter)",
    category="Category where ticket channels are created",
)
async def partnership_setup(interaction: discord.Interaction, role: discord.Role, category: discord.CategoryChannel):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    config = load_guild_config(interaction.guild.id)
    roles = list(config.get("partnership_role_ids", []))
    if role.id not in roles:
        roles.append(role.id)
    save_guild_config(interaction.guild.id, {
        "partnership_role_ids": roles,
        "partnership_category_id": category.id,
        "linkfilter_enabled": True,
    })
    await interaction.response.send_message(
        f"✅ Partnership system set up!\n• Role: {role.mention}\n• Ticket category: {category.mention}\n"
        f"• Link filter was automatically enabled.\n"
        f"💡 Add more roles anytime with `/partnership addrole`, or manage the full list on the dashboard.",
        ephemeral=True,
    )


@partnership_group.command(name="addrole", description="Add another role allowed to post promo/links")
@app_commands.describe(role="The role to add")
async def partnership_addrole(interaction: discord.Interaction, role: discord.Role):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    config = load_guild_config(interaction.guild.id)
    ...
    if role.id not in roles:
        roles.append(role.id)
    save_guild_config(interaction.guild.id, {"partnership_role_ids": roles})
    await interaction.response.send_message(f"✅ {role.mention} can now post links.", ephemeral=True)


@partnership_group.command(name="removerole", description="Remove a role from the partnership list")
@app_commands.describe(role="The role to remove")
async def partnership_removerole(interaction: discord.Interaction, role: discord.Role):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    config = load_guild_config(interaction.guild.id)
    roles = [r for r in config.get("partnership_role_ids", []) if r != role.id]
    save_guild_config(interaction.guild.id, {"partnership_role_ids": roles})
    await interaction.response.send_message(f"✅ {role.mention} removed from the partnership list.", ephemeral=True)


bot.tree.add_command(partnership_group)
...
linkfilter_group = app_commands.Group(name="linkfilter", description="Automatische Link-Entfernung (Promo-Schutz)")


@linkfilter_group.command(name="toggle", description="Turn the link filter on/off")
@app_commands.describe(enabled="On or off")
async def linkfilter_toggle(interaction: discord.Interaction, enabled: bool):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    save_guild_config(interaction.guild.id, {"linkfilter_enabled": enabled})
    await interaction.response.send_message(f"✅ Link filter is now **{'ON' if enabled else 'OFF'}**.", ephemeral=True)


@linkfilter_group.command(name="bypass", description="Add a role that can still post links")
@app_commands.describe(role="Role")
async def linkfilter_bypass(interaction: discord.Interaction, role: discord.Role):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    config = load_guild_config(interaction.guild.id)
    bypass = list(config.get("linkfilter_bypass_role_ids", []))
    if role.id not in bypass:
        bypass.append(role.id)
    save_guild_config(interaction.guild.id, {"linkfilter_bypass_role_ids": bypass})
    await interaction.response.send_message(f"✅ {role.mention} can now post links.", ephemeral=True)


bot.tree.add_command(linkfilter_group)
...
channelaccess_group = app_commands.Group(name="channelaccess", description="Restrict users/roles to specific channels")


@channelaccess_group.command(name="add", description="Allow a user/role only certain channels")
@app_commands.describe(channel="Channel to allow", member="User (optional)", role="Role (optional)")
async def channelaccess_add(
    interaction: discord.Interaction, channel: discord.TextChannel, member: discord.Member = None, role: discord.Role = None
):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    if not member and not role:
        await interaction.response.send_message("❌ Provide either a user or a role.", ephemeral=True)
        return
    config = load_guild_config(interaction.guild.id)
    restricted = dict(config.get("restricted_access", {}))
    key = f"user:{member.id}" if member else f"role:{role.id}"
    chans = set(restricted.get(key, []))
    chans.add(channel.id)
    restricted[key] = list(chans)
    save_guild_config(interaction.guild.id, {"restricted_access": restricted})
    target_label = member.mention if member else role.mention
    await interaction.response.send_message(
        f"✅ {target_label} can now ONLY write in their allowed channels (just added: {channel.mention}).",
        ephemeral=True,
    )


@channelaccess_group.command(name="remove", description="Remove a channel from the allow list")
@app_commands.describe(channel="Channel", member="User (optional)", role="Role (optional)")
async def channelaccess_remove(
    interaction: discord.Interaction, channel: discord.TextChannel, member: discord.Member = None, role: discord.Role = None
):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
...
@channelaccess_group.command(name="clear", description="Remove the entire restriction for a target")
@app_commands.describe(member="User (optional)", role="Role (optional)")
async def channelaccess_clear(interaction: discord.Interaction, member: discord.Member = None, role: discord.Role = None):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    if not member and not role:
    ...
    restricted.pop(key, None)
    save_guild_config(interaction.guild.id, {"restricted_access": restricted})
    await interaction.response.send_message("✅ Restriction removed.", ephemeral=True)


@channelaccess_group.command(name="list", description="Show all channel restrictions")
async def channelaccess_list(interaction: discord.Interaction):
    config = load_guild_config(interaction.guild.id)
    restricted = config.get("restricted_access", {})
    if not restricted:
        await interaction.response.send_message("No restrictions active.", ephemeral=True)
        return
    lines = []
    for key, chans in restricted.items():
        kind, _, raw_id = key.partition(":")
        label = f"<@{raw_id}>" if kind == "user" else f"<@&{raw_id}>"
        chan_list = ", ".join(f"<#{c}>" for c in chans) or "—"
        lines.append(f"{label}: {chan_list}")
    embed = discord.Embed(title="🔒 Channel Restrictions", description="\n".join(lines), color=discord.Color.from_rgb(99, 102, 241))
    await interaction.response.send_message(embed=embed, ephemeral=True)


bot.tree.add_command(channelaccess_group)
...
modlog_group = app_commands.Group(name="modlog", description="Mod-log settings (ban/kick/timeout)")


@modlog_group.command(name="ban", description="Set the channel for ban logs")
@app_commands.describe(channel="Log channel")
async def modlog_ban(interaction: discord.Interaction, channel: discord.TextChannel):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    save_guild_config(interaction.guild.id, {"banlog_channel_id": channel.id})
    await interaction.response.send_message(f"✅ Ban-log channel set: {channel.mention}", ephemeral=True)


@modlog_group.command(name="kick", description="Set the channel for kick logs")
@app_commands.describe(channel="Log channel")
async def modlog_kick(interaction: discord.Interaction, channel: discord.TextChannel):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    save_guild_config(interaction.guild.id, {"kicklog_channel_id": channel.id})
    await interaction.response.send_message(f"✅ Kick-log channel set: {channel.mention}", ephemeral=True)


@modlog_group.command(name="timeout", description="Set the channel for timeout/untimeout logs")
@app_commands.describe(channel="Log channel")
async def modlog_timeout(interaction: discord.Interaction, channel: discord.TextChannel):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    save_guild_config(interaction.guild.id, {"timeoutlog_channel_id": channel.id})
    await interaction.response.send_message(f"✅ Timeout-log channel set: {channel.mention}", ephemeral=True)


@modlog_group.command(name="messages", description="Set the channel for edited/deleted messages")
@app_commands.describe(channel="Log channel")
async def modlog_messages(interaction: discord.Interaction, channel: discord.TextChannel):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    save_guild_config(interaction.guild.id, {"msglog_channel_id": channel.id})
    await interaction.response.send_message(f"✅ Message-log channel set: {channel.mention}", ephemeral=True)


@modlog_group.command(name="off", description="Turn off a log")
@app_commands.describe(which="Which log to turn off")
@app_commands.choices(which=[
    app_commands.Choice(name="Ban", value="ban"),
    app_commands.Choice(name="Kick", value="kick"),
    app_commands.Choice(name="Timeout", value="timeout"),
    app_commands.Choice(name="Messages (edited/deleted)", value="messages"),
])
async def modlog_off(interaction: discord.Interaction, which: app_commands.Choice[str]):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    field = "msglog_channel_id" if which.value == "messages" else f"{which.value}log_channel_id"
    save_guild_config(interaction.guild.id, {field: None})
    await interaction.response.send_message(f"✅ {which.name} log disabled.", ephemeral=True)


bot.tree.add_command(modlog_group)
...
@bot.tree.command(name="timeout", description="Time out a member (Staff/Manager)")
@app_commands.describe(member="Member", duration="Duration, e.g. 10m, 1h, 1d (max 28 days)", reason="Reason (optional)")
async def slash_timeout(interaction: discord.Interaction, member: discord.Member, duration: str, reason: str = None):
    ...
    if seconds is None or seconds < 10:
        await interaction.response.send_message("❌ Invalid duration. Examples: `10m`, `1h`, `1d`", ephemeral=True)
        return
    seconds = min(seconds, 28 * 24 * 3600)
    try:
        await member.timeout(discord.utils.utcnow() + timedelta(seconds=seconds), reason=reason or f"Timeout by {interaction.user}")
    except discord.Forbidden:
        await interaction.response.send_message("❌ I'm missing permission for that.", ephemeral=True)
        return
    await interaction.response.send_message(
...
@bot.tree.command(name="untimeout", description="Remove a member's timeout (Staff/Manager)")
...
async def slash_untimeout(interaction: discord.Interaction, member: discord.Member):
    ...
    try:
        ...
        await interaction.response.send_message("❌ I'm missing permission for that.", ephemeral=True)
        return
    await interaction.response.send_message(f"🔊 {member.mention}'s timeout was removed.")
...
<div class="wrap">
  ...
  <div class="panel" id="moderation">
    <h2>🔇 Moderation</h2>
    <div class="desc">Time out a member for a set duration, or remove an active timeout — same as the <code>/timeout</code> / <code>/untimeout</code> Discord commands. Logged to the timeout-log channel if one is set on the Settings page.</div>
    <div class="row3">
