            ...
            try:
                if config.get("partnership_role_id"):
                    desc = (
                        ...
                        f"<@&{config['partnership_role_id']}> role.\n\n"
...
@modlog_group.command(name="channel", description="Set the channel for ban/kick/timeout logs")
...
async def modlog_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await interaction.response.send_message(f"✅ Mod-log channel set: {channel.mention}", ephemeral=True)
...
@modlog_group.command(name="off", description="Turn off a mod-log")
@app_commands.describe(which="Which log to turn off (default: ban/kick/timeout)")
...
async def modlog_off(interaction: discord.Interaction, which: app_commands.Choice[str] = None):
    ...
    if which and which.value == "messages":
        ...
        await interaction.response.send_message("✅ Message log disabled.", ephemeral=True)
    else:
        save_guild_config(interaction.guild.id, {"modlog_channel_id": None})
        await interaction.response.send_message("✅ Ban/Kick/Timeout log disabled.", ephemeral=True)
...
<div class="wrap">
  ...
  <form method="POST">
    ...
    <div class="card">
      <h2>🚨 Mod-Log</h2>
      <div class="desc">Bans, kicks, and timeouts are logged here automatically (moderator + reason).</div>
      <label>Mod-log channel (ban/kick/timeout)</label>
      <select name="modlog_channel_id">
        <option value="0" {% if not config.modlog_channel_id %}selected{% endif %}>— Off —</option>
      ...
      <div class="hint">💡 Logs every edited or deleted message — including images, videos, GIFs, and links — with author and content.</div>
    </div>

    <div class="card">
      <h2>🔗 Link Filter &amp; Partnership</h2>
      <div class="desc">Automatically removes links/promo from anyone without the partnership role, and offers a ticket system for it instead.</div>
      ...
      <label>Partnership Recruiter role (allowed to post links)</label>
      <select name="partnership_role_id">
        <option value="0" {% if not config.partnership_role_id %}selected{% endif %}>— none —</option>
      ...
      <div class="hint">💡 Staff/Manager and the partnership role can always post links. Giveaways are managed on their own <a href="/dashboard/{{ guild_id }}/giveaways" style="color:#c4b5fd;">🎉 Giveaways page</a>.</div>
    </div>
