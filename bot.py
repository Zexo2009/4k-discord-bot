...
# --- First-submission tracking: the AI-rating hint only starts showing from someone's SECOND
# video submission onward, never their first. Two tips landing on one brand-new post (the ping
# reminder + the AI hint) felt like spam — this way new posters get one thing at a time. ---
def load_has_submitted_users(guild_id: int) -> set:
    raw = db_get_raw(f"has_submitted:{guild_id}")
    if raw is not None:
        try:
            return set(json.loads(raw))
        except json.JSONDecodeError:
            return set()
    path = f"has_submitted_{guild_id}.json"
    if not os.path.exists(path):
        return set()
    try:
        with open(path, "r") as f:
...
def mark_user_has_submitted(guild_id: int, user_id: int):
    users = load_has_submitted_users(guild_id)
    users.add(user_id)
    data = list(users)
    if db_set_raw(f"has_submitted:{guild_id}", json.dumps(data)):
        return
    with open(f"has_submitted_{guild_id}.json", "w") as f:
        json.dump(data, f)
    ...
    if message.guild and not message.author.bot:
        ...
        if submit_channel and picker_role and message.channel.id == submit_channel.id:
            ...
            if sources_for_rating_hint:
                has_submitted_before = message.author.id in load_has_submitted_users(message.guild.id)
                if not has_submitted_before:
                    # Their very first submission ever — just remember it happened, don't
                    # hint yet. The ping reminder below (if needed) is the only thing that
                    # should land on this first post.
                    mark_user_has_submitted(message.guild.id, message.author.id)
                else:
...
async def airating_cmd(ctx, *, video_url: str = None):
    ...
    async with ctx.typing():
        ...
        if not video_path:
            await ctx.send(
                "⚠️ I can't find a video there — attach one, reply to a message with a video, "
                "or give a link: `.airating <link>`."
            )
            return
...
RATE_EDIT_PROMPT = (
    "You are a harsh, veteran video editor judging frames pulled from someone's edit for a "
    "competitive server where only the best submissions should score well. Grade like a strict "
    "judge, not a supportive friend — most edits are mediocre and should be scored accordingly. "
    "Do NOT round scores up out of politeness, and do not soften real flaws with vague praise. "
    "Based only on what's visible in these frames, write a short, direct review covering:\n"
    "1. An overall score out of 10 for visual polish (grading, sync/impact framing, composition). "
    "Reserve 8-10 only for genuinely excellent, near-flawless work; a merely decent/average edit "
    "should land around 4-6; a rough or amateurish one should score 1-3. Do not be generous.\n"
    "2. A skill tier that best fits this edit — pick exactly one of: Beginner, Intermediate, "
    "Advanced, Pro. Label it clearly as 'Skill tier: X'. Default to a lower tier unless the frames "
    "clearly show advanced technique — most submissions are Beginner or Intermediate, and 'Pro' "
    "should be rare.\n"
    ...
    "4. 2-3 concrete, specific criticisms and tips to improve — call out actual weaknesses "
    "(flat/unbalanced color grading, muddy composition, low-effort framing, generic look, etc.) "
    "rather than only offering gentle suggestions.\n"
    "Be blunt and critical, not falsely encouraging — this is tough, honest feedback, not a pep "
    "talk. Keep it under 170 words. "
