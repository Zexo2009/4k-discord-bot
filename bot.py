    ...
    try:
      if results_channel:
        ...
        if is_booster and config.get("booster_bonus_enabled", True):
            bonus_percent = config.get("booster_bonus_percent", 50)
            bonus_points = round(winner_points * bonus_percent / 100)
        ...
        if bonus_points:
            points_value = f"**+{winner_points}** base + 💎 **+{bonus_points}** booster bonus = **+{total_award}** (total: **{new_total}**)"
            points_plain = f"+{winner_points} base + 💎 +{bonus_points} booster bonus = +{total_award} (total: {new_total})"
        else:
