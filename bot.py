...
async def run_collect_job_for_guild(guild: discord.Guild):
    ...
    # Only the CURRENT window (today's collect_hour to now) — a missed day is skipped rather
    # than swept into today's poll. Same current_collect_window_start() /previewvotes uses, so
    # what you preview always matches exactly what the real run will pick up.
    since = current_collect_window_start(config)
