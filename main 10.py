...
@bot.command(name="vhqax")
async def vhqax_cmd(ctx, url: str = None):
    if url is None:
        await ctx.send("Please provide a video URL, e.g. `!vhqax https://...`")
        return
    status_msg = await ctx.send("⏳ Downloading video...")
...
def build_help_embed():
    ...
    embed.add_field(name="/vhqax or !vhqax <url>", value="Downloads a video and extracts its audio as MP3.", inline=False)
...
@bot.tree.command(name="vhqax", description="Download a video and extract its audio as MP3")
@app_commands.describe(url="Video URL (TikTok, direct link, etc.)")
async def slash_vhqax(interaction: discord.Interaction, url: str):
    await interaction.response.send_message("⏳ Downloading video...")
    status_msg = await interaction.original_response()
