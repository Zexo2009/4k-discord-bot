...
async def purge_messages_with_word(channel: discord.TextChannel, word: str, scan_limit: int = 10000) -> int:
...
async def purgeword(ctx, channel: discord.TextChannel = None, *, word: str = None):
    ...
    if not channel.permissions_for(ctx.guild.me).manage_messages:
        ...
        return
    status = await ctx.send(f"⏳ Deleting every message containing **{word}** in {channel.mention}... this can take a bit for large channels.")
...
async def slash_purgeword(interaction: discord.Interaction, channel: discord.TextChannel, word: str):
    ...
    if not channel.permissions_for(interaction.guild.me).manage_messages:
        ...
        return
    await interaction.response.send_message(f"⏳ Deleting every message containing **{word}** in {channel.mention}... this can take a bit for large channels.")
    deleted = await purge_messages_with_word(channel, word)
