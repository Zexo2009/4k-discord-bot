...
def web_dashboard():
    ...
    entries_count = len(current_poll.get("entries") or [])
...
def save_badwords(words):
    if db_set_raw("badwords", json.dumps(words)):
        return
    with open(BADWORDS_FILE, "w") as f:
        json.dump(words, f)
...
HELP_CATEGORIES = {
    "points": {
        ...
        "description": (
            ...
            "(has a *Press for more info* button showing your exact leaderboard standing)\n"
            "`ranks` — See every rank, its threshold, and how to earn points\n"
            "`leaderboard` — Top 20 point rankings, 1-10 then swipe for 11-20"
        ),
    },
    "voting": {
        ...
        "description": (
            ...
            "`previewvotes` — See which edits are currently in today's vote *(only visible to you)*"
        ),
    },
    "staff": {
        "label": "🛠️ Staff Tools",
        "description": (
            "*Top Edit Picker Manager / Staff only*\n\n"
            "`testcollect` — Manually run the collect job\n"
            "`testresults` — Manually run the results job\n"
            "`addpoints @user <amount>` — Add points *(only visible to you)*\n"
            "`removepoints @user <amount>` — Remove points *(only visible to you)*\n"
            "`purgeword #channel word` — Delete every message with that word\n"
            "`badwords add/remove/list <word>` — Manage the risk-word list\n"
            "`purgebadwords #channel|all` — Purge every risk-word match, one channel or ALL of them"
        ),
    },
    "utility": {
        "label": "⚙️ Utility",
        "description": (
            ...
            "`tutorial` — Full walkthrough of how the Top Edit system works\n"
            "`flip` — Flip a coin"
        ),
    },
}
...
def build_help_home_embed():
    ...
    for cat in HELP_CATEGORIES.values():
        embed.add_field(name=cat["label"], value=cat["description"], inline=False)
    embed.set_footer(text="Tip: use the buttons to jump straight to a category.")
    return embed
...
def build_tutorial_embed():
    ...
    embed.add_field(
        ...
        value=(
            f"Post your edit video in <#{SUBMIT_CHANNEL_ID}> and **@mention the {PICKER_ROLE_NAME} role** "
    ...
    embed.add_field(
        ...
        value=(
            f"Every day at **{COLLECT_HOUR}:00 (Europe/Berlin)**, the bot scans that channel for new "
    ...
    embed.add_field(
        name="3️⃣ Voting",
        value=(
            f"The bot posts every collected edit in <#{DISCUSSION_CHANNEL_ID}> and opens a poll, pinging "
            f"**@{PICKER_ROLE_NAME}**. Voting stays open for **{POLL_DURATION_HOURS} hours**, closing at "
            "**{}:00**. Use `/previewvotes` any time to privately see what's currently in the vote.".format(RESULTS_HOUR)
        ),
        inline=False,
    )
    embed.add_field(
        ...
        value=(
            f"When voting ends, the bot tallies the poll and announces the winner in <#{RESULTS_CHANNEL_ID}>. "
...
async def build_leaderboard_embed(guild: discord.Guild, page: int = 1):
    ...
    if not page_entries:
        ...
        embed.set_footer(text=f"Day {load_day()} · {total_editors} editor(s) total")
        return embed
    ...
    embed.set_footer(text=f"Day {load_day()} · Page {page}/{total_pages} · {total_editors} editor(s) total")
    return embed


class LeaderboardView(discord.ui.View):
    """Top 10, with a swipe button to see 11-20. Nothing beyond that."""
    ...
    async def prev_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.page = 1
        self._sync_buttons()
        await interaction.response.edit_message(embed=await build_leaderboard_embed(self.guild, self.page), view=self)
...
async def build_points_standing_embed(guild: discord.Guild, member: discord.Member) -> discord.Embed:
    """Full leaderboard standing for one member: their server-wide rank and the 5 editors closest above them."""
    data = load_points()
    total = data.get(str(member.id), 0)
    sorted_entries = sorted(data.items(), key=lambda x: x[1], reverse=True)
    total_editors = len(sorted_entries)
...
async def tutorial(ctx):
    await ctx.send(embed=build_tutorial_embed())
...
async def leaderboard(ctx):
    ...
    embed = await build_leaderboard_embed(ctx.guild, page=1)
    view.message = await ctx.send(embed=embed, view=view)


@bot.command()
async def previewvotes(ctx):
    entries = current_poll.get("entries") or []
    if not entries:
        try:
            await ctx.author.send("📭 No edits are currently in the vote.")
        except discord.Forbidden:
            await ctx.send("📭 No edits are currently in the vote. *(couldn't DM you — check your privacy settings)*")
            return
    else:
        lines = [f"**{i}.** {entry['author_name']} — {entry['video_url']}" for i, entry in enumerate(entries, start=1)]
        embed = discord.Embed(
            title="🗳️ Current Vote Preview",
            description="\n".join(lines),
            color=discord.Color.blurple(),
        )
        embed.set_footer(text=f"{len(entries)} edit(s) currently in the vote")
        try:
...
async def slash_tutorial(interaction: discord.Interaction):
    await interaction.response.send_message(embed=build_tutorial_embed())
...
@bot.tree.command(name="previewvotes", description="Preview which edits are currently in today's vote (only visible to you)")
async def slash_previewvotes(interaction: discord.Interaction):
    entries = current_poll.get("entries") or []
    if not entries:
        await interaction.response.send_message("📭 No edits are currently in the vote.", ephemeral=True)
        return
    ...
    embed.set_footer(text=f"{len(entries)} edit(s) currently in the vote · Only visible to you")
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def slash_leaderboard(interaction: discord.Interaction):
    ...
    embed = await build_leaderboard_embed(interaction.guild, page=1)
