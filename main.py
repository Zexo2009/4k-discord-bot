...
def oauth_callback():
    ...
    session["user"] = {
        ...
        "guilds": [{"id": g["id"], "name": g["name"], "permissions": g.get("permissions", "0")} for g in guilds],
    }
    return redirect(url_for('web_dashboard_default'))
...
def home():
    ...
    <style>
      * { box-sizing:border-box; }
      ...
      h1 { font-size:3.2rem; font-weight:800; margin:0; letter-spacing:-0.02em; line-height:1.05;
      ...
      p.tag { color:#9a9ab0; margin:16px 0 30px; max-width:560px; font-size:1.05rem; line-height:1.5; }
      ...
      a.secondary { display:inline-block; padding:14px 32px; border-radius:999px; background:rgba(255,255,255,0.06);
          border:1px solid rgba(255,255,255,0.14); color:#f4f4f8; text-decoration:none; font-weight:600; }
      .scroll-hint { position:absolute; bottom:30px; color:#6b6b80; font-size:0.8rem; animation:bounce 2s infinite; }
      ...
      .card { background:rgba(20,20,31,0.6); border:1px solid rgba(255,255,255,0.08); border-radius:18px;
              padding:26px; backdrop-filter:blur(12px); transition:transform 0.2s ease, border-color 0.2s ease; }
      .card:hover { transform:translateY(-4px); border-color:rgba(139,92,246,0.5); }
    ...
    <body>
      ...
      <section id="features">
        <h2>Everything the bot does</h2>
        <div class="sub">One system, fully automated, every single day.</div>
        <div class="grid">
          <div class="card"><div class="icon">🎬</div><h3>Any video source</h3>
          ...
          <div class="card"><div class="icon">🗳️</div><h3>Daily live polls</h3>
          ...
          <div class="card"><div class="icon">🏆</div><h3>Points &amp; ranks</h3>
            <p>Winners earn points automatically, rank roles sync themselves, and the leaderboard updates in real time.</p></div>
          <div class="card"><div class="icon">⚙️</div><h3>Full web dashboard</h3>
            <p>Log in with Discord and manage everything from your browser — no memorizing slash commands.</p></div>
          <div class="card"><div class="icon">✍️</div><h3>100% customizable text</h3>
            <p>Winner announcements, reminders, and poll questions — write your own, or let Zexo pick from 100+ built-in lines.</p></div>
          <div class="card"><div class="icon">🚫</div><h3>Badword filtering</h3>
      ...
      <section>
        <h2>How it works</h2>
        <div class="sub">Set up once, runs itself every day after that.</div>
        <div class="steps">
      ...
      <footer>
        Built by <span class="void">Void HQ</span> — the developer server behind Zexo.
      </footer>
    </body></html>
    """
...
<body>
<div class="wrap">
  <a class="back" href="/dashboard/{{ guild_id }}">← Back to dashboard</a>
  ...
  <form method="POST" class="card">
    ...
    <div class="row">
      <div>
        <label>Collect hour (Europe/Berlin, 0-23)</label>
        <input type="number" name="collect_hour" min="0" max="23" value="{{ config.collect_hour }}">
      </div>
      <div>
        <label>Results hour (Europe/Berlin, 0-23)</label>
    ...
    if request.method == 'POST':
        ...
        for field in ("submit_channel_id", "discussion_channel_id", "results_channel_id", "log_channel_id"):
            ...
            if val in channel_ids:
                updates[field] = val
        for field in ("picker_role_id", "manager_role_id", "staff_role_id", "top_edits_role_id"):
            try:
                val = int(request.form.get(field, 0))
            except ValueError:
                val = 0
...
<div class="wrap">
  ...
  <div class="panel" id="triggers">
    ...
    <div class="desc">Force today's collect or results step right now instead of waiting for the scheduled hour.
      ⚠️ Runs for every server this bot is in, not just this one.</div>
...
def api_trigger(guild_id):
    ...
    action = data.get("action")
    if action == "collect":
        run_on_bot_loop(run_collect_job(), timeout=120)
        return {"ok": True, "message": "Collect run finished."}
    if action == "results":
        run_on_bot_loop(run_results_job(), timeout=120)
        return {"ok": True, "message": "Results run finished."}
    return {"ok": False, "error": "Unknown action."}, 400
...
async def run_collect_job():
    ...
    another's. No cap on how many edits get collected: every qualifying, not-recently-used
    edit posted since the last run is included, however many that is."""
    for guild in bot.guilds:
...
async def run_results_job():
    """Runs independently for every server: each resolves its own poll(s) and announces its
