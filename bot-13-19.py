        ...
        if submit_channel and picker_role and message.channel.id == submit_channel.id:
            ...
            if sources and not pinged:
                ...
                if message.author.id not in reminded:
                    ...
                    reminder_embed.set_thumbnail(url=message.author.display_avatar.url)
            ...
            if sources and message.author.id in load_has_pinged_users(message.guild.id):
                ...
                if message.author.id not in hint_key_users:
                    ...
                    hint_embed = discord.Embed(
                        title="✨ Get AI Feedback on This Edit",
                        description=(
                            "> Run **`.airating`** for a score, software guess, and tips — "
                            "free, takes a few seconds."
                        ),
                        color=discord.Color.from_rgb(34, 255, 176),  # brand teal/green
                        timestamp=discord.utils.utcnow(),
                    )
                    hint_embed.set_thumbnail(url=message.author.display_avatar.url)
                    hint_embed.set_footer(
                        text="Zexo • AI Tools",
                        icon_url=bot.user.display_avatar.url if bot.user else discord.Embed.Empty,
                    )
                    try:
...
def _yt_dlp_download(url: str, out_path: str, max_bytes: int):
    ydl_opts = {
        ...
        "extractor_args": {
            "youtube": {"player_client": ["web", "tv", "android", "ios"]},
        },
        "http_headers": {
