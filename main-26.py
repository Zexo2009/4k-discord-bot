...
def home():
    ...
    <style>
      * { box-sizing:border-box; }
      ...
      /* --- scroll reveal: fades in AND fades back out as it leaves the viewport --- */
      .reveal { opacity:0; transform:translateY(38px); transition:opacity 0.7s cubic-bezier(.2,.8,.2,1), transform 0.7s cubic-bezier(.2,.8,.2,1); }
      .reveal.in { opacity:1; transform:translateY(0); }
      .reveal.out-up { opacity:0; transform:translateY(-38px); }

      .grid { display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:20px; }
      .card { background:rgba(20,20,31,0.6); border:1px solid rgba(255,255,255,0.08); border-radius:18px;
              padding:26px; backdrop-filter:blur(12px); transition:transform 0.25s ease, border-color 0.25s ease, opacity 0.6s ease, transform 0.6s ease; }
    ...
    <body>
      ...
      <section id="features">
        ...
        <div class="grid">
          <div class="card reveal"><div class="icon">🎬</div><h3>Any video source</h3>
          ...
          <div class="card reveal"><div class="icon">🗳️</div><h3>Daily live polls</h3>
...
<style>
  * { box-sizing:border-box; }
  ...
  .sub { color:#9a9ab0; font-size:0.9rem; margin-bottom:26px; }
  ...
  .card { display:flex; align-items:center; justify-content:space-between; gap:14px;
          background:rgba(20,20,31,0.65); border:1px solid rgba(255,255,255,0.08); border-radius:16px;
          padding:18px 20px; margin-bottom:14px; backdrop-filter:blur(14px); transition:transform 0.2s ease, border-color 0.2s ease; }
  .card:hover { transform:translateY(-3px); border-color:rgba(139,92,246,0.4); }
  .card .name { font-weight:700; font-size:1.02rem; }
  .card .tag { display:inline-block; margin-top:5px; font-size:0.72rem; padding:3px 10px; border-radius:999px; }
  ...
  a.btn { display:inline-block; padding:9px 16px; border-radius:999px; text-decoration:none; font-weight:600; font-size:0.82rem;
...
<div class="wrap">
  <div class="top">
    <div><h1>⚡ Your Servers</h1><div class="sub" style="margin-bottom:0;">Logged in as {{ user.username }}</div></div>
    <a class="logout" href="/logout">Log out</a>
  </div>
  <div class="sub">Pick a server to open its full settings — every channel, role, rank, timing and text is customizable per server.</div>

  {% for s in servers %}
  <div class="card">
    <div>
      <div class="name">{{ s.name }}</div>
      {% if not s.bot_present %}<span class="tag missing">Zexo not in this server</span>
    ...
    <div class="btns">
      ...
      {% else %}
        <a class="btn ghost" href="{{ invite_url }}" target="_blank">Invite Zexo</a>
      {% endif %}
    </div>
  </div>
  {% else %}
...
<style>
  * { box-sizing: border-box; }
  ...
  .tier-row { display:grid; grid-template-columns:60px 1fr; gap:10px; align-items:center; margin-bottom:10px; }
  .tier-badge { text-align:center; font-weight:700; font-size:0.85rem; padding:6px 0; border-radius:8px;
...
<div class="wrap">
  ...
  <form method="POST">
    ...
    <div class="card">
      <h2>🏆 Ranks &amp; points</h2>
      <div class="desc">The role given at each point threshold, plus how many points a win is worth.</div>
      <label>Unranked role (0 points)</label>
      ...
      <div class="tier-row">
        <div class="tier-badge">{{ emoji }} {{ tier_label }}<br><span style="font-weight:400; color:#9a9ab0; font-size:0.68rem;">{{ threshold }}+ pts</span></div>
    ...
    <div class="card">
      ...
      <div class="hint">Note: this only changes how long a poll stays open, not the results time above — keep the poll duration shorter than the gap between collect and results so it's closed by the time results run.</div>
    </div>
