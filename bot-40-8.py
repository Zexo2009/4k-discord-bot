...
# Render (and most non-TTY hosts) block-buffer stdout by default, so plain print()
# calls can sit unseen in a buffer for a long time instead of showing up in the log
# viewer right away — while discord.py's own logging-module output always appears
# instantly (it flushes per line already). Forcing line-buffering here means every
# print() in this file (existing ones, plus the diagnostic ones added for the
# on_ready/sync fixes) shows up immediately too, so the logs are actually trustworthy.
...
RANKUP_CHANNEL_ID = 1459696021335703583  # where rank-up announcements get posted
...
# --- /removesubs: FREE, self-hosted removal of burned-in subtitles/text (YOLO ONNX +
# OpenCV inpainting, ported from the open-source hjunior29/video-text-remover model —
# see text_remover.py for full setup instructions and performance notes). No external
# API, no per-run cost — runs on the bot's own CPU, which also means it's noticeably
# slower than a paid GPU service (budget minutes, not seconds, per video).
MAX_REMOVESUBS_SECONDS = 90  # keep runtime sane on CPU-only hosting
...
_bot_channel_delete_events = {}  # (guild_id, user_id) -> list of {"snapshot", "time", "handled"}
...
# Manual channel restore (/restorechannel, /deletedchannels) — separate from the anti-raid
# burst detection above. EVERY channel deletion (human staff cleaning up, a bot, anything)
# gets a snapshot recorded here so staff can bring one back on purpose later, even when it
# was never a raid and the automatic system above never touched it.
_recent_deleted_channels = {}  # guild_id -> list of {"snapshot", "deleted_at", "deleter", "restored"} (newest first)
MAX_TRACKED_DELETED_CHANNELS = 15
CHANNEL_TYPE_EMOJI = {
    "TextChannel": "#️⃣",
    "VoiceChannel": "🔊",
    "CategoryChannel": "📁",
...
def _record_general_channel_delete(guild_id: int, snapshot: dict, deleter_label: str):
    """Keeps a rolling history (newest first, capped) of every deleted channel so
    /restorechannel and /deletedchannels always have something to work with — independent of
    whether the anti-raid burst detector above ever gets involved."""
    entries = _recent_deleted_channels.setdefault(guild_id, [])
    entries.insert(0, {
        "snapshot": snapshot,
        "deleted_at": discord.utils.utcnow(),
        "deleter": deleter_label,
        "restored": False,
    })
    del entries[MAX_TRACKED_DELETED_CHANNELS:]


def build_deleted_channels_list(guild_id: int) -> str:
    entries = _recent_deleted_channels.get(guild_id) or []
    if not entries:
        return "No recently deleted channels tracked yet — delete one and it'll show up here."
    now = discord.utils.utcnow()
    lines = []
    for i, e in enumerate(entries, start=1):
        snap = e["snapshot"]
        emoji = CHANNEL_TYPE_EMOJI.get(snap["type"], "❓")
        ago = format_timedelta(now - e["deleted_at"])
        status = " *(already restored)*" if e["restored"] else ""
        lines.append(f"**{i}.** {emoji} `{snap['name']}` — deleted {ago} ago by {e['deleter']}{status}")
    return "\n".join(lines)


async def _do_restore_channel(guild: discord.Guild, actor_label: str, index: int) -> str:
    """Restores the Nth most recently deleted channel tracked for this guild (1 = most
    recent). Returns a human-readable result message for the command to send back."""
    entries = _recent_deleted_channels.get(guild.id) or []
    if not entries:
        return "❌ No recently deleted channels tracked for this server yet."
    if index < 1 or index > len(entries):
        return f"❌ Invalid number — pick between **1** and **{len(entries)}**. Use `/deletedchannels` to see the list."
    entry = entries[index - 1]
    if entry["restored"]:
        return f"⚠️ `{entry['snapshot']['name']}` was already restored earlier — check the server, it should still be there."
    new_channel = await _recreate_channel_from_snapshot(
        guild, entry["snapshot"],
        audit_reason=f"Manually restored via /restorechannel by {actor_label}",
        post_message=f"♻️ This channel was deleted and has just been restored by {actor_label}.",
    )
    if new_channel is None:
        return f"❌ Couldn't restore `{entry['snapshot']['name']}` — I'm likely missing the **Manage Channels** permission."
    entry["restored"] = True
    mention = getattr(new_channel, "mention", None) or f"**{new_channel.name}**"
    return f"✅ Restored **{entry['snapshot']['name']}** — it's back as {mention}."
...
def _record_channel_delete_event(guild_id: int, user_id: int, snapshot: dict, window_seconds: int):
    """Same pattern again, but for deletions — remembers a full snapshot of each deleted
    channel (name, type, position, category, overwrites, etc.) so it can be rebuilt exactly
    once a delete-burst is confirmed as a raid. A single deletion is never restored — see the
    threshold check in on_guild_channel_delete."""
    key = (guild_id, user_id)
    now = discord.utils.utcnow()
    events = _bot_channel_delete_events.setdefault(key, [])
    events[:] = [e for e in events if (now - e["time"]).total_seconds() <= window_seconds]
    events.append({"snapshot": snapshot, "time": now, "handled": False})
    return events


async def _recreate_channel_from_snapshot(
    guild: discord.Guild,
    snapshot: dict,
    audit_reason: str = "Automatic anti-raid: restoring a channel deleted during a raid",
    post_message: str = "🛡️ This channel was deleted during a raid and has been automatically restored by Zexo's anti-raid protection.",
):
    """Rebuilds a channel from a snapshot taken in on_guild_channel_delete. Used both by the
    automatic anti-raid burst-restore AND by the manual /restorechannel command — the
    audit_reason/post_message let each caller phrase it appropriately. Returns the new
    channel object on success, or None if it couldn't be created (missing permissions, etc.)."""
    new_channel = None
    try:
        kind = snapshot["type"]
        if kind == "TextChannel":
            new_channel = await guild.create_text_channel(
                name=snapshot["name"], category=snapshot["category"], position=snapshot["position"],
                overwrites=snapshot["overwrites"], topic=snapshot["topic"], nsfw=snapshot["nsfw"],
                slowmode_delay=snapshot["slowmode_delay"], reason=audit_reason,
            )
            if post_message:
                try:
                    await new_channel.send(post_message)
                except discord.HTTPException:
                    pass
        elif kind == "VoiceChannel":
            new_channel = await guild.create_voice_channel(
                name=snapshot["name"], category=snapshot["category"], position=snapshot["position"],
                overwrites=snapshot["overwrites"], bitrate=snapshot["bitrate"], user_limit=snapshot["user_limit"], reason=audit_reason,
            )
        elif kind == "CategoryChannel":
            new_channel = await guild.create_category(
                name=snapshot["name"], position=snapshot["position"], overwrites=snapshot["overwrites"], reason=audit_reason,
            )
        elif kind == "ForumChannel":
            new_channel = await guild.create_forum(
                name=snapshot["name"], category=snapshot["category"], position=snapshot["position"],
                overwrites=snapshot["overwrites"], topic=snapshot["topic"] or None, reason=audit_reason,
            )
        elif kind == "StageChannel":
            new_channel = await guild.create_stage_channel(
                name=snapshot["name"], category=snapshot["category"], position=snapshot["position"],
                overwrites=snapshot["overwrites"], reason=audit_reason,
            )
        return new_channel
    except discord.Forbidden:
        print(f"⚠️ Failed to restore channel '{snapshot['name']}' in {guild.name}: missing permission.")
        return None
    except discord.HTTPException as e:
        print(f"⚠️ Failed to restore channel '{snapshot['name']}': {e}")
        return None
...
def get_poll_state(guild_id: int) -> dict:
    return current_polls.setdefault(guild_id, {"polls": [], "entries": [], "disqualified": set()})
...
_last_snapshot_prune_date = None  # date() the message-snapshot cleanup last ran (global, not per-guild)
...
def has_channel_restore_permission(member: discord.Member) -> bool:
    """Administrator, Manage Channels, or Top Edit Manager/Staff — restoring a deleted
    channel needs roughly the same trust level as being able to delete one in the first place."""
...
def get_db_connection():
    global _db_pool
    if _db_pool is None:
        # Passing sslmode="require" here AND having it already embedded in DATABASE_URL
        # (as Neon/Supabase connection strings usually are, e.g. "...?sslmode=require")
        # makes psycopg2 raise `ProgrammingError: duplicate parameter "sslmode"` on every
        # single connection attempt — silently killing all DB reads/writes while DB_URL
        # still "looks" configured. Only add it if it isn't already in the URL.
        pool_kwargs = {"connect_timeout": 8}
        if "sslmode=" not in DATABASE_URL.lower():
            pool_kwargs["sslmode"] = "require"
        _db_pool = psycopg2.pool.ThreadedConnectionPool(1, 10, DATABASE_URL, **pool_kwargs)
...
def _db_get_raw_blocking(key: str):
    """The actual blocking network round-trip. Only called on a cache miss.
    Retries once with a fresh connection if the pooled one turned out to be dead (the DB
    server can silently drop idle pooled connections — 'SSL connection has been closed
    unexpectedly' — and the pool has no way to know until a query actually fails on it)."""
    last_err = None
    for _attempt in range(2):
        ...
        try:
            ...
            last_err = e
            _db_pool.putconn(conn._raw_conn, close=True)  # discard the dead connection, don't recycle it
    raise last_err


def _db_set_raw_blocking(key: str, value: str):
    """The actual blocking network round-trip. Runs on a background thread.
    Same stale-connection retry as _db_get_raw_blocking above."""
    last_err = None
    for _attempt in range(2):
        conn = get_db_connection()
        try:
            ...
            last_err = e
            _db_pool.putconn(conn._raw_conn, close=True)
    raise last_err


# --- Stability layer ---------------------------------------------------------
# Every load_*/save_* helper in this file (load_points, save_guild_config, etc. — 90+
# call sites all over the bot) ultimately calls db_get_raw()/db_set_raw() below. Almost
# all of those call sites are inside `async def` event handlers and commands with no
# `await`, which means every one of them used to block the whole event loop on a real
# Postgres round-trip. A slow/stalled query in ANY of them was enough to miss a gateway
# heartbeat, cause a reconnect, and take down in-flight work elsewhere (exactly the
# "I add one feature and unrelated old stuff breaks" pattern).
#
# Fix, without having to rewrite 90+ call sites by hand (risky on a file this size with
# no way to test-run it here): db_get_raw()/db_set_raw() keep their exact same sync
# signatures so every existing call site keeps working untouched, but underneath:
#   - reads are served from an in-memory cache after the first fetch of a given key,
#     so repeat reads (the overwhelming majority — same guild's config/points/etc.
#     read over and over) are instant, no network call at all.
#   - writes update the cache immediately (so the rest of the bot instantly sees the
#     new value) and hand the actual Postgres write off to a background thread, so the
#     calling event handler never blocks on it.
# Only a genuine first-ever read of a brand-new key still does a blocking round-trip —
# rare, and a single one, not stacked dozens of times per event like before.
_kv_cache = {}
_kv_cache_lock = threading.Lock()
_kv_write_queue = queue.Queue()
_kv_writer_started = False


def _kv_writer_loop():
    while True:
        key, value = _kv_write_queue.get()
        try:
            if value is None:
                _db_delete_raw_blocking(key)
            else:
                _db_set_raw_blocking(key, value)
        except Exception as e:
            print(f"⚠️ Background DB write failed for '{key}': {e}")
        finally:
            _kv_write_queue.task_done()


def _ensure_kv_writer():
    global _kv_writer_started
    if not _kv_writer_started:
        _kv_writer_started = True
        threading.Thread(target=_kv_writer_loop, daemon=True, name="kv-writer").start()


def _flush_kv_writes_before_exit(signum=None, frame=None):
    """Render (and most hosts) send SIGTERM to the old process during a redeploy/restart.
    Without this, any dashboard save whose background write hadn't landed yet (see the
    stability-layer note above) would be silently lost right when a deploy happens — which
    is exactly what 'settings reset after every deploy' looks like from the outside. This
    blocks shutdown for up to 10s to let the write queue actually finish, then exits either
    way (a bounded wait, not queue.join(), so a stuck DB connection can't hang shutdown)."""
    pending = _kv_write_queue.qsize()
    if pending:
        print(f"⏳ Flushing {pending} pending DB write(s) before shutdown...")
        deadline = time.monotonic() + 10
        while _kv_write_queue.unfinished_tasks and time.monotonic() < deadline:
            time.sleep(0.1)
        left = _kv_write_queue.unfinished_tasks
        print("✅ Pending DB writes flushed." if not left else f"⚠️ Gave up after 10s with {left} write(s) still pending.")
    if signum is not None:
        sys.exit(0)


atexit.register(_flush_kv_writes_before_exit)
signal.signal(signal.SIGTERM, _flush_kv_writes_before_exit)


def db_get_raw(key: str):
    """Returns the stored string for `key`, or None if missing/DB unavailable.
    Served from an in-memory cache after the first fetch — see stability-layer note above."""
    if not DATABASE_URL:
        return None
    with _kv_cache_lock:
        if key in _kv_cache:
    ...
    try:
        value = _db_get_raw_blocking(key)
    except Exception as e:
...
def _db_delete_raw_blocking(key: str):
    """The actual blocking network round-trip for a delete. Runs on the background writer
    thread, same stale-connection retry as the read/write helpers above."""
    last_err = None
    for _attempt in range(2):
        conn = get_db_connection()
        try:
...
def db_list_keys_with_prefix(prefix: str) -> list:
    """Blocking — only used by the once-a-day snapshot-pruning job, never on a hot path."""
    if not DATABASE_URL:
        return []
    try:
        conn = get_db_connection()
    except Exception as e:
        print(f"⚠️ DB connect failed while listing '{prefix}*': {e}")
        return []
    try:
        with conn.cursor() as cur:
        ...
        print(f"⚠️ DB list failed for '{prefix}*': {e}")
        return []
    finally:
        conn.close()
...
def db_set_raw(key: str, value: str) -> bool:
    """Stores `value` under `key`. Updates the cache immediately and queues the actual
    Postgres write on a background thread so callers never block on it — see
    stability-layer note above. Returns True as long as the write was queued
    (DATABASE_URL is set); background failures are logged, not raised."""
    if not DATABASE_URL:
        return False
    with _kv_cache_lock:
        _kv_cache[key] = value
    _ensure_kv_writer()
    _kv_write_queue.put((key, value))
    return True


def db_delete_raw(key: str) -> bool:
    """Deletes `key`. Same fire-and-forget background pattern as db_set_raw — a None value
    on the write queue is the writer loop's signal to delete rather than upsert."""
    if not DATABASE_URL:
        return False
    with _kv_cache_lock:
        _kv_cache.pop(key, None)
    _ensure_kv_writer()
    _kv_write_queue.put((key, None))
    return True
...
def default_guild_config() -> dict:
    """Fallback config for a guild that hasn't set anything on the dashboard yet. Channel/role
    fields below default to None (shows as "— none —" on the dashboard) rather than this bot's
    own home-server IDs — those IDs meant nothing on any other guild anyway. find_channel/
    find_role already fall back to matching by name (SUBMIT_CHANNEL_NAME etc.) when the id is
    None, so the home server keeps working as long as its channels/roles still have those names;
    every other server just starts clean until an admin picks channels on Settings."""
    return {
        ...
        "top_edits_role_id": None,
        "unranked_role_id": None,
        ...
        # Server Booster perk: winners who are currently boosting the server get a percentage
        # bonus on top of winner_points (matches the "+50% Edit Points" booster perk).
        "booster_bonus_enabled": True,
        "booster_bonus_percent": 50,
        # Extra, fully manual bonus rules configurable on the dashboard: a list of
        # {"type": "user"|"role", "target_id": "<id as string>", "percent": int}. Every rule
        # that matches the winner (their own user id, or any role they hold) stacks on top of
        # the automatic booster bonus above.
        "point_bonuses": [],
        ...
        "custom_rankup_messages": [],  # list of strings; if non-empty, replaces RANK_UP_MESSAGES entirely
        ...
        # Per-text "send as embed?" switches — every customizable text can be posted as a
        # fancy embed (default, matches original behavior) or as a plain text message instead.
        "winner_use_embed": True,
        "rankup_use_embed": True,
        "reminder_use_embed": True,
        # --- New systems: giveaways, mod-log, link-filter/partnership, channel access ---
        "banlog_channel_id": None,           # ban logs go here if set
        "kicklog_channel_id": None,          # kick logs go here if set
        "timeoutlog_channel_id": None,       # timeout/untimeout logs go here if set
        "msglog_channel_id": None,           # edited/deleted message logs go here if set
        "linkfilter_enabled": False,         # auto-delete links from anyone without a bypass role
        "linkfilter_bypass_role_ids": [],    # extra roles (besides Staff/Manager/Partnership) allowed to post links
        "partnership_role_ids": [],          # role(s) required to post links ("Partnership Recruiter")
        "partnership_category_id": None,     # category where partnership ticket channels get created
        "restricted_access": {},             # {"user:<id>"|"role:<id>": [channel_id, ...]} — locks a user/role to only those channels
        # --- /removesubs (free, self-hosted subtitle/text removal) ---
        "removesubs_enabled": True,
        "removesubs_default_method": "hybrid",
        "removesubs_default_quality": "480p",
        "removesubs_max_seconds": MAX_REMOVESUBS_SECONDS,
    }
...
def load_guild_config(guild_id: int) -> dict:
    ...
    if raw is None:
        # DATABASE_URL not set (or the DB read failed) — same local-file fallback every
        # other load_*/save_* pair in this file already has. Without this, settings saved
        # from the dashboard were silently discarded on the very next page load whenever
        # there was no DATABASE_URL configured.
    ...
    # Migration: a single combined "modlog_channel_id" used to cover ban/kick/timeout —
    # now each has its own channel. Old servers get all three pre-filled with it once.
    if "modlog_channel_id" in config:
        old_modlog = config.pop("modlog_channel_id")
        if old_modlog:
            for field in ("banlog_channel_id", "kicklog_channel_id", "timeoutlog_channel_id"):
                if not config.get(field):
                    config[field] = old_modlog
    # Migration: "partnership_role_id" (single) -> "partnership_role_ids" (list).
    if "partnership_role_id" in config:
        old_val = config.pop("partnership_role_id")
        if old_val and old_val not in config["partnership_role_ids"]:
            config["partnership_role_ids"] = list(config["partnership_role_ids"]) + [old_val]
    return config
...
def save_guild_config(guild_id: int, updates: dict) -> dict:
    ...
    if not db_set_raw(f"config:{guild_id}", payload):
        # Same fallback as load_guild_config above: db_set_raw returns False whenever
        # DATABASE_URL isn't set, in which case it writes nothing at all (unlike
        # save_points/save_giveaways/etc., which already fall back to a local file).
        # Without this, "Save changes" looked like it worked (the page reflects the new
        # values for that one request) but nothing was ever persisted.
...
# --- Embed builder: remembers what got sent so it can be edited again later ---

def load_sent_embeds(guild_id: int) -> dict:
    """Keyed by message id (as a string). Each entry holds everything needed to re-render and
    re-send an edit: which channel/thread it lives in, the embed fields, and the plain content."""
    raw = db_get_raw(f"sent_embeds:{guild_id}")
    if raw is not None:
        try:
            return json.loads(raw)
        except json.JSONDecodeError:
            return {}
    path = f"sent_embeds_{guild_id}.json"
    if not os.path.exists(path):
        return {}
    try:
        with open(path, "r") as f:
...
def save_sent_embeds(guild_id: int, data: dict):
    if db_set_raw(f"sent_embeds:{guild_id}", json.dumps(data)):
        return
    with open(f"sent_embeds_{guild_id}.json", "w") as f:
        json.dump(data, f)


def record_sent_embed(guild_id: int, message_id: int, target_id: int, is_forum: bool,
                       embed_data: dict, content: str, forum_title: str = None):
    """Called right after the embed builder successfully sends an embed, so it shows up in the
    'Sent embeds' list and can be reopened for editing later."""
    data = load_sent_embeds(guild_id)
    data[str(message_id)] = {
        "target_id": target_id,
        "is_forum": is_forum,
    ...
    save_sent_embeds(guild_id, data)


def forget_sent_embed(guild_id: int, message_id: int) -> bool:
    """Drops a sent embed from storage — used once the underlying Discord message is gone
    (deleted manually, or via the dashboard) so the list never shows stale entries that can
    no longer be edited."""
    data = load_sent_embeds(guild_id)
...
async def award_points(guild: discord.Guild, member_id: int, amount: int):
    old_total = load_points(guild.id).get(str(member_id), 0)
    new_total = add_points(guild.id, member_id, amount)
    await apply_point_roles(guild, member_id, new_total)
    if new_total > old_total:  # only a POSITIVE change can ever be a rank-UP (removepoints skips this)
        await maybe_announce_rank_up(guild, member_id, old_total, new_total)
    return new_total


async def maybe_announce_rank_up(guild: discord.Guild, member_id: int, old_total: int, new_total: int):
    """Fires whenever a member's points cross into a HIGHER tier than before (e.g. D -> C).
    Silently does nothing if the tier is unchanged, or if points went up but not enough to
    cross a threshold. Never fires for a rank going down (that path never reaches here — see
    award_points above, which only calls this when new_total > old_total)."""
    tiers = get_guild_rank_tiers(guild.id)
    if not tiers:
        return
    labels_in_order = [label for _threshold, _role_id, label, _emoji in tiers]
    old_label = current_rank_label(old_total, guild.id)
    new_label = current_rank_label(new_total, guild.id)
    if new_label is None or new_label == old_label:
        return
    if old_label is not None and new_label in labels_in_order and labels_in_order.index(new_label) <= labels_in_order.index(old_label):
        return  # defensive — shouldn't happen since new_total > old_total, but never announce a "downgrade"

    config = load_guild_config(guild.id)
    channel = guild.get_channel(config.get("rankup_channel_id")) or find_channel(guild, config.get("rankup_channel_id"), RANKUP_CHANNEL_NAME)
    if not channel:
        print(f"⚠️ Rank-up channel not found for {guild.name} (ID {config.get('rankup_channel_id')}). Check the ID on the Settings page.")
        return

    emoji = current_rank_emoji(new_total, guild.id)
    member_mention = f"<@{member_id}>"
    line = random.choice(config.get("custom_rankup_messages") or RANK_UP_MESSAGES).format(member=member_mention, rank=new_label)

    member = guild.get_member(member_id)

    try:
        if config.get("rankup_use_embed", True):
            tier_rgb = TIER_COLORS.get(new_label)
            embed_color = discord.Color.from_rgb(*tier_rgb) if tier_rgb else discord.Color.gold()

            embed = discord.Embed(
                title=f"{emoji} RANK UP — {new_label}",
                description=line,
                color=embed_color,
            )
            embed.add_field(name="New rank", value=f"{emoji} **{new_label}**", inline=True)
            embed.add_field(name="Total points", value=f"**{new_total}**", inline=True)
            if old_label:
                embed.add_field(name="Previous rank", value=old_label, inline=True)
            if member:
                embed.set_thumbnail(url=member.display_avatar.url)
            embed.set_footer(text="Keep submitting to climb even higher!")
            embed.timestamp = datetime.now(timezone.utc)
            await channel.send(content=member_mention, embed=embed)
        else:
            # Plain-text fallback — same information, no embed box.
            prev_line = f" (up from {old_label})" if old_label else ""
            plain = (
                f"{member_mention} {line}\n"
                f"{emoji} New rank: **{new_label}**{prev_line} — total points: **{new_total}**"
            )
            await channel.send(content=plain)
    except discord.Forbidden:
        print(f"⚠️ Missing permission to post the rank-up announcement in {channel} ({guild.name}).")
    except discord.HTTPException as e:
        print(f"⚠️ Failed to post rank-up announcement: {e}")
...
# Rank-up announcements — posted to the Rankings channel whenever someone's points cross
# into a new tier (e.g. D -> C). {member} = mention, {rank} = the new rank's label/name.
RANK_UP_MESSAGES = [
    "⬆️ {member} just ranked up to **{rank}**! The grind paid off.",
    "🎉 New tier unlocked — {member} is now **{rank}**!",
    "📈 {member} climbed the ladder and landed on **{rank}**.",
    "🔥 {member} leveled up to **{rank}**! Keep that momentum going.",
    "🚀 Lift-off — {member} just hit **{rank}** rank.",
    "✨ {member} earned their way into **{rank}**. Well deserved!",
    "🏅 {member} just promoted to **{rank}**!",
    "💪 Hard work shows — {member} is officially **{rank}** now.",
    "🌟 {member} broke into **{rank}** rank. Congrats!",
    "⚡ {member} surged up to **{rank}**!",
    "🎯 Target hit — {member} is now ranked **{rank}**.",
    "🧗 {member} clawed their way up to **{rank}**.",
    "🥳 Rank up! {member} just reached **{rank}**.",
    "📊 {member}'s points just tipped them into **{rank}** territory.",
    "🔓 {member} unlocked **{rank}** rank!",
    "🏆 {member} just secured the **{rank}** tier.",
    "💎 {member} polished their way into **{rank}**.",
    "🌱 {member} grew into **{rank}** rank — nice growth!",
    "🎖️ {member} earned the **{rank}** badge of honor.",
    "🏔️ {member} just summited into **{rank}** rank.",
    "⛰️ Another peak conquered — {member} is now **{rank}**.",
    "💥 {member} just blasted into **{rank}** rank!",
    "🧨 {member} detonated the competition on the way to **{rank}**.",
    "🕹️ Level up! {member} is now **{rank}**.",
    "🎮 {member} just cleared the next stage — welcome to **{rank}**.",
    "🛤️ {member} is riding the fast track, now at **{rank}**.",
    "🌈 {member} chased the rainbow and landed **{rank}** rank.",
    "🪄 A little magic, a lot of grinding — {member} is **{rank}** now.",
    "🧬 {member} evolved into **{rank}** rank.",
    "🦾 {member} powered up to **{rank}**.",
    "🛡️ {member} just earned the **{rank}** crest.",
    "🗝️ {member} unlocked the door to **{rank}** rank.",
    "🎇 Fireworks for {member} — freshly ranked **{rank}**!",
    "🎊 Confetti time — {member} just hit **{rank}**.",
    "📣 Announcement: {member} is now **{rank}** rank!",
    "🧭 {member} navigated their way to **{rank}**.",
    "🏗️ Built up point by point — {member} is now **{rank}**.",
    "🧱 Brick by brick, {member} built their way to **{rank}**.",
    "🏹 {member} took aim and landed **{rank}** rank.",
    "🎣 {member} reeled in a shiny new rank: **{rank}**.",
    "🧩 The final piece clicked — {member} is now **{rank}**.",
    "🛰️ {member} just entered **{rank}** orbit.",
    "🌠 {member} caught a shooting star and rose to **{rank}**.",
    "🪂 {member} dropped in as a fresh **{rank}**.",
    "🚂 All aboard — {member} just arrived at **{rank}** station.",
    "🏁 {member} crossed the finish line into **{rank}** rank.",
    "🥇 {member} just claimed the **{rank}** milestone.",
    "🧗‍♂️ {member} scaled the leaderboard up to **{rank}**.",
    "🎢 What a ride — {member} just reached **{rank}**.",
    "🏋️ Reps paid off — {member} is now **{rank}** rank.",
    "🧠 Smart, consistent submissions got {member} to **{rank}**.",
    "🔨 {member} hammered their way to **{rank}** rank.",
    "🧊 Cool and consistent — {member} just froze in a new **{rank}** rank.",
    "🌪️ {member} swept through the ranks, now sitting at **{rank}**.",
    "🎬 New scene, new rank — {member} just hit **{rank}**.",
    "📽️ Roll credits — {member} is officially **{rank}** now.",
    "🖋️ {member} signed their name into the **{rank}** tier.",
    "🧵 Thread by thread, {member} stitched together a **{rank}** rank.",
    "🪁 {member} caught the wind and flew up to **{rank}**.",
    "🐉 {member} just awakened as **{rank}** rank.",
    "🦅 {member} soared into **{rank}** rank.",
    "🐺 {member} clawed their way to **{rank}** — well earned.",
    "🦁 {member} roared into **{rank}** rank.",
    "🐯 {member} pounced straight into **{rank}**.",
    "🦂 {member} stung their way up to **{rank}** rank.",
    "🌋 {member} erupted into **{rank}** rank!",
    "🧨 Boom — {member} just hit **{rank}**.",
    "🎆 {member}'s progress just lit up the sky — welcome to **{rank}**.",
    "🛎️ Ding! {member} just checked into **{rank}** rank.",
    "📯 Sound the horn — {member} is now **{rank}**.",
    "🥁 Drumroll for {member} — freshly ranked **{rank}**!",
    "🎺 {member} just trumpeted their way to **{rank}**.",
    "🎻 Smooth climb for {member}, now resting at **{rank}**.",
    "🖼️ {member} just framed themselves a **{rank}** rank.",
    "🏺 {member} unearthed a brand new **{rank}** rank.",
    "🔮 It was written in the stars — {member} is now **{rank}**.",
    "🧿 {member} warded off the competition and reached **{rank}**.",
    "🪙 {member} cashed in enough points for **{rank}** rank.",
    "💰 {member}'s point stash just bought them **{rank}** rank.",
    "🧮 Numbers don't lie — {member} is officially **{rank}**.",
    "📐 Precision pays off — {member} just measured up to **{rank}**.",
    "🗺️ {member} charted a course straight to **{rank}** rank.",
    "🧗‍♀️ Another handhold, another rank — {member} is now **{rank}**.",
    "🏕️ {member} set up camp at the **{rank}** tier.",
    "🛶 {member} paddled their way into **{rank}** rank.",
    "⛵ Smooth sailing — {member} just docked at **{rank}**.",
    "🚁 {member} just touched down at **{rank}** rank.",
    "✈️ {member}'s progress just took off — welcome to **{rank}**.",
    "🛩️ {member} cruised into **{rank}** rank.",
    "🚦 Green light for {member} — now officially **{rank}**.",
    "🚥 {member} sailed through every checkpoint on the way to **{rank}**.",
    "🧗 Every submission was a foothold — {member} reached **{rank}**.",
    "🏛️ {member} just built themselves a place in **{rank}** rank.",
    "🕯️ Steady effort, bright result — {member} is now **{rank}**.",
    "🪔 {member}'s consistency lit the way to **{rank}** rank.",
    "🌻 {member} bloomed into **{rank}** rank.",
    "🍀 A little luck, a lot of skill — {member} is now **{rank}**.",
    "🐾 One step at a time, {member} tracked their way to **{rank}**.",
    "🧗‍♂️ {member} just topped out at **{rank}** rank — for now.",
    "🎆 Rank secured — {member} is officially **{rank}**. On to the next one!",
]

# Used as flavor text for /points and elsewhere — professional English lines.
POINTS_CHECK_FLAVOR = [
    "Bro checked their points and still didn't submit an edit. Couldn't be me.",
    "Sir/ma'am this is a leaderboard, not a participation trophy ceremony.",
    "You have points. Congrats, I guess. Go earn more, nerd.",
    "Peace was never an option, only points.",
    "Touch grass? Nah, submit an edit instead.",
    "This is your daily reminder that the leaderboard doesn't care about your feelings.",
    "You miss 100% of the top edits you don't submit.",
    "The rank fairy only visits people who actually post.",
    "Stat check complete. Verdict: mid, but improvable.",
    "Somewhere, a rival editor just got +1 point. Do something about it.",
    "Your points called. They said 'more please.'",
    "Ranking up is basically just showing up on purpose.",
    "This message was legally required to hype you up. You're welcome.",
    "No cap, your rank could be higher if you actually tried.",
    "Bro thinks checking /points counts as progress. Adorable.",
    "The grind doesn't stop just because you looked at a number.",
    "Certified point-checker, uncertified edit-submitter. Fix that.",
    "You vs. the guy who submits every single day. He's winning.",
    "This rank isn't gonna climb itself, chief.",
    "Bestie, the leaderboard is right there. Go do something about it.",
    "Point gang rise up (submit an edit first though).",
    "L take: checking points instead of posting edits.",
    "W take: you're at least paying attention.",
    "Rank up or shut up, those are the options.",
    "The algorithm respects consistency, not vibes.",
    "This is not a drill. Go submit an edit.",
    "One does not simply rank up by staring at the leaderboard.",
    "Your points are like a plant — they need daily watering (submissions).",
    "Skill issue? No. Submission issue? Also kind of.",
    "The next tier is closer than your last excuse, my guy.",
    "You could be ranking up right now instead of reading this joke.",
    "Legend says the top editors never stopped grinding. Be that legend.",
    "Peace, love, and consistent submissions.",
    "The leaderboard doesn't do participation trophies, only results.",
    "Big number energy incoming if you actually submit today.",
    "You're closer to rank-up than you are to finishing your last edit. Coincidence?",
    "Rome wasn't built in a day, but your rank could climb in a week.",
    "This is your sign to stop scrolling and start editing.",
    "Bro really checked points for the fifth time today instead of submitting.",
    "The clout is earned one submission at a time, not one refresh at a time.",
    "Somewhere a Top Edit slot is empty and waiting for you.",
    "Ranked systems fear consistency. Be feared.",
    "Point gap? What point gap. Go close it.",
    "You checked. Now go earn.",
    "The bot believes in you (mildly).",
    "Rank up speedrun: any%. Go.",
    "The leaderboard remembers everything. Act accordingly.",
    "Statistically, editors who submit more, win more. Groundbreaking research.",
    "You could either read another joke or go submit an edit. Choose wisely.",
    "This bot has seen your rank. It has opinions.",
    "Consistency arc loading...",
    "Point check complete. Motivation levels: questionable.",
    "The road to S-rank is paved with submitted edits, not vibes.",
    "Your rivals aren't sleeping on the leaderboard. Are you?",
    "Honestly kind of impressive you found this command. Now find /find-asset too.",
    "This message self-destructs the moment you actually submit something.",
    "Peace out, go touch some timelines (editing timelines, not grass).",
    "The grind respects no days off, but we respect naps. Nap first, then edit.",
    "Rank progress: loading... (you have to actually submit for it to move)",
    "You're basically one good edit away from bragging rights.",
    "Somewhere a competitor just climbed a tier. Rude of them, honestly.",
    "This is a friendly reminder disguised as a joke: go submit.",
    "The vibes are immaculate, the points are... a work in progress.",
    "Rank up talk is cheap. Submissions are currency.",
    "You + one edit today = measurable improvement. Do the math.",
    "Bro's out here farming jokes instead of points.",
    "The leaderboard has trust issues with people who only check, never submit.",
    "Certified point enjoyer. Uncertified point earner.",
    "This isn't a horoscope, but if it were: 'submit an edit today.'",
    "Not to be dramatic but your rank is literally waiting for you.",
    "The bot has faith in you. The bot also has low standards. Win-win.",
    "One submission a day keeps the Unranked role away.",
    "Rank climbing is a group project and you're currently AFK.",
    "The competition doesn't check points, they submit edits. Just saying.",
    "This message brought to you by the 'go actually do something' committee.",
    "Peace, points, and a little bit of chaos — the editor's trinity.",
    "Someone's about to overtake you. Might be you overtaking yourself, who knows.",
    "The vibes said 'W' but the leaderboard said 'prove it.'",
    "Your points are the physical manifestation of your effort. No pressure.",
    "The next rank is one solid edit away. GO.",
    "Peace treaties don't apply to the leaderboard. Everyone's at war for #1.",
    "Bro checked points and immediately closed the app. We see you.",
    "Consistency > talent. Also consistency > excuses.",
    "This command exists so you can flex or cope. Which one is it today?",
    "Somewhere, a top editor started exactly where you are now.",
    "You're not behind, you're just early to the next submission.",
    "The rank system doesn't lie, unlike your 'I'll submit tomorrow.'",
    "Point check: successful. Life check: still pending, go submit.",
    "The only thing standing between you and a rank-up is... you.",
    "This is a motivational speech disguised as a badly written joke.",
    "Somewhere in this server, someone is grinding harder than you right now.",
    "You could've submitted an edit in the time it took to read this.",
    "The bot respects the hustle, not the hesitation.",
    "Rank up plans: step 1, actually submit something.",
    "Your points are like XP — they don't grind themselves.",
    "This is your daily unsolicited pep talk. You're welcome.",
    "Peace was never an option, only the next edit deadline.",
    "The leaderboard doesn't care about your excuses, only your submissions.",
    "One more edit and you might just leapfrog someone. Petty motivation, but effective.",
    "This joke was 90% filler, 10% 'go submit an edit.' Guess which part matters.",
    "You checked your points. Bold strategy. Now execute the follow-up: submit.",
    "The grind called, it left a voicemail: 'where's my submission?'",
    "Rank up or become a cautionary tale. Those are the options.",
    "Somewhere a picker is reviewing edits right now. Yours could be one of them.",
    "The leaderboard has no chill, and neither should your work ethic.",
    "This is your sign. The sign says 'submit an edit.' Not subtle, I know.",
    "Rank-up dreams don't pay bills, submissions do (in points, not actual money, sorry).",
    "The leaderboard is basically a group chat where actions talk louder than checking stats.",
    "This is peak self-care: check points, feel something, go submit an edit anyway.",
]
...
def _migrate_legacy_data_once_sync(guild_ids: list):
    """Blocking body of the migration — run via asyncio.to_thread so it never stalls
    the event loop (and, in turn, never blocks the slash-command sync in on_ready)."""
    if db_get_raw("migrated_v2"):
        return
    for guild_id in guild_ids:
        for base in ("points", "day", "video_history", "last_collect", "badwords"):
            legacy = db_get_raw(base)
            if legacy is not None and db_get_raw(f"{base}:{guild_id}") is None:
                db_set_raw(f"{base}:{guild_id}", legacy)
...
async def migrate_legacy_data_once():
    ...
    await asyncio.to_thread(_migrate_legacy_data_once_sync, [g.id for g in bot.guilds])
...
async def on_error(event_method, *args, **kwargs):
    """Without this, discord.py's default on_error just prints to stderr and moves on — fine,
    except on a host like Render that's easy to miss, and a couple of event handlers in this
    file don't have their own try/except. This makes sure every crash in an event handler is
    at least loudly logged with a full traceback and NEVER takes the gateway connection down."""
    print(f"⚠️ Unhandled error in event '{event_method}':")
    traceback.print_exc()
...
async def on_ready():
    ...
    try:
        ...
        print(f"⚠️ Legacy data migration failed (continuing anyway): {e}")

    # Re-attach persistent views so buttons keep working after a restart/redeploy.
    try:
        bot.add_view(PartnershipTicketView())
        bot.add_view(TicketCloseView())
    except Exception as e:
        print(f"⚠️ Failed to re-attach base persistent views (continuing anyway): {e}")
    for guild in bot.guilds:
        try:
            gw_data = load_giveaways(guild.id)
        except Exception:
            continue
        for key, gw in gw_data.items():
            try:
                if gw.get("cancelled"):
                    continue
                if gw.get("ended"):
                    bot.add_view(GiveawayEndedView(f"{guild.id}:{key}"))
                else:
                    bot.add_view(GiveawayView(f"{guild.id}:{key}"))
            except Exception as e:
                print(f"⚠️ Failed to re-attach giveaway view for {key}: {e}")
    try:
        if not giveaway_check_loop.is_running():
            giveaway_check_loop.start()
    except Exception as e:
        print(f"⚠️ Failed to start giveaway_check_loop (continuing anyway): {e}")

    # Slash-command sync — deliberately placed after try/except blocks above so that a
    # failure anywhere earlier in on_ready can NEVER prevent this from running.
    #
    # Only a per-guild sync is done here (no separate global bot.tree.sync() call).
    # Doing BOTH a global sync and a per-guild copy_global_to+sync used to register every
    # command twice — once as a global command, once as a guild command with the same
    # name — which Discord shows as two separate entries in the /-menu instead of merging
    # them. Per-guild sync alone covers every server the bot is already in (this loop) and
    # on_guild_join already handles brand-new servers the same way, so nothing is missed.
    for guild in bot.guilds:
        try:
            bot.tree.copy_global_to(guild=guild)
            guild_synced = await asyncio.wait_for(bot.tree.sync(guild=guild), timeout=30)
            print(f"Synced {len(guild_synced)} slash command(s) instantly for {guild.name}")
        except asyncio.TimeoutError:
            print(f"⚠️ Guild sync for {guild.name} TIMED OUT after 30s")
        except Exception as e:
            print(f"⚠️ Instant guild sync failed for {guild.name}: {e}")

    # One-off cleanup: earlier versions of this bot also ran a global bot.tree.sync(),
    # which registered every command a second time as a global command alongside the
    # per-guild copy above — that's what caused duplicate entries in the /-menu. Clearing
    # the global command list here (AFTER copy_global_to already ran above) removes those
    # leftover global registrations from Discord without touching the per-guild ones.
    # Safe to run on every startup — a no-op once the global list is already empty.
    try:
        bot.tree.clear_commands(guild=None)
        await asyncio.wait_for(bot.tree.sync(), timeout=30)
    except Exception as e:
        print(f"⚠️ Failed to clear leftover global commands: {e}")
...
@bot.event
async def _disqualify_deleted_submission(guild_id: int, message_id: int):
    """If a deleted message was one of today's submitted edits and voting is still open,
    pulls it from the running vote — its poll answer can't be removed (Discord's native
    Poll object doesn't support that once posted), so instead it's marked disqualified and
    skipped when picking a winner, even if people already voted for it."""
    poll_state = get_poll_state(guild_id)
    if not poll_state.get("polls"):
        return  # no vote currently running today
    for idx, entry in enumerate(poll_state["entries"]):
        if entry.get("message_id") == message_id:
            poll_state.setdefault("disqualified", set()).add(idx)
            guild = bot.get_guild(guild_id)
            config = load_guild_config(guild_id) if guild else {}
            discussion_channel = find_channel(guild, config.get("discussion_channel_id"), DISCUSSION_CHANNEL_NAME) if guild else None
            if discussion_channel:
                try:
                    await discussion_channel.send(
                        f"🗑️ **{entry['author_name']}**'s edit was deleted by them — it's out of today's voting, "
                        f"even if it already has votes."
                    )
                except discord.HTTPException:
                    pass
            print(f"Submission from {entry['author_name']} disqualified in guild {guild_id} (submission message deleted).")
            break


@bot.event
async def on_raw_message_delete(payload: discord.RawMessageDeleteEvent):
    """If a message that the embed builder sent gets deleted (by staff, by hand, however),
    drop it from the 'sent embeds' store too — otherwise the dashboard would keep offering an
    'Edit' button for something that no longer exists."""
    if payload.guild_id is None:
        return
    forget_sent_embed(payload.guild_id, payload.message_id)
    await _disqualify_deleted_submission(payload.guild_id, payload.message_id)
    await _log_deleted_message(payload.guild_id, payload.channel_id, payload.cached_message, message_id=payload.message_id)
    db_delete_raw(f"msgsnap:{payload.message_id}")  # used (or not needed) — no reason to keep it


@bot.event
async def on_raw_bulk_message_delete(payload: discord.RawBulkMessageDeleteEvent):
    """Same cleanup as on_raw_message_delete, but for bulk/purge-style deletes which fire this
    event instead of one on_raw_message_delete per message."""
    if payload.guild_id is None:
        return
    for message_id in payload.message_ids:
        forget_sent_embed(payload.guild_id, message_id)
        await _disqualify_deleted_submission(payload.guild_id, message_id)
    config = load_guild_config(payload.guild_id)
    log_channel_id = config.get("msglog_channel_id")
    ...
    if log_channel:
        try:
            await log_channel.send(embed=discord.Embed(
                ...
                description=f"**{len(payload.message_ids)}** messages were deleted in <#{payload.channel_id}> (e.g. via purge).",
...
async def on_message_edit(before: discord.Message, after: discord.Message):
    if not before.guild or before.author.bot or before.content == after.content:
        return
    config = load_guild_config(before.guild.id)
    log_channel_id = config.get("msglog_channel_id")
    if not log_channel_id:
        return
    ...
    if not log_channel:
        return
    old_content = before.content or "*(no text)*"
    new_content = after.content or "*(no text)*"
    if len(old_content) > 1000:
        old_content = old_content[:1000] + "…"
    if len(new_content) > 1000:
        new_content = new_content[:1000] + "…"
    embed = discord.Embed(title="✏️ Message Edited", color=discord.Color.from_rgb(234, 179, 8), timestamp=datetime.now(timezone.utc))
    ...
    links_before = _extract_link_field(before.content)
    links_after = _extract_link_field(after.content)
    if links_before or links_after:
        if links_before == links_after:
            embed.add_field(name="🔗 Detected Links", value=(links_before or links_after)[:1024], inline=False)
        else:
            if links_before:
                embed.add_field(name="🔗 Links (before)", value=links_before[:1024], inline=False)
            if links_after:
                embed.add_field(name="🔗 Links (after)", value=links_after[:1024], inline=False)
...
async def _strip_roles(guild: discord.Guild, user: discord.Member, reason: str):
    try:
...
async def _timeout_member(guild: discord.Guild, user: discord.Member, reason: str):
    try:
...
async def punish_raider(guild: discord.Guild, user, reason: str):
    ...
    both at the same time rather than one after the other, since they're independent API calls.
    ...
    await asyncio.gather(
        _strip_roles(guild, user, reason),
        _timeout_member(guild, user, reason),
        return_exceptions=True,
    )


async def _ban_bot(guild: discord.Guild, bot_member: discord.Member, reason: str):
    try:
...
async def _find_and_punish_adder(guild: discord.Guild, bot_member: discord.Member, reason: str):
    adder = None
    try:
...
async def handle_nuke_bot_detected(guild: discord.Guild, bot_member: discord.Member, reason: str):
    ...
    spam). Timeout, ban, and looking up + punishing whoever added it all happen AT THE SAME
    TIME — these are three independent Discord API operations, so there's no reason to make
    the bot wait for one to finish before starting the next."""
    print(f"🚨 Anti-raid: nuke pattern detected from {bot_member} in {guild.name} — {reason}")
    await asyncio.gather(
        _timeout_member(guild, bot_member, reason),
        _ban_bot(guild, bot_member, reason),
        _find_and_punish_adder(guild, bot_member, reason),
        return_exceptions=True,
    )
...
async def on_guild_channel_delete(channel):
    # Anti-raid: a single channel deletion — by a human OR a bot — is left completely alone.
    # Only once a BURST of deletions from the same bot confirms an actual raid does anything
    # get AUTO-restored; a mod deleting one channel on purpose should never see it pop back by
    # itself. It's still tracked below either way though, so staff can bring it back on purpose
    # with /restorechannel whenever they want — that's a deliberate, on-demand action, not the
    # automatic anti-raid system kicking in.
    # EXCEPT for channels we deleted ourselves as raid cleanup (see on_guild_channel_create)
    # — recreating (or tracking) a junk channel we just removed on purpose would defeat the point.
    ...
    try:
        ...
        pass  # no audit-log access — still track the deletion below, just without a "deleted by"
    ...
    # Track EVERY deletion (human or bot) for manual /restorechannel — independent of the
    # nuke-burst detection below, which only ever auto-fires for confirmed bot raids.
    _record_general_channel_delete(guild.id, snapshot, deleter.mention if deleter else "someone (no audit-log access)")

    if not deleter or not deleter.bot or not bot.user or deleter.id == bot.user.id:
        return  # not a bot deletion — never AUTO-restored, no matter how many happen

    events = _record_channel_delete_event(guild.id, deleter.id, snapshot, NUKE_ACTION_WINDOW_SECONDS)
    if len(events) < NUKE_ACTION_THRESHOLD:
        return  # not enough of a burst yet to call it a raid — leave it deleted, do nothing

    to_restore = [e["snapshot"] for e in events if not e["handled"]]
    for e in events:
        e["handled"] = True

    deleter_member = guild.get_member(deleter.id)
    tasks = [asyncio.create_task(_recreate_channel_from_snapshot(guild, snap)) for snap in to_restore]
    if deleter_member:
        tasks.append(asyncio.create_task(handle_nuke_bot_detected(guild, deleter_member, reason="mass channel deletion (nuke pattern)")))
    if tasks:
        await asyncio.gather(*tasks, return_exceptions=True)
...
async def on_guild_join(guild: discord.Guild):
    ...
    try:
        ...
        print(f"Synced {len(synced)} slash command(s) instantly for new server {guild.name}")
    except Exception as e:
        print(f"⚠️ Instant slash command sync failed for new server {guild.name}: {e}")
...
async def on_message(message: discord.Message):
    ...
    # --- Message-log snapshot: remember content/attachments so a deleted message can still be
    # shown later even after discord.py's own cache has evicted it (see save_message_snapshot).
    # Only for guilds that actually have the message-log configured, to avoid needless writes
    # everywhere else.
    if message.guild and not message.author.bot:
        _snapshot_config = load_guild_config(message.guild.id)
        if _snapshot_config.get("msglog_channel_id"):
            save_message_snapshot(message)

    # --- Channel-access lock: certain users/roles may ONLY send in specific channels ---
    if message.guild and not message.author.bot:
        config = load_guild_config(message.guild.id)
        restricted = config.get("restricted_access", {})
        if restricted and not message.author.guild_permissions.administrator:
            allowed_channels = set()
            matched = False
            ukey = f"user:{message.author.id}"
            if ukey in restricted:
                matched = True
                allowed_channels.update(restricted[ukey])
            member_role_ids = {r.id for r in message.author.roles}
            for rkey, chans in restricted.items():
                if rkey.startswith("role:") and int(rkey.split(":", 1)[1]) in member_role_ids:
                    matched = True
                    allowed_channels.update(chans)
            if matched and message.channel.id not in allowed_channels:
                try:
                    await message.delete()
                except (discord.Forbidden, discord.NotFound):
                    pass
                try:
                    allowed_label = ", ".join(f"<#{c}>" for c in allowed_channels) or "no channels"
                    await message.channel.send(
                        f"🔒 {message.author.mention}, you're only allowed to write in {allowed_label}.",
                        delete_after=8,
                    )
                except discord.HTTPException:
                    pass
    ...
    # --- Link filter / Partnership promo protection: deletes links from anyone without a
    # bypass role and points them to opening a Partnership ticket instead. ---
    if message.guild and not message.author.bot and message.content:
        config = load_guild_config(message.guild.id)
        tickets = load_tickets(message.guild.id)
        is_ticket_channel = str(message.channel.id) in tickets
        if not is_ticket_channel and _guild_link_filter_check(message, config):
            try:
                await message.delete()
            except (discord.Forbidden, discord.NotFound):
                pass
            try:
                partnership_roles = config.get("partnership_role_ids", [])
                if partnership_roles:
                    role_mentions = " or ".join(f"<@&{rid}>" for rid in partnership_roles)
                    desc = (
                        f"{message.author.mention}, links/promo are only allowed here with the "
                        f"{role_mentions} role.\n\n"
                        f"Want to promote your project? Open a Partnership ticket below!"
                    )
                else:
                    desc = f"{message.author.mention}, links/promo aren't allowed here."
                link_embed = discord.Embed(title="🔗 Link Removed", description=desc, color=PARTNERSHIP_COLOR)
                link_embed.set_footer(text="Zexo • Partnership Protection")
                view = PartnershipTicketView() if config.get("partnership_category_id") else None
                await message.channel.send(embed=link_embed, view=view, delete_after=30)
            except discord.HTTPException:
                pass
            return
    ...
    if message.guild and not message.author.bot:
        ...
        if submit_channel and picker_role and message.channel.id == submit_channel.id:
            ...
            # Ping reminder — pinging is required for the edit to count at all. Fires once per
            # user (not every time) so it doesn't spam someone who posts often, and only for an
            # actual VIDEO (sources is already video-only now, gifs/photos never reach here).
            ...
            # Two short, eye-catchy embeds sent one after another — ping reminder first (it's
            # the more urgent one: without the ping the edit doesn't count at all), then the
            # AI-rating hint — instead of one combined message.
            if show_ping_reminder:
                try:
                    if config.get("reminder_use_embed", True):
                        ping_embed = discord.Embed(
                            description=f"📌 **Don't forget to ping!**\n{reminder_text}",
                            color=discord.Color.from_rgb(245, 158, 11),
                        )
                        ping_embed.set_footer(text="Zexo • Top Edit")
                        await message.reply(embed=ping_embed, mention_author=False)
                    else:
                        await message.reply(content=f"📌 **Don't forget to ping!**\n{reminder_text}", mention_author=False)
                except discord.HTTPException:
                    pass
                ...
                try:
                    if config.get("reminder_use_embed", True):
                        hint_embed = discord.Embed(
                            description="✨ **Want feedback?** Run `.airating` for a score, software guess, and tips.",
                            color=discord.Color.from_rgb(34, 255, 176),
                        )
                        hint_embed.set_footer(text="Zexo • Top Edit")
                        await message.reply(embed=hint_embed, mention_author=False)
                    else:
                        await message.reply(content="✨ **Want feedback?** Run `.airating` for a score, software guess, and tips.", mention_author=False)
                except discord.HTTPException:
                    pass
...
async def scheduler_loop():
    ...
    global _last_snapshot_prune_date
    if now.hour == 3 and now.minute == 0 and _last_snapshot_prune_date != now.date():
        _last_snapshot_prune_date = now.date()
        try:
            await _prune_old_message_snapshots()
        except Exception:
            traceback.print_exc()
            print("⚠️ Message-snapshot pruning failed — will retry tomorrow at the same time.")

    for guild in bot.guilds:
        try:
            # Same fix as giveaway_check_loop: don't block the event loop with a
            # synchronous DB read every tick.
            config = await asyncio.to_thread(load_guild_config, guild.id)
            gid = guild.id
            ...
            # A crash for ONE server (bad video link, missing permission, Discord hiccup,
            # whatever) must never take down the scheduler for every other server, and must
            # never silently kill the loop for future days either — both used to happen here,
            # since tasks.loop stops for good on an uncaught exception. Log it and move on.
            traceback.print_exc()
            print(f"⚠️ Scheduler tick failed for {guild.name} ({guild.id}) — see traceback above. "
                  f"That server's job for today was skipped; it'll try again at the same time tomorrow.")


@scheduler_loop.error
async def scheduler_loop_error(error):
    # Safety net in case something throws OUTSIDE the per-guild try above (e.g. while listing
    # bot.guilds itself). Without this, discord.ext.tasks would let the whole loop die silently
    # and NOTHING would ever run again until the bot is manually restarted.
    traceback.print_exc()
    print(f"🚨 scheduler_loop crashed outside the per-guild guard: {error!r} — restarting it now.")
    if not scheduler_loop.is_running():
        scheduler_loop.restart()
...
_NON_VIDEO_EXTENSIONS = (".gif", ".png", ".jpg", ".jpeg", ".webp", ".bmp", ".heic", ".tiff", ".svg")


def _is_video_attachment(attachment) -> bool:
    """True only for an attachment that's actually a video — a gif or a static image must
    never count as a Top Edit submission, even though it's technically an 'attachment' too.
    Works on both a real discord.Attachment and a raw API attachment dict (has .get)."""
    ...
    if content_type.startswith("video"):
        return True
    if filename.endswith(_NON_VIDEO_EXTENSIONS):
        return False
    return True  # unknown type with no obviously-image extension — don't block it


def _is_video_url(url: str) -> bool:
    """A bare link only counts as a Top Edit submission if it doesn't obviously point at a
    still image/gif file. Not trying to be exhaustive about what IS a video — is_social_video_
    link already covers that for known platforms — this just filters out the clear-cut image
    case (a plain .gif/.png/.jpg link) that was getting miscounted as an edit before."""
    return not url.lower().split("?")[0].endswith(_NON_VIDEO_EXTENSIONS)
...
# ============================================================
# /removesubs — AI removal of burned-in subtitles/text (local text_remover + ffmpeg audio remux)
# ============================================================

def _get_video_duration_seconds(video_path: str) -> float:
    """Best-effort video duration via ffprobe. Returns 0 if it can't be determined."""
    try:
...
def _run_text_removal_sync(video_path: str, method: str, resolution: str = "480p") -> str | None:
    """Blocking call: runs the FREE, self-hosted YOLO+inpainting text remover
    (text_remover.py) on the local video file and returns a local output path, or None
    on any failure. MUST run in an executor — this is real CPU-bound work and can take
    anywhere from under a minute to several minutes depending on video length/resolution."""
    try:
        return text_remover.remove_text_from_video(video_path, method=method, resolution=resolution)
    except Exception as e:
        print(f"⚠️ Local text removal failed: {e}")
        return None


def _remux_original_audio_sync(processed_video_path: str, original_video_path: str) -> str:
    """The text remover strips audio entirely, so this re-attaches the ORIGINAL video's
    audio track onto the processed (subtitle-free) video via ffmpeg, without re-encoding
    the video stream. Falls back to returning the silent processed video if the original
    has no audio track or the mux fails for any reason."""
    out_path = f"/tmp/nosubs_final_{random.randint(0, 10**9)}.mp4"
    try:
        result = subprocess.run(
            ["ffmpeg", "-y", "-i", processed_video_path, "-i", original_video_path,
        ...
        print(f"⚠️ Re-muxing original audio failed: {e}")
    if os.path.exists(out_path):
        os.remove(out_path)
...
def _extract_frames(video_path: str, count: int = 4) -> list[str]:
    ...
    for ts in timestamps:
        ...
        try:
            subprocess.run(
                # Downscaled to max 720px wide — at 60 frames a submission, full source
                # resolution would balloon the upload to tens of MB and burn way more tokens
                # per rating for no real accuracy gain (grading/composition still reads fine
                # at 720p). scale=-2 keeps the aspect ratio and forces an even height.
                ["ffmpeg", "-y", "-ss", str(ts), "-i", video_path, "-frames:v", "1",
                 "-vf", "scale='min(720,iw)':-2", "-q:v", "3", out_path],
...
def _call_gemini_vision(frame_paths: list[str], prompt: str) -> str:
    ...
    def _do_request(model_name):
        return http_requests.post(
            ...
            # 60 frames per rating now instead of 8 — a bigger upload, give it more room
            # before timing out instead of failing a slow-but-fine request.
            timeout=120,
        )
...
async def _raw_message_snapshots(msg: discord.Message):
    ...
    try:
        raw = await asyncio.wait_for(msg._state.http.get_message(msg.channel.id, msg.id), timeout=8)
    except Exception:
        # Covers both real errors (403/404/network) AND a request that simply never comes
        # back — without the explicit timeout above, a single stuck request here could stall
        # the entire /previewvotes scan indefinitely with no error ever surfacing.
        return []
...
async def extract_video_sources(msg: discord.Message):
    ...
    for word in msg.content.split():
        if (word.startswith("http://") or word.startswith("https://")) and _is_video_url(word):
    ...
    for snapshot in (getattr(msg, "message_snapshots", None) or []):
        ...
        for embed in getattr(snapshot, "embeds", []) or []:
            if getattr(embed, "url", None) and _is_video_url(embed.url):
    ...
    if not getattr(msg, "message_snapshots", None) and getattr(msg, "reference", None) is not None:
        for raw_snap in await _raw_message_snapshots(msg):
            ...
            for attachment in snap_msg.get("attachments", []) or []:
                if attachment.get("url") and _is_video_attachment(attachment):
                    sources.append((attachment["url"], msg.author))
            for word in (snap_msg.get("content") or "").split():
                if (word.startswith("http://") or word.startswith("https://")) and _is_video_url(word):
                    sources.append((word, msg.author))
            for embed in snap_msg.get("embeds", []) or []:
                if embed.get("url") and _is_video_url(embed["url"]):
                    sources.append((embed["url"], msg.author))
    ...
    if reference is not None:
        ...
        if replied is None and reference.message_id and reference.channel_id:
            try:
                ref_channel = msg.guild.get_channel(reference.channel_id) or await asyncio.wait_for(msg.guild.fetch_channel(reference.channel_id), timeout=8)
                replied = await asyncio.wait_for(ref_channel.fetch_message(reference.message_id), timeout=8)
        ...
        if isinstance(replied, discord.Message):
            ...
            for snapshot in (getattr(replied, "message_snapshots", None) or []):
                # Same gap as above: the replied-to message can itself be a Forward.
                for attachment in getattr(snapshot, "attachments", []) or []:
                    if _is_video_attachment(attachment):
                        sources.append((attachment.url, replied.author))
                for word in (getattr(snapshot, "content", "") or "").split():
                    if (word.startswith("http://") or word.startswith("https://")) and _is_video_url(word):
                        sources.append((word, replied.author))
    ...
    if not unique_sources and (msg.reference is None):
        # Common real-world pattern: post the video as its own message, then ping the Picker
        # role in a SEPARATE follow-up message (not a reply, nothing attached to the ping
        # itself). Neither case above catches this since there's no reference to resolve.
        # Look a few messages back for the nearest one from the SAME author that actually
        # has something — bounded to a short window so this can't turn into a slow scan.
        try:
            async for prior in msg.channel.history(before=msg, limit=5):
                if prior.author.id != msg.author.id:
                    continue
                prior_sources = [a.url for a in prior.attachments if _is_video_attachment(a)]
                prior_sources += [w for w in prior.content.split() if (w.startswith("http://") or w.startswith("https://")) and _is_video_url(w)]
                for snapshot in (getattr(prior, "message_snapshots", None) or []):
                    prior_sources += [a.url for a in (getattr(snapshot, "attachments", []) or []) if _is_video_attachment(a)]
                    prior_sources += [w for w in (getattr(snapshot, "content", "") or "").split() if (w.startswith("http://") or w.startswith("https://")) and _is_video_url(w)]
                if prior_sources:
                    unique_sources = [(url, prior.author) for url in prior_sources]
                    break
        except discord.HTTPException:
            pass
...
def current_collect_window_start(config: dict) -> datetime:
    """The start of the window that the NEXT collect run will cover: the most recent
    collect_hour/collect_minute that has already passed (today if it's passed today,
    otherwise yesterday's), in Europe/Berlin — same clock the dashboard countdown uses.
    A flat 24h rolling window: anything posted after today's collect_hour is meant for
    TOMORROW's collect_hour poll, not today's — confirmed directly by the server owner.

    Used by /previewvotes so the preview always reflects the live dashboard setting rather
    than whenever the collect job last actually ran (which can lag behind if the bot was
    offline) — changing the time on the dashboard changes this immediately, with no save/
    redeploy needed."""
    now = datetime.now(EU_TZ)
    today_collect = now.replace(hour=config["collect_hour"], minute=config.get("collect_minute", 0), second=0, microsecond=0)
    window_start = today_collect if now >= today_collect else today_collect - timedelta(days=1)
    return window_start.astimezone(timezone.utc)


async def extract_simple_sources(msg: discord.Message):
    """Fast local-only edit fingerprint for a message: attachment URLs and http(s) links in
    its own content, or — for a reply that itself carries no link (someone replying to add the
    Picker ping they forgot the first time) — the ORIGINAL message's link/attachment instead,
    credited to ITS author rather than whoever replied.

    Also handles the non-reply version of the same pattern: video posted as its own message,
    Picker ping sent as a separate follow-up message right after (nothing attached to the ping
    itself, no reply link between them). In that case only, this looks a few messages back for
    the nearest one from the SAME author that has something — one bounded history() call
    (limit=5), not a per-message fetch, so it stays fast even though it's not purely local.

    Returns (sources, credited_author). A message with no attachment/link anywhere falls back
    to its own message ID as the fingerprint, so it still counts as one entry rather than
    being silently dropped."""
    sources = [a.url for a in msg.attachments if _is_video_attachment(a)]
    sources += [w for w in msg.content.split() if (w.startswith("http://") or w.startswith("https://")) and _is_video_url(w)]
    credited_author = msg.author

    if not sources:
        replied = getattr(msg.reference, "resolved", None) if msg.reference else None
        if replied is None and msg.reference and msg.reference.message_id:
            # Discord doesn't always hand us the replied-to message for free (seems to depend
            # on how the message was fetched — via history() it's often missing even though
            # it's there on a live gateway event). One bounded, guarded fetch — NOT a full
            # reply-chasing chain like extract_video_sources used to do — so a reply whose
            # target Discord didn't preload still gets credited to the right person instead
            # of silently falling back to whoever replied.
            try:
                ref_channel = msg.guild.get_channel(msg.reference.channel_id) or msg.channel
                replied = await asyncio.wait_for(ref_channel.fetch_message(msg.reference.message_id), timeout=5)
            except (discord.NotFound, discord.Forbidden, discord.HTTPException, asyncio.TimeoutError):
                replied = None
        if isinstance(replied, discord.Message):
            sources = [a.url for a in replied.attachments if _is_video_attachment(a)]
            sources += [w for w in (replied.content or "").split() if (w.startswith("http://") or w.startswith("https://")) and _is_video_url(w)]
            if not sources:
                # The replied-to message can itself be a Forward — its real content lives in
                # message_snapshots, not .content/.attachments, which is exactly what was still
                # missed here: replying to someone's forwarded video fell through to nothing.
                for snapshot in (getattr(replied, "message_snapshots", None) or []):
                    sources += [a.url for a in (getattr(snapshot, "attachments", []) or []) if _is_video_attachment(a)]
                    sources += [w for w in (getattr(snapshot, "content", "") or "").split() if (w.startswith("http://") or w.startswith("https://")) and _is_video_url(w)]
            if not sources:
                # discord.py doesn't always model message_snapshots even when Discord's raw API
                # response has them (same gap documented on message_pings_role_async above) — when
                # that happens here, `sources` stays empty and credited_author silently falls back
                # to whoever replied, NOT the original poster. That's the exact bug where the
                # person who just added the missing ping got credited for someone else's edit.
                # Going straight to the raw HTTP response sidesteps discord.py's modeling gap.
                for raw_snap in await _raw_message_snapshots(replied):
                    snap_msg = raw_snap.get("message", {}) or {}
                    sources += [a.get("url") for a in (snap_msg.get("attachments") or []) if a.get("url") and _is_video_attachment(a)]
                    sources += [w for w in (snap_msg.get("content") or "").split() if (w.startswith("http://") or w.startswith("https://")) and _is_video_url(w)]
            if sources:
                credited_author = replied.author

    if not sources and msg.reference is None:
        try:
            async for prior in msg.channel.history(before=msg, limit=5):
                if prior.author.id != msg.author.id:
                    continue
                ...
                if prior_sources:
                    ...
                    credited_author = prior.author
                    break
        except discord.HTTPException:
            pass

    # No fallback to a fake "msg:<id>" fingerprint anymore — a ping with no actual video/link/
    # attachment/forward attached must NOT be registered at all, matching what the real collect
    # job (extract_video_sources) already does. Returns an empty list in that case.
    return sources, credited_author


async def collect_pending_entries(guild: discord.Guild):
    """Live look at what the next collect run would pick up: who has pinged the Picker role
    in the submission channel since the last collect, and when — without posting or marking
    anything as used. Returns None if the channel/role can't be found, else a list.

    Deliberately does NOT resolve video links (extract_video_sources): that requires an extra
    Discord API round-trip per reply/forward message, which is what made this take minutes on
    a channel with a lot of those. The real collect job (run_collect_job_for_guild) still does
    the full resolution — this is preview-only, so a name and a timestamp is all it needs."""
    print(f"🔎 collect_pending_entries: starting for {guild.name} ({guild.id})")
    config = load_guild_config(guild.id)
    ...
    if not submit_channel or not picker_role:
        print(f"🔎 collect_pending_entries: submit_channel={submit_channel} picker_role={picker_role} — bailing out")
        return None

    perms = submit_channel.permissions_for(guild.me)
    print(f"🔎 collect_pending_entries: scanning #{submit_channel.name} — read_messages={perms.read_messages} read_history={perms.read_message_history}")
    if not perms.read_message_history or not perms.read_messages:
        print(f"⚠️ collect_pending_entries: missing permissions in #{submit_channel.name} — the bot needs View Channel + Read Message History there.")
        return []

    # Only count what falls in the CURRENT window (dashboard's collect_hour), not "since
    # whenever the job last actually ran" — those can differ if the bot had downtime, and the
    # person changing the time on the dashboard should see the preview react immediately.
    since = current_collect_window_start(config)
    video_history = load_video_history(guild.id)
    entries = []
    seen_edits = set()  # same edit (link/attachment) pinged more than once -> only counts once;
                         # same person with a DIFFERENT edit still gets a separate entry.
    ...
    try:
        async for msg in submit_channel.history(after=since, limit=None):
            scanned += 1
            if scanned % 100 == 0:
                print(f"🔎 collect_pending_entries: still scanning #{submit_channel.name} — {scanned} message(s) so far")
            try:
                if msg.author.bot:
                    continue
                if not message_pings_role(msg, picker_role):
                    continue
                sources, credited_author = await extract_simple_sources(msg)
                if credited_author.bot:
                    continue
                if not sources:
                    # Plain ping with nothing attached — no video, link, attachment, or forward
                    # found anywhere on this message or what it replies to. Must not register.
                    continue
                # A message can (rarely) carry more than one link/attachment; treat it as one
                # submission and key on the first source so it doesn't get split into several.
                dedupe_key = sources[0]
                if dedupe_key in seen_edits:
                    continue
                seen_edits.add(dedupe_key)
                if is_video_recently_used(dedupe_key, video_history):
                    continue  # already used before, OR manually removed via /removeedit
                entries.append({"author_id": credited_author.id, "author_name": credited_author.display_name, "message_id": msg.id, "posted_at": msg.created_at, "source_key": dedupe_key})
            except Exception:
                # A single message failing to parse must never abort the whole scan — every
                # other message, including ones after this one in history, still needs checking.
                traceback.print_exc()
                print(f"⚠️ previewvotes: failed to read message {msg.id} in {guild.name} — skipping just that message.")
                continue
    except Exception:
        # An error while PAGINATING history itself (e.g. missing permission, rate limit,
        # network hiccup) — this is outside the per-message try above, so it needs its own
        # handling or the whole scan silently never finishes.
        traceback.print_exc()
        print(f"⚠️ collect_pending_entries: history scan of #{submit_channel.name} failed after {scanned} message(s) — returning what we have so far.")
        return entries

    print(f"🔎 collect_pending_entries: scanned {scanned} message(s) since {since}, found {len(entries)} pending entrie(s)")
    return entries
...
async def run_collect_job_for_guild(guild: discord.Guild):
    ...
    # The real job only ever runs exactly AT collect_hour (the scheduler triggers it then), so
    # the window is always "yesterday's collect_hour to now" — no ambiguity about which side of
    # the boundary "now" falls on. Deliberately NOT calling current_collect_window_start() here:
    # that function decides today-vs-yesterday by comparing against live now(), which is exactly
    # wrong at the one moment this job actually runs (now == today's collect_hour), collapsing
    # the window to a few seconds and finding nothing. previewvotes runs at arbitrary times
    # before the next collect, so that ambiguity-resolving logic is correct THERE, just not here.
    now = datetime.now(EU_TZ)
    since = (now.replace(hour=config["collect_hour"], minute=config.get("collect_minute", 0), second=0, microsecond=0)
              - timedelta(days=1)).astimezone(timezone.utc)
    run_ts = datetime.now(timezone.utc)
    video_history = prune_video_history(load_video_history(guild.id))
    entries = []
    seen_this_run = set()  # same video pinged more than once in this window -> only the first counts
        ...
        try:
            ...
            for video_source, video_author in await extract_video_sources(msg):
                if is_video_recently_used(video_source, video_history):
                    continue
                if video_source in seen_this_run:
                    continue
                seen_this_run.add(video_source)
                entries.append(
                    {"author_id": video_author.id, "author_name": video_author.display_name,
                     "video_url": video_source, "message_id": msg.id}
                )
                resolved_names[str(video_author.id)] = video_author.display_name
        except Exception:
            # Same reasoning as in collect_pending_entries above: one bad message must not
            # cut the scan short and silently drop every message that came after it.
            traceback.print_exc()
            print(f"⚠️ Collect job for {guild.name}: failed to read message {msg.id} — skipping just that message.")
            continue
    ...
    # One bad entry (broken/expired link, download timeout, file too big, brief Discord
    # hiccup) must NEVER be able to sink the entire day's poll for everyone else — that used
    # to happen here, because a single unhandled exception mid-loop aborted this whole
    # function before anything got saved (no poll, no persisted history, no advanced
    # timestamp). Now each entry is isolated: a failure is logged and that one entry is
    # skipped, everything else still goes out.
    successful_entries = []
    for entry in entries:
        try:
            ...
            traceback.print_exc()
            print(f"⚠️ Collect job for {guild.name}: failed to post the edit from "
                  f"{entry['author_name']} ({entry['video_url']}) — skipping just this one, "
                  f"the rest of today's collect continues.")

    # Persist whatever succeeded right away — even if poll creation below fails too, we don't
    # want to lose the video-history/timestamp progress we already made.
    save_video_history(guild.id, video_history)
    save_last_collect_ts(guild.id, run_ts)

    entries = successful_entries
    if not entries:
        print(f"⚠️ Collect job for {guild.name}: every edit failed to post today — no poll opened.")
        return
    ...
    try:
        for batch_index, batch in enumerate(batches):
            try:
                ...
                traceback.print_exc()
                print(f"⚠️ Collect job for {guild.name}: failed to open poll batch "
                      f"{batch_index + 1}/{len(batches)} — skipping that batch.")
    finally:
        # Always save whatever poll(s) actually went out, even if a later batch failed —
        # a partial poll is still better than losing the whole day silently.
        poll_state["polls"] = polls_meta
        poll_state["entries"] = entries

    if not polls_meta:
        print(f"⚠️ Collect job for {guild.name}: no poll could be opened today (all batches failed).")
    else:
...
async def run_results_job_for_guild(guild: discord.Guild):
    ...
    disqualified = poll_state.get("disqualified") or set()
    votes = [0] * len(entries)
    last_channel = None
    any_found = False
    ...
    if not any_found or not votes:
        print(f"⚠️ Results job for {guild.name}: couldn't read any poll results (message(s) missing "
              f"or not fetchable) — no winner announced today.")
        return

    eligible_indices = [i for i in range(len(votes)) if i not in disqualified]
    if not eligible_indices:
        print(f"⚠️ Results job for {guild.name}: every entry was disqualified (deleted by their authors) — no winner announced today.")
        return
    max_votes = max(votes[i] for i in eligible_indices)
    tied_indices = [i for i in eligible_indices if votes[i] == max_votes]
    ...
    if winner_index >= len(entries):
        print(f"⚠️ Results job for {guild.name}: winner index out of range — no winner announced today.")
        return
    ...
    if not results_channel:
        print(f"⚠️ Results job for {guild.name}: no results channel configured/found — winner "
              f"was picked ({best_entry['author_name']}) but nothing was posted. Check the "
              f"results channel setting on the dashboard.")

    # Everything from here on (video download, sending the announcement) can fail for reasons
    # that have nothing to do with who won (a dead link, a file too big, a brief Discord
    # hiccup). That must never leave the bot stuck re-trying the same expired poll forever —
    # so the whole block is wrapped, and save_day()/clearing poll_state always happens in the
    # finally below no matter what goes wrong inside.
    try:
      if results_channel:
        winner_points = config.get("winner_points", WINNER_POINTS)

        # Server Booster bonus: check whether the winner is currently boosting this server
        # (member.premium_since is set for as long as an active boost is applied) and, if so,
        # award the configured bonus percentage on top of the normal winner points.
        winner_member = guild.get_member(best_entry["author_id"])
        if winner_member is None:
            try:
                winner_member = await guild.fetch_member(best_entry["author_id"])
            except (discord.NotFound, discord.HTTPException):
                winner_member = None
        is_booster = bool(winner_member and winner_member.premium_since)
        bonus_percent_total = 0
        if is_booster and config.get("booster_bonus_enabled", True):
            bonus_percent_total += config.get("booster_bonus_percent", 50)

        # Manual per-role/per-user bonus rules from the dashboard — every rule that matches
        # the winner stacks additively on top of the automatic booster bonus above.
        if winner_member:
            member_role_ids = {str(r.id) for r in winner_member.roles}
            for rule in (config.get("point_bonuses") or []):
                rule_type = rule.get("type")
                target_id = str(rule.get("target_id") or "")
                if not target_id:
                    continue
                if rule_type == "user" and target_id == str(winner_member.id):
                    bonus_percent_total += int(rule.get("percent") or 0)
                elif rule_type == "role" and target_id in member_role_ids:
                    bonus_percent_total += int(rule.get("percent") or 0)

        bonus_points = round(winner_points * bonus_percent_total / 100) if bonus_percent_total else 0
        total_award = winner_points + bonus_points

        new_total = await award_points(guild, best_entry["author_id"], total_award)
        ...
        winner_use_embed = config.get("winner_use_embed", True)

        if bonus_points:
            points_value = f"**+{winner_points}** base + 🎁 **+{bonus_points}** bonus = **+{total_award}** (total: **{new_total}**)"
            points_plain = f"+{winner_points} base + 🎁 +{bonus_points} bonus = +{total_award} (total: {new_total})"
        else:
            points_value = f"**+{total_award}** (total: **{new_total}**)"
            points_plain = f"+{total_award} (total: {new_total})"
        ...
        if winner_use_embed:
            ...
            embed.add_field(name="💰 Points earned", value=points_value, inline=True)
            if is_booster:
                embed.add_field(
                    name="💎 Server Booster",
                    value="Boosting the server earns bonus Edit Points — thanks for the support!",
                    inline=False,
                )
        else:
            plain_text = (
                f"🏆 **TOP EDIT OF THE DAY — DAY {day_number}**\n"
                f"{win_line}\n"
                f"🗳️ Votes: **{best_votes}** {vote_word} · 💰 Points earned: {points_plain}"
                + ("\n💎 Server Booster bonus applied!" if is_booster else "")
            )
        ...
        if is_social_video_link(best_entry["video_url"]):
            try:
                ...
                traceback.print_exc()
                print(f"⚠️ Results job for {guild.name}: downloading the winning video failed — "
                      f"falling back to posting the link instead.")
                downloaded_path = None
            ...
            if downloaded_path:
                ...
                try:
                    await results_channel.send(
                        content=ping if embed else f"{ping}\n{plain_text}".strip(),
                        embed=embed,
                ...
                if embed:
                    ...
                    await results_channel.send(content=f"{ping}\n{plain_text}".strip())
                await results_channel.send(content=best_entry["video_url"])
        except discord.HTTPException:
            traceback.print_exc()
            print(f"⚠️ Results job for {guild.name}: the full announcement failed to send — "
                  f"trying a bare-bones fallback message so the win isn't lost entirely.")
            try:
                await results_channel.send(
                    content=(f"{ping}\n🏆 **{member_mention}** won today's Top Edit vote with "
                              f"**{best_votes}** {vote_word}!\n{best_entry['video_url']}").strip()
                )
            except discord.HTTPException:
                traceback.print_exc()
                print(f"⚠️ Results job for {guild.name}: even the fallback announcement failed to send.")
        ...
        traceback.print_exc()
        print(f"⚠️ Results job for {guild.name}: something failed while announcing today's winner "
              f"({best_entry['author_name']}) — see traceback above. Points/state below are still "
              f"finalized so the bot doesn't get stuck re-trying an expired poll tomorrow.")
    finally:
...
HELP_CATEGORIES = {
    "points": {
        ...
        "description": (
            "**`points [@user]`**\n"
            "Eye-catching embed with points, rank, progress bar, and leaderboard standing. Has a "
            "*Press for more info* button that reveals your exact leaderboard standing.\n\n"
            "**`ranks`**\n"
            "See every rank, its point threshold, and how to earn points.\n\n"
            "**`leaderboard`**\n"
            "Top 20 rankings — shows 1-10 first, swipe for 11-20.\n\n"
            "**💎 Bonus points**\n"
            "Server Boosters earn a configurable % bonus on top of the daily win automatically — "
            "and staff can set up extra bonus rules for specific roles or users on the website "
            "dashboard. All bonuses stack.\n\n"
            "**🎉 Rank-up announcements**\n"
            "Whenever your points cross into a higher rank (e.g. D → C), you get pinged with "
            "a clean, eye-catching embed in the Rankings channel — a random message from a "
            "100-line pool, your new rank, and your total points. Fully customizable "
            "(channel + message pool) by admins on the website dashboard."
        ),
    },
    "voting": {
        ...
        "description": (
            ...
            "**`previewvotes`**\n"
            "Live preview of edits already queued for the next vote — only counts messages with an actual "
            "video/link/attachment/forward attached, a bare ping registers nothing. *Only visible to you.*\n\n"
    ...
    "editing": {
        ...
        "description": (
            ...
            "**`removesubs [video] [video_url] [method] [quality]`**\n"
            "AI-removes burned-in subtitles/text from a video — upload a file directly or paste "
            "a link, original audio is kept. Free & self-hosted (runs on CPU), defaults configurable "
    ...
    "staff": {
        ...
        "description": (
            ...
            "**`deletedchannels`** *(Manage Channels/Staff/Manager)*\n"
            "Lists recently deleted channels, VCs, forums, categories, and stage channels for "
            "this server — newest first, numbered.\n\n"
            "**`restorechannel [number]`** *(Manage Channels/Staff/Manager)*\n"
            "Brings a deleted channel back exactly as it was (name, category, position, "
            "permissions, topic, etc.) — `1` (default) is the most recently deleted. Works for "
            "ANY deletion, not just raids — the built-in anti-raid system only auto-restores "
            "confirmed mass-deletion bursts from a bot; this command lets you restore a single "
            "accidental or malicious deletion on demand, anytime.\n\n"
            "**`create-embed`**\n"
    ...
    "moderation": {
        "label": "🎉 Giveaways & Mod-Tools",
        "description": (
            "*Staff/Manager only — replies are only visible to you unless noted.*\n\n"
            "**`/giveaway start [channel] [requirement_role] [bonus_role] [bonus_entries] "
            "[attachment] [ping_here]`**\n"
            "Opens a popup form (Duration, Number of Winners, Prize, Description — just like "
            "GiveawayBot) to finish setting it up. Attach an image or video — it shows up "
            "right in the giveaway embed.\n\n"
            "**`/giveaway list`** / **`/giveaway end <id>`** / **`/giveaway reroll <id>`** / "
            "**`/giveaway cancel <id>`**\n"
            "Manage running or finished giveaways. IDs come from `/giveaway list`.\n\n"
            "*Mod-log channels (ban/kick/timeout/messages), the link filter, partnership-ticket "
            "roles/category, and channel-access locks are all configured on the website "
            "dashboard's Settings page now — no commands needed for any of that.*"
        ),
    },
    "utility": {
        "label": "⚙️ Utility",
...
def build_help_home_embed():
    ...
    embed.add_field(
        name="✨ What Zexo can do",
        value=(
            "🏅 Points, ranks & daily Top-Edit voting · 💎 Booster & role/user bonus points\n"
            "🎉 Giveaways · 🛡️ Mod-log (ban/kick/timeout) · 🔗 Link filter & partnership tickets\n"
            "🔒 Channel access control · 🧽 `/removesubs` — free AI subtitle/text removal from videos\n"
            "🤖 `/rate-edit`, `/find-asset`, `/askai` — AI helpers for editors\n"
            "🌐 Full web dashboard — nearly everything above is configurable there, no commands needed"
        ),
        inline=False,
    )
...
class HelpView(discord.ui.View):
    ...
    @discord.ui.button(label="🎉 Giveaways & Mod-Tools", style=discord.ButtonStyle.secondary, row=1)
    async def moderation_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(embed=build_help_category_embed("moderation"), view=self)
...
def build_tutorial_embed(guild: discord.Guild):
    ...
    winner_points_val = config.get("winner_points", WINNER_POINTS)
    booster_pct = config.get("booster_bonus_percent", 50)
    bonus_note = ""
    if config.get("booster_bonus_enabled", True) and booster_pct:
        bonus_note += f" 💎 **Server Boosters** get an extra **{booster_pct}%** on top automatically."
    if config.get("point_bonuses"):
        bonus_note += " 🎁 A few roles/users also have extra bonus rules set by staff."
    embed.add_field(
        name="4️⃣ Winner & points",
        value=(
            f"When voting ends, the bot tallies the poll and announces the winner in <#{config['results_channel_id']}>. "
            f"The winner earns **+{winner_points_val} point(s)**.{bonus_note} Ties are broken with a coin-flip bracket."
        ),
        inline=False,
    )
    ...
    embed.add_field(
        name="5️⃣ Ranks",
        value=f"Points add up over time and unlock ranks:\n{tier_lines}\n\nUse `/ranks` for the full breakdown. Rank up and you'll get pinged with an announcement in <#{config.get('rankup_channel_id')}>!",
        inline=False,
    )
    embed.add_field(
        name="📊 Track your progress",
        value=(
            "`/points` — an embed with your points, rank, progress bar, and leaderboard standing, plus a "
            "*Press for more info* button showing the 5 closest people above you\n"
            "`/leaderboard` — top 20 rankings\n"
            "`/timeleft` — countdown to the next voting phase\n"
            "`/previewvotes` — private peek at today's vote (only counts entries with an actual video/link/attachment — a bare ping does nothing)"
        ),
        inline=False,
    )
...
async def build_points_embed(guild: discord.Guild, member: discord.Member) -> discord.Embed:
    """Eye-catchy embed version of /points — same data as build_points_message, laid out as
    fields with a tier-accent color and the member's avatar, instead of a plain-text block."""
    data = load_points(guild.id)
    total = data.get(str(member.id), 0)
    resolved_names[str(member.id)] = member.display_name
    rank_label = current_rank_label(total)
    ...
    tier_rgb = TIER_COLORS.get(rank_label or UNRANKED_LABEL, TIER_COLORS[UNRANKED_LABEL])
    embed = discord.Embed(
        title=f"{rank_emoji if rank_label else UNRANKED_EMOJI}  {member.display_name}",
        description=f"## {total} point(s)",
        color=discord.Color.from_rgb(*tier_rgb),
    )
    if member.display_avatar:
        embed.set_thumbnail(url=member.display_avatar.url)

    rank_value = f"{rank_emoji} **{rank_label}**" if rank_label else f"{UNRANKED_EMOJI} *{UNRANKED_LABEL}*"
    embed.add_field(name="🏅 Rank", value=rank_value, inline=True)

    if next_label:
        bar = progress_bar(total - low, high - low)
        embed.add_field(name=f"📈 Next: {next_label}", value=f"{bar}\n**{needed}** more point(s)", inline=True)
    else:
        embed.add_field(name="📈 Progress", value="Highest rank reached! 🎉👑", inline=True)
    ...
    if position is None:
        ...
        standing_text = f"**#1** of **{len(sorted_entries)}** editors — 👑 first place!"
    else:
        above_uid, above_score = sorted_entries[position - 2]
        above_name = await resolve_display_name(guild, above_uid)
        above_rank_emoji = current_rank_emoji(above_score)
        ...
        gap = above_score - total
        standing_text = (
            f"**#{position}** of **{len(sorted_entries)}** editors\n"
            f"⬆️ **{above_name}** — `{above_score}` pts {above_rank_emoji} *{above_rank_label}* "
            f"({gap} to catch up)"
        )
    embed.add_field(name="📊 Standing", value=standing_text, inline=False)
...
@bot.command(name="deletedchannels", aliases=["restorelist", "channelbackups"])
async def deletedchannels(ctx):
    if not has_channel_restore_permission(ctx.author):
        await ctx.send("❌ You need Manage Channels, Staff, or Manager permission to use this.")
        return
    embed = discord.Embed(
        title="🗑️ Recently Deleted Channels",
        description=build_deleted_channels_list(ctx.guild.id),
        color=discord.Color.blurple(),
    )
    embed.set_footer(text="Use !restorechannel <number> (or /restorechannel) to bring one back — 1 is the most recent.")
    await ctx.send(embed=embed)


@bot.command(name="restorechannel", aliases=["restorechan", "unarchivechannel"])
async def restorechannel(ctx, index: int = 1):
    if not has_channel_restore_permission(ctx.author):
        await ctx.send("❌ You need Manage Channels, Staff, or Manager permission to use this.")
        return
    result = await _do_restore_channel(ctx.guild, ctx.author.mention, index)
...
async def points(ctx, member: discord.Member = None):
    ...
    embed = await build_points_embed(ctx.guild, member)
...
async def previewvotes(ctx):
    ...
    if not entries:
        ...
        lines = [f"**{i}.** {entry['author_name']} — <t:{int(entry['posted_at'].timestamp())}:R>" for i, entry in enumerate(entries, start=1)]
        embed = discord.Embed(
            title="🗳️ Pending Edits Preview",
            description=(
                "These are the people currently queued to be in the **next voting poll** — "
                "not a ranking, not results, just who's pinged the Picker role so far:\n\n" + "\n".join(lines)
            ),
            color=discord.Color.blurple(),
        )
...
async def airating_cmd(ctx, *, video_url: str = None):
    ...
    async with ctx.typing():
        ...
        frame_paths = []
        try:
            # 60 frames instead of 4 — dense, evenly-spaced coverage across the whole clip gives
            # the model a real basis for judging grading *consistency* and cut frequency, not
            # just a couple of lucky/unlucky stills. Downscaled + smaller JPEGs in _extract_frames
            # keep this from ballooning the upload/token cost despite the much higher count.
            frame_paths = await asyncio.get_running_loop().run_in_executor(None, _extract_frames, video_path, 60)
            if not frame_paths:
            ...
            embed = _build_rating_embed(feedback, ctx.author.display_avatar.url)
            await ctx.send(embed=embed)
...
async def create_embed_cmd(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"*(Log in with Discord if asked — you need Administrator, Manage Server, Manage Channels, "
            f"or Manage Messages on this server.)*"
        ),
...
@bot.tree.command(name="setup", description="Get the web dashboard link to configure this server")
async def slash_setup(interaction: discord.Interaction):
    embed = discord.Embed(
        title="⚙️ Set up Zexo",
        description=(
            "Nearly everything — channels, roles, ranks & points, timing — is configured on the "
            "web dashboard, no commands needed:\n\n"
            "🌐 **https://fourk-discord-bot-6.onrender.com**\n\n"
            "New here? Run **`/tutorial`** for a full walkthrough, or **`/help`** for the full command list."
        ),
        color=discord.Color.blurple(),
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def slash_previewvotes(interaction: discord.Interaction):
    print(f"➡️ /previewvotes invoked by {interaction.user} in {interaction.guild.name if interaction.guild else '?'}")
    await interaction.response.defer(ephemeral=True)
    print("➡️ /previewvotes: deferred, calling collect_pending_entries...")
    try:
        entries = await asyncio.wait_for(collect_pending_entries(interaction.guild), timeout=240)
    except asyncio.TimeoutError:
        print("⚠️ /previewvotes: collect_pending_entries took over 240s — aborting and telling the user.")
        await interaction.followup.send("⚠️ This is taking too long (probably a lot of messages to scan). Try again in a bit.", ephemeral=True)
        return
    print(f"➡️ /previewvotes: collect_pending_entries returned {('None' if entries is None else len(entries))}")
    if entries is None:
...
class LeaveServerConfirmView(discord.ui.View):
    """Second step: shows exactly which server was picked and requires an explicit Confirm
    click before actually leaving — a wrong dropdown click should never be enough on its own
    to make the bot leave a server."""
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await interaction.response.edit_message(content=f"⚠️ Couldn't leave **{name}**: {e}", view=None, embed=None)
            return
        await interaction.response.edit_message(content=f"👋 Left **{name}**.", view=None, embed=None)

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary)
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(content="Cancelled — didn't leave anything.", view=None, embed=None)


class LeaveServerSelectView(discord.ui.View):
    """First step: a dropdown of every server the bot is currently in. Only the person who
    opened this (the bot owner) can use it — checked both in the Select's own interaction_check
    and, redundantly, on the command itself before this view is even shown."""
    def __init__(self, owner_id: int, guilds: list[discord.Guild]):
        super().__init__(timeout=60)
        self.owner_id = owner_id
        # Discord select menus cap out at 25 options — with more servers than that, only the
        # first 25 show up here; there's no realistic path to more right now, but silently
        # truncating instead of erroring keeps this from ever hard-breaking as the bot grows.
        options = [
            discord.SelectOption(label=g.name[:100], description=f"{g.member_count} members · ID {g.id}", value=str(g.id))
            for g in guilds[:25]
        ]
        self.guild_by_id = {g.id: g for g in guilds}
        select = discord.ui.Select(placeholder="Pick a server to leave...", options=options)
    ...
    async def on_select(self, interaction: discord.Interaction):
        ...
        if guild is None:
            await interaction.response.edit_message(content="⚠️ That server isn't available anymore.", view=None)
            return
        embed = discord.Embed(
            title="⚠️ Confirm leaving this server?",
            description=f"**{guild.name}**\n{guild.member_count} members · ID `{guild.id}`",
            color=discord.Color.red(),
        )
        await interaction.response.edit_message(embed=embed, view=LeaveServerConfirmView(self.owner_id, guild))


@bot.tree.command(name="leaveserver", description="Owner only: make the bot leave a server")
async def slash_leaveserver(interaction: discord.Interaction):
    ...
    if not guilds:
        await interaction.response.send_message("The bot isn't in any servers.", ephemeral=True)
        return
    await interaction.response.send_message(
        f"Pick a server for me to leave ({len(guilds)} total):",
        view=LeaveServerSelectView(interaction.user.id, guilds),
        ephemeral=True,
    )


class RemoveEditConfirmView(discord.ui.View):
    """Second step: confirms exactly which pending edit is about to be pulled before it
    actually happens — same pattern as LeaveServerConfirmView, for the same reason."""
    def __init__(self, owner_id: int, guild: discord.Guild, entry: dict):
        super().__init__(timeout=60)
        self.owner_id = owner_id
        self.guild = guild
        self.entry = entry
    ...
    @discord.ui.button(label="Confirm Remove", style=discord.ButtonStyle.danger, emoji="🗑️")
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        history = load_video_history(self.guild.id)
        mark_video_used(self.entry["source_key"], history)
        save_video_history(self.guild.id, history)
        await interaction.response.edit_message(
            content=f"🗑️ Removed **{self.entry['author_name']}**'s edit — it won't be included in today's (or any future) vote.",
            view=None, embed=None,
        )

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary)
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(content="Cancelled — didn't remove anything.", view=None, embed=None)


class RemoveEditSelectView(discord.ui.View):
    """First step: a dropdown of every edit currently pending for the next vote in this
    server (same list /previewvotes shows). Owner-only, same as the leave-server flow."""
    def __init__(self, owner_id: int, guild: discord.Guild, entries: list):
        super().__init__(timeout=60)
        self.owner_id = owner_id
        self.guild = guild
        ...
        options = [
            discord.SelectOption(
                label=entry["author_name"][:100],
                description=f"posted {discord.utils.format_dt(entry['posted_at'], 'R')}"[:100],
                value=str(i),
            )
            for i, entry in enumerate(entries[:25])
        ]
        select = discord.ui.Select(placeholder="Pick an edit to remove...", options=options)
        select.callback = self.on_select
        self.add_item(select)
    ...
    async def on_select(self, interaction: discord.Interaction):
        entry = self.entries[int(interaction.data["values"][0])]
        embed = discord.Embed(
            title="⚠️ Confirm removing this edit?",
            description=f"**{entry['author_name']}**\nposted {discord.utils.format_dt(entry['posted_at'], 'R')}\n\nWon't be included in today's vote, and won't reappear on future collects either.",
            color=discord.Color.red(),
        )
        await interaction.response.edit_message(embed=embed, view=RemoveEditConfirmView(self.owner_id, self.guild, entry))


@bot.tree.command(name="removeedit", description="Owner only: pull a pending edit out of today's vote")
async def slash_removeedit(interaction: discord.Interaction):
    if not await bot.is_owner(interaction.user):
        await interaction.response.send_message("⛔ This command is owner-only.", ephemeral=True)
        return
    ...
    if not entries:
        await interaction.followup.send("Nothing pending right now — nothing to remove.", ephemeral=True)
        return
    await interaction.followup.send(
        f"Pick an edit to remove ({len(entries)} pending):",
        view=RemoveEditSelectView(interaction.user.id, interaction.guild, entries),
        ephemeral=True,
    )
...
async def slash_points(interaction: discord.Interaction, member: discord.Member = None):
    ...
    embed = await build_points_embed(interaction.guild, member)
...
@bot.tree.command(name="deletedchannels", description="List recently deleted channels/VCs/forums you can bring back (Manage Channels/Staff/Manager only)")
async def slash_deletedchannels(interaction: discord.Interaction):
    if not has_channel_restore_permission(interaction.user):
        await interaction.response.send_message("❌ You need Manage Channels, Staff, or Manager permission to use this.", ephemeral=True)
        return
    embed = discord.Embed(
        title="🗑️ Recently Deleted Channels",
        description=build_deleted_channels_list(interaction.guild.id),
        color=discord.Color.blurple(),
    )
    embed.set_footer(text="Use /restorechannel <number> to bring one back — 1 is the most recent.")
    await interaction.response.send_message(embed=embed, ephemeral=True)


@bot.tree.command(name="restorechannel", description="Restore a recently deleted channel, VC, forum, category, or stage (Staff/Manager only)")
@app_commands.describe(number="Which deleted channel to restore — 1 is most recent. See /deletedchannels for the list.")
async def slash_restorechannel(interaction: discord.Interaction, number: int = 1):
    await interaction.response.defer(ephemeral=True)
    if not has_channel_restore_permission(interaction.user):
        await interaction.followup.send("❌ You need Manage Channels, Staff, or Manager permission to use this.", ephemeral=True)
        return
    result = await _do_restore_channel(interaction.guild, interaction.user.mention, number)
...
RATE_EDIT_PROMPT = (
    "You are a blunt, no-mercy video editor giving cold, fact-only feedback on 60 evenly-spaced "
    "frames pulled from across someone's edit, start to finish. No sympathy, no cushioning, no "
    "encouragement-for-its-own-sake — every claim has to be backed by something actually visible "
    "in these frames. Compare the frames against each other to judge whether color grading, "
    "exposure, and visual style stay CONSISTENT throughout, or drop off partway — that's real "
    "signal, factor it into the score.\n\n"
    "CALIBRATION — read this carefully:\n"
    "Weigh overall craft and intent above isolated technical hiccups. An edit that clearly shows "
    "deliberate color grading, sync/impact framing, or intentional text/style choices earns a "
    "7-8 even if a frame or two has exposure or motion-blur issues — a couple of imperfect frames "
    "in an otherwise well-crafted edit is a minor note in your tips, not a reason to drag the "
    "whole score down to mediocre. Save the low end (3-5) for edits that genuinely lack effort "
    "throughout: default unedited cuts, no grading at all (or a single flat filter), no deliberate "
    "framing choices, nothing distinctive. Reserve 9-10 for edits that would hold up next to "
    "professional work. Don't default to a 'safe' middle score out of politeness — commit to the "
    "number the frames actually support, high or low, and back it with what you actually saw.\n\n"
    "Reply in EXACTLY this format, nothing before or after it, no extra commentary:\n"
    "SCORE: <X>/10\n"
    "TIER: <Beginner|Intermediate|Advanced|Pro>\n"
    "SOFTWARE: <best guess, or 'Not identifiable from stills'>\n"
    "TIPS:\n"
    "- <specific, concrete point #1 — under 18 words>\n"
    "- <specific, concrete point #2 — under 18 words>\n"
    "- <specific, concrete point #3 — under 18 words>\n\n"
    "Tier must match the score: Beginner 1-4, Intermediate 5-6, Advanced 7-8, Pro 9-10. Only name "
    "specific software if the style gives a clear, distinctive signal (recognizable preset, "
    "transition, text-animation look) — otherwise say it's not identifiable.\n\n"
    "TIPS RULES — this is where you tend to go soft and vague, don't:\n"
    "- At least 2 of the 3 tips MUST be actual weaknesses/flaws, stated directly — vague filler "
    "like 'could be more dynamic' or 'try experimenting more' is USELESS and forbidden. Name the "
    "concrete thing: what's wrong, and where/how it shows (e.g. 'flat, uncorrected white balance "
    "in every frame', 'text overlay is static, no motion or sync to any cut', 'composition is "
    "dead-center in nearly every frame, no variation'). If there's a real technical issue "
    "(blown highlights, crushed blacks, motion blur, low resolution/compression artifacts, "
    "mismatched aspect ratio, shaky/unstabilized footage), call it out by name.\n"
    "- If — and only if — the edit is genuinely strong with no real flaws visible, up to 1 of the "
    "3 tips can note what's working instead, stated as fact, not praise. Never pad a weak edit "
    "with a compliment to soften it.\n"
    "- No hedging language anywhere ('maybe', 'consider', 'might help', 'could try') — flat "
    "statements only. You're judging still frames, not motion/timing/sync — don't claim to assess "
    "smoothness or transitions you can't see, and you have no access to other submissions or a "
    "live leaderboard to compare against."
)


_RATING_SCORE_RE = re.compile(r"SCORE:\s*(\d{1,2})\s*/\s*10", re.IGNORECASE)
_RATING_TIER_RE = re.compile(r"TIER:\s*(Beginner|Intermediate|Advanced|Pro)", re.IGNORECASE)
_RATING_SOFTWARE_RE = re.compile(r"SOFTWARE:\s*(.+)", re.IGNORECASE)
_RATING_TIPS_RE = re.compile(r"TIPS:\s*((?:\s*-.+\n?)+)", re.IGNORECASE)

_TIER_EMOJI = {"beginner": "🌱", "intermediate": "⚙️", "advanced": "🔥", "pro": "👑"}


def _build_rating_embed(raw_feedback: str, avatar_url: str) -> discord.Embed:
    """Turns the model's structured SCORE/TIER/SOFTWARE/TIPS reply into a compact, scannable
    embed instead of dumping one long paragraph into the description. Falls back to the raw
    text if the model didn't follow the format for some reason, so a rating never just fails."""
    score_m = _RATING_SCORE_RE.search(raw_feedback)
    tier_m = _RATING_TIER_RE.search(raw_feedback)
    software_m = _RATING_SOFTWARE_RE.search(raw_feedback)
    tips_m = _RATING_TIPS_RE.search(raw_feedback)

    if not (score_m and tier_m):
        # Format didn't parse — better to show the raw feedback than nothing at all.
        embed = discord.Embed(title="🎬 Edit Rating", description=raw_feedback.strip()[:4000], color=discord.Color.purple())
        embed.set_footer(text="AI-generated feedback from a few sampled frames — take it as a starting point, not gospel.")
        _style_ai_result_embed(embed, avatar_url)
        return embed

    score = int(score_m.group(1))
    tier = tier_m.group(1).capitalize()
    software = (software_m.group(1).strip() if software_m else "Not identifiable from stills")
    software = software.split("\n")[0].strip()
    tips_raw = tips_m.group(1).strip() if tips_m else ""
    tips = "\n".join(f"• {line.lstrip('- ').strip()}" for line in tips_raw.splitlines() if line.strip())

    if score >= 9:
        color, bar = discord.Color.gold(), "🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩"
    elif score >= 7:
        color, bar = discord.Color.green(), "🟩🟩🟩🟩🟩🟩🟩🟩⬜⬜"
    elif score >= 5:
        color, bar = discord.Color.orange(), "🟧🟧🟧🟧🟧⬜⬜⬜⬜⬜"
    else:
        color, bar = discord.Color.red(), "🟥🟥🟥⬜⬜⬜⬜⬜⬜⬜"

    embed = discord.Embed(title=f"🎬 Edit Rating — {score}/10", color=color)
    embed.description = bar
    embed.add_field(name=f"{_TIER_EMOJI.get(tier.lower(), '🎯')} Tier", value=f"**{tier}**", inline=True)
    embed.add_field(name="🛠️ Software", value=software or "Not identifiable from stills", inline=True)
    if tips:
        embed.add_field(name="💡 Improve", value=tips, inline=False)
    embed.set_footer(text="AI feedback from sampled frames — a starting point, not gospel.")
    _style_ai_result_embed(embed, avatar_url)
    return embed
...
async def _run_rate_edit(interaction: discord.Interaction, source_message: discord.Message):
    ...
    frame_paths = []
    try:
        # Same accuracy bump as .airating — 60 evenly-spaced frames instead of 4.
        frame_paths = await asyncio.get_running_loop().run_in_executor(None, _extract_frames, video_path, 60)
        if not frame_paths:
        ...
        embed = _build_rating_embed(feedback, interaction.user.display_avatar.url)
        await interaction.followup.send(embed=embed)
...
REMOVESUBS_METHOD_CHOICES = [
    app_commands.Choice(name="Hybrid (recommended)", value="hybrid"),
    app_commands.Choice(name="Fast inpaint", value="inpaint"),
    app_commands.Choice(name="Smooth gradients (Navier-Stokes)", value="inpaint_ns"),
    app_commands.Choice(name="Blur", value="blur"),
    app_commands.Choice(name="Black box", value="black"),
    app_commands.Choice(name="Background color fill", value="background"),
]
REMOVESUBS_QUALITY_CHOICES = [
    app_commands.Choice(name="Fast (360p — quickest, draft quality)", value="360p"),
    app_commands.Choice(name="Balanced (480p — recommended)", value="480p"),
    app_commands.Choice(name="High (720p — slower)", value="720p"),
    app_commands.Choice(name="Highest (1080p — slowest, best detail)", value="1080p"),
]


@bot.tree.command(name="removesubs", description="Free AI removal of burned-in subtitles/text from a video — upload a file or paste a link")
@app_commands.describe(
    video="Upload a video file directly (mp4/mov/webm/avi)",
    video_url="...or paste a YouTube/TikTok/Streamable/etc. link instead",
    method="Removal algorithm — leave empty to use this server's default",
    quality="Processing quality vs. speed — leave empty to use this server's default",
)
@app_commands.choices(method=REMOVESUBS_METHOD_CHOICES, quality=REMOVESUBS_QUALITY_CHOICES)
async def slash_removesubs(
    interaction: discord.Interaction,
    ...
    settings = load_guild_config(interaction.guild.id) if interaction.guild else default_guild_config()
    if not settings.get("removesubs_enabled", True):
        await interaction.response.send_message("⚠️ Subtitle removal is turned off on this server (a staff member can enable it on the dashboard).", ephemeral=True)
        return
    if not text_remover.model_available():
        await interaction.response.send_message(
            "⚠️ Subtitle removal isn't set up yet — the detector model file is missing on the bot's host. "
            "See the setup instructions at the top of `text_remover.py`.",
            ephemeral=True,
        )
        return
    if video is None and not video_url:
        await interaction.response.send_message("⚠️ Attach a video file, or give a `video_url`.", ephemeral=True)
        return
    if video is not None and video.content_type and not video.content_type.startswith("video"):
        await interaction.response.send_message("⚠️ That attachment isn't a video.", ephemeral=True)
        return

    await interaction.response.defer()
    chosen_method = method.value if method else settings.get("removesubs_default_method", "hybrid")
    chosen_quality = quality.value if quality else settings.get("removesubs_default_quality", "480p")
    max_seconds = settings.get("removesubs_max_seconds", MAX_REMOVESUBS_SECONDS)
    local_path = processed_path = final_path = None
    try:
        if video is not None:
            local_path = f"/tmp/removesubs_src_{random.randint(0, 10**9)}.mp4"
            await video.save(local_path)
        else:
            if is_social_video_link(video_url):
                local_path = await download_social_video(video_url, interaction.guild)
            if not local_path:
                await interaction.followup.send("⚠️ Couldn't download that link — try uploading the file directly instead.")
                return

        duration = await asyncio.get_running_loop().run_in_executor(None, _get_video_duration_seconds, local_path)
        if duration and duration > max_seconds:
            await interaction.followup.send(
                f"⚠️ That video is {int(duration)}s long — subtitle removal is capped at "
                f"{max_seconds}s on this server since this runs on the bot's own CPU (no paid GPU service)."
            )
            return

        await interaction.followup.send("⏳ Removing subtitles — this runs on CPU, so it can take a few minutes for longer clips...")

        processed_path = await asyncio.get_running_loop().run_in_executor(
            None, _run_text_removal_sync, local_path, chosen_method, chosen_quality
        )
        if not processed_path:
            await interaction.followup.send("⚠️ Subtitle removal failed. Try again, or try a shorter clip.")
            return

        final_path = await asyncio.get_running_loop().run_in_executor(
            None, _remux_original_audio_sync, processed_path, local_path
        )

        max_bytes = interaction.guild.filesize_limit if interaction.guild else (25 * 1024 * 1024)
        if os.path.getsize(final_path) > max_bytes:
            await interaction.followup.send("⚠️ The cleaned video is too large to upload to this server (Discord's file size limit).")
            return

        await interaction.followup.send(
            content=f"✅ Subtitles removed (`{chosen_method}` method, {chosen_quality}).",
            file=discord.File(final_path, filename="no_subs.mp4"),
        )
    except Exception as e:
        print(f"⚠️ /removesubs failed: {e}")
        traceback.print_exc()
        await interaction.followup.send("⚠️ Something went wrong removing the subtitles. Try again in a bit.")
    finally:
        for p in {local_path, processed_path, final_path}:
...
# ============================================================
# NEW SYSTEMS: Giveaways, Mod-Log (ban/kick/timeout), Link-Filter + Partnership Tickets,
# Channel-Access locks, and standalone /timeout /untimeout commands.
# Follows the exact same storage convention as everything above: Postgres kv_store if
# DATABASE_URL is set, else a local per-guild JSON file that survives until the next wipe.
# ============================================================

GIVEAWAY_COLOR = discord.Color.from_rgb(168, 85, 247)
GIVEAWAY_ENDED_COLOR = discord.Color.from_rgb(107, 114, 128)
PARTNERSHIP_COLOR = discord.Color.from_rgb(59, 130, 246)
MODLOG_BAN_COLOR = discord.Color.from_rgb(239, 68, 68)
MODLOG_KICK_COLOR = discord.Color.from_rgb(249, 115, 22)
MODLOG_TIMEOUT_COLOR = discord.Color.from_rgb(234, 179, 8)
MODLOG_UNTIMEOUT_COLOR = discord.Color.from_rgb(34, 197, 94)


# --- Storage ---

def load_giveaways(guild_id: int) -> dict:
    raw = db_get_raw(f"giveaways:{guild_id}")
    if raw is not None:
        try:
            return json.loads(raw)
        except json.JSONDecodeError:
            return {}
    path = f"giveaways_{guild_id}.json"
    if not os.path.exists(path):
        return {}
    try:
        with open(path, "r") as f:
...
def save_giveaways(guild_id: int, data: dict):
    if db_set_raw(f"giveaways:{guild_id}", json.dumps(data)):
        return
    with open(f"giveaways_{guild_id}.json", "w") as f:
        json.dump(data, f)


def load_tickets(guild_id: int) -> dict:
    raw = db_get_raw(f"tickets:{guild_id}")
    if raw is not None:
        try:
            return json.loads(raw)
        except json.JSONDecodeError:
            return {}
    path = f"tickets_{guild_id}.json"
    if not os.path.exists(path):
        return {}
    try:
        with open(path, "r") as f:
...
def save_tickets(guild_id: int, data: dict):
    if db_set_raw(f"tickets:{guild_id}", json.dumps(data)):
        return
    with open(f"tickets_{guild_id}.json", "w") as f:
        json.dump(data, f)


# --- Shared permission + parsing helpers ---

def has_mod_permission(member: discord.Member) -> bool:
    """Administrator, or anyone with real Discord mod permissions, or configured Staff/Manager."""
    if (member.guild_permissions.administrator or member.guild_permissions.moderate_members
            or member.guild_permissions.kick_members or member.guild_permissions.ban_members):
        return True
    config = load_guild_config(member.guild.id)
    return (
        _member_has_any_role(member, config.get("staff_role_ids", []), STAFF_ROLE_NAME)
        or _member_has_any_role(member, config.get("manager_role_ids", []), MANAGER_ROLE_NAME)
    )


def _parse_duration(text: str):
    """Parses '10m', '1h', '2h30m', '1d', or a bare number (treated as minutes) into seconds.
    ...
    total = sum(int(num) * unit_seconds[unit] for num, unit in matches)
    return total if total > 0 else None


async def _audit_actor_and_reason(guild: discord.Guild, action, target_id: int):
    """Best-effort lookup of who did a moderation action and why, via the audit log.
    Only trusts entries from the last 15 seconds so we never misattribute an old,
    unrelated log entry to a fresh event."""
    try:
...
async def _send_modlog(guild: discord.Guild, embed: discord.Embed, kind: str):
    """kind is one of 'ban', 'kick', 'timeout' — each has its own separately configurable
    log channel now, instead of one combined Mod-Log channel."""
    config = load_guild_config(guild.id)
    channel_id = config.get(f"{kind}log_channel_id")
    if not channel_id:
        return
...
def save_message_snapshot(message: discord.Message):
    """Stores a lightweight snapshot (author, content, attachment URLs) of a message so the
    message-log can still show what a deleted message contained no matter how long ago it was
    posted — even past discord.py's own short-lived message cache, and even across a bot
    restart. Scoped to guilds that have msglog_channel_id configured, so servers not using
    this feature don't pay for the extra DB writes."""
    data = {
    ...
    db_set_raw(f"msgsnap:{message.id}", json.dumps(data))


def load_message_snapshot(message_id: int):
    raw = db_get_raw(f"msgsnap:{message_id}")
    if not raw:
        return None
    try:
        return json.loads(raw)
...
async def _prune_old_message_snapshots(max_age_days: int = 30):
    """Runs once a day (see scheduler_loop). Without this, msgsnap: rows would accumulate in
    kv_store forever — every logged message adds one and nothing ever removed them."""
    keys = await asyncio.to_thread(db_list_keys_with_prefix, "msgsnap:")
    if not keys:
        return
    cutoff = datetime.now(timezone.utc) - timedelta(days=max_age_days)
    removed = 0
    for key in keys:
        snap = await asyncio.to_thread(load_message_snapshot, int(key.split(":", 1)[1]))
        if snap is None:
            continue
        try:
            posted_at = datetime.fromisoformat(snap["created_at"])
        ...
        if posted_at < cutoff:
            ...
            removed += 1
    if removed:
        print(f"🧹 Pruned {removed} message snapshot(s) older than {max_age_days} days.")


async def _log_deleted_message(guild_id: int, channel_id: int, cached_message, message_id: int = None):
    """Logs a deleted message (text, images, gifs, videos, tiktok/other links — whatever it
    contained) to the configured message-log channel. cached_message is only available if the
    message was still in discord.py's internal cache when it got deleted; otherwise this falls
    back to our own longer-lived snapshot (see save_message_snapshot) before finally giving up
    and logging just the bare fact that *something* got deleted."""
    config = load_guild_config(guild_id)
    log_channel_id = config.get("msglog_channel_id")
    if not log_channel_id:
        return
    ...
    if not log_channel:
        return
    if cached_message is not None and cached_message.author.bot:
        return  # never log the bot's own deleted messages (reminders, embeds, etc.)

    snapshot = None
    if cached_message is None and message_id is not None:
        snapshot = await asyncio.to_thread(load_message_snapshot, message_id)
        if snapshot is not None and snapshot.get("author_id") == (bot.user.id if bot.user else None):
            return  # never log the bot's own deleted messages, snapshot path too

    embed = discord.Embed(title="🗑️ Message Deleted", color=discord.Color.from_rgb(239, 68, 68), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Channel", value=f"<#{channel_id}>", inline=True)
    if cached_message is not None:
        embed.add_field(name="Author", value=f"{cached_message.author.mention} (`{cached_message.author.id}`)", inline=True)
        content = cached_message.content or "*(no text — attachment/embed only)*"
        if len(content) > 1000:
            content = content[:1000] + "…"
        embed.add_field(name="Content", value=content, inline=False)
        if cached_message.attachments:
            files_list = "\n".join(f"📎 [{a.filename}]({a.url})" for a in cached_message.attachments[:5])
            embed.add_field(name="Attachments (Image/Video/GIF)", value=files_list, inline=False)
        link_field = _extract_link_field(cached_message.content)
        if link_field:
            embed.add_field(name="🔗 Detected Links", value=link_field[:1024], inline=False)
        avatar = getattr(cached_message.author, "display_avatar", None)
        if avatar:
            embed.set_thumbnail(url=avatar.url)
    elif snapshot is not None:
        embed.add_field(name="Author", value=f"<@{snapshot['author_id']}> (`{snapshot['author_id']}`)", inline=True)
        content = snapshot.get("content") or "*(no text — attachment/embed only)*"
        if len(content) > 1000:
            content = content[:1000] + "…"
        embed.add_field(name="Content", value=content, inline=False)
        attachments = snapshot.get("attachments") or []
        if attachments:
            files_list = "\n".join(f"📎 [{a['filename']}]({a['url']})" for a in attachments[:5])
            embed.add_field(name="Attachments (Image/Video/GIF)", value=files_list, inline=False)
        link_field = _extract_link_field(snapshot.get("content") or "")
        if link_field:
            embed.add_field(name="🔗 Detected Links", value=link_field[:1024], inline=False)
        if snapshot.get("author_avatar"):
            embed.set_thumbnail(url=snapshot["author_avatar"])
        embed.set_footer(text="Recovered from message-log history (not in Discord's live cache)")
    else:
        embed.description = "*(Message content wasn't cached — no further details available)*"
    try:
        await log_channel.send(embed=embed)
    except discord.HTTPException:
        pass
...
GENERIC_LINK_REGEX = re.compile(r'(https?://|www\.)\S+', re.IGNORECASE)
LINK_EXTRACT_REGEX = re.compile(r'(?:https?://\S+|www\.\S+)', re.IGNORECASE)
# Only actual Discord server invites — discord.gg/xxx or discord.com(/app)/invite/xxx.
# This is what the link filter should act on now, so GIFs (tenor/giphy), YouTube, TikTok,
# images, etc. are left alone.
DISCORD_INVITE_REGEX = re.compile(
    r'(discord\.gg/|discord(?:app)?\.com/invite/)\S+', re.IGNORECASE
)


def _classify_link_label(url: str) -> str:
    host = _get_host(url)
    if "youtu" in host:
        return "▶️ YouTube"
    if "tiktok" in host:
    ...
    if "spotify" in host:
        return "🎧 Spotify"
    if "tenor" in host or "giphy" in host:
        return "🖼️ GIF"
    if "imgur" in host:
        return "🖼️ Imgur"
    return "🔗 Link"
...
def _extract_link_field(text: str):
    """Pulls every link out of a message and labels it by platform (YouTube, TikTok, Discord
    invite, etc.) so a mod-log entry shows at a glance WHAT kind of link was posted/removed —
    not just a wall of raw text."""
    if not text:
        return None
    urls = LINK_EXTRACT_REGEX.findall(text)
    if not urls:
        return None
    lines = []
    for url in urls[:8]:
        clean = url.rstrip(").,!?>*_\u200b")
        lines.append(f"{_classify_link_label(clean)}: {clean}")
    return "\n".join(lines)


def _guild_link_filter_check(message: discord.Message, config: dict) -> bool:
    """True if this message should be deleted by the link filter."""
    if not config.get("linkfilter_enabled"):
        return False
    ...
    if member.guild_permissions.administrator:
        return False
    if _member_has_any_role(member, config.get("staff_role_ids", []), STAFF_ROLE_NAME):
        return False
    if _member_has_any_role(member, config.get("manager_role_ids", []), MANAGER_ROLE_NAME):
        return False
    partnership_role_ids = config.get("partnership_role_ids", [])
    if partnership_role_ids and _member_has_any_role(member, partnership_role_ids, "Partnership Recruiter"):
        return False
    if _member_has_any_role(member, config.get("linkfilter_bypass_role_ids", []), ""):
        return False
    return True
...
def _pick_winners(weighted_pool: list, count: int) -> list:
    """Weighted random pick without replacement — duplicates in weighted_pool (from bonus
    entries) increase a user's odds without letting them win the same giveaway twice."""
...
def _build_giveaway_embed(gw: dict, guild: discord.Guild, ended: bool = False) -> discord.Embed:
    entries_count = len(gw.get("entries", []))
    if ended:
        title = "🎉 GIVEAWAY ENDED 🎉"
        winners = gw.get("winners", [])
        if gw.get("cancelled"):
            desc = f"**Prize:** {gw['prize']}\n\n🚫 **Cancelled — no winner.**"
        elif winners:
    ...
    embed.add_field(name="🏆 Winner(s)", value=str(gw["winners_count"]), inline=True)
    embed.add_field(name="👥 Entries", value=str(entries_count), inline=True)
    if not ended:
        embed.add_field(name="⏰ Ends", value=f"<t:{int(gw['end_ts'])}:R>", inline=True)
    if gw.get("requirement_role_id"):
        embed.add_field(name="🔒 Requirement", value=f"<@&{gw['requirement_role_id']}>", inline=True)
    if gw.get("bonus_role_id"):
        embed.add_field(
            name="✨ Bonus Entries",
            value=f"<@&{gw['bonus_role_id']}> gets +{gw.get('bonus_entries', 1)} entries",
            inline=True,
        )
    embed.add_field(name="🎙️ Host", value=f"<@{gw['host_id']}>", inline=True)
    embed.set_footer(text=f"Zexo • Giveaway ID: {gw['message_id']}")
    if gw.get("image_url"):
...
class GiveawayView(discord.ui.View):
    """Persistent entry button. custom_id embeds the guild+message id so a fresh instance
    re-attached after a restart still routes clicks to the right giveaway."""

    def __init__(self, giveaway_key: str):
        super().__init__(timeout=None)
        self.giveaway_key = giveaway_key
        btn = discord.ui.Button(
            label="🎉 Enter",
            style=discord.ButtonStyle.success,
            custom_id=f"gw_enter:{giveaway_key}",
        )
        btn.callback = self._on_enter
        self.add_item(btn)

    async def _on_enter(self, interaction: discord.Interaction):
        guild_id_str, msg_id_str = self.giveaway_key.split(":")
        guild_id = int(guild_id_str)
        data = load_giveaways(guild_id)
        gw = data.get(msg_id_str)
        ...
        if gw.get("requirement_role_id") and not any(r.id == gw["requirement_role_id"] for r in member.roles):
            await interaction.response.send_message(
                f"❌ You need the <@&{gw['requirement_role_id']}> role to enter.", ephemeral=True
            )
            return
        entries = gw.setdefault("entries", [])
        weighted = gw.setdefault("weighted_entries", [])
        uid = member.id
        if uid in entries:
            entries.remove(uid)
            gw["weighted_entries"] = [e for e in weighted if e != uid]
            save_giveaways(guild_id, data)
            await interaction.response.send_message("↩️ You've withdrawn your entry.", ephemeral=True)
        else:
            entries.append(uid)
            weight = 1
            if gw.get("bonus_role_id") and any(r.id == gw["bonus_role_id"] for r in member.roles):
                weight += gw.get("bonus_entries", 1)
            weighted.extend([uid] * weight)
            save_giveaways(guild_id, data)
            await interaction.response.send_message(f"✅ You're entered! (Entries: {weight})", ephemeral=True)
        try:
            channel = interaction.guild.get_channel(gw["channel_id"])
            msg = await channel.fetch_message(gw["message_id"])
            await msg.edit(embed=_build_giveaway_embed(gw, interaction.guild))
        except (discord.HTTPException, AttributeError):
            pass


class GiveawayEndedView(discord.ui.View):
    """Shown on an ended giveaway's message — lets staff reroll without a slash command."""

    def __init__(self, giveaway_key: str):
        super().__init__(timeout=None)
        self.giveaway_key = giveaway_key
        btn = discord.ui.Button(
            label="🔁 Reroll", style=discord.ButtonStyle.primary, custom_id=f"gw_reroll:{giveaway_key}"
        )
    ...
    async def _on_reroll(self, interaction: discord.Interaction):
        if not has_mod_permission(interaction.user):
            await interaction.response.send_message("❌ Only Staff/Manager can reroll.", ephemeral=True)
            return
        guild_id_str, msg_id_str = self.giveaway_key.split(":")
        gw, winners = await _reroll_giveaway_by_key(int(guild_id_str), msg_id_str)
        if not gw:
            await interaction.response.send_message("⚠️ Giveaway not found.", ephemeral=True)
            return
        await interaction.response.edit_message(embed=_build_giveaway_embed(gw, interaction.guild, ended=True))
        if winners:
            await interaction.channel.send(
                f"🔁 Reroll! New winner(s): {', '.join(f'<@{w}>' for w in winners)} — **{gw['prize']}**"
            )
        else:
            await interaction.channel.send("😢 No valid entrants for the reroll.")


async def _reroll_giveaway_by_key(guild_id: int, key: str):
    data = load_giveaways(guild_id)
    gw = data.get(key)
    if not gw:
        return None, []
    pool = gw.get("weighted_entries") or gw.get("entries", [])
    winners = _pick_winners(pool, gw["winners_count"])
    gw["winners"] = winners
    save_giveaways(guild_id, data)
    return gw, winners


async def _end_giveaway(guild: discord.Guild, gw: dict, cancelled: bool = False):
    data = load_giveaways(guild.id)
    key = str(gw["message_id"])
    gw = data.get(key)
    ...
    try:
        msg = await channel.fetch_message(gw["message_id"])
        view = None if cancelled else GiveawayEndedView(f"{guild.id}:{key}")
        await msg.edit(embed=_build_giveaway_embed(gw, guild, ended=True), view=view)
    except discord.HTTPException:
        pass
    try:
        if cancelled:
            await channel.send("🚫 The giveaway was cancelled — no winner.")
        elif winners:
            await channel.send(
                f"🎉 Congratulations {', '.join(f'<@{w}>' for w in winners)}! You won **{gw['prize']}**!"
            )
        else:
            await channel.send("😢 Nobody entered validly — no winner.")
    except discord.HTTPException:
        pass
...
async def giveaway_check_loop():
    ...
    for guild in bot.guilds:
        try:
            # Offloaded to a thread — a blocking psycopg2 call here directly on the
            # event loop was freezing the gateway heartbeat (>10s) and killing the
            # bot's connection to Discord.
            data = await asyncio.to_thread(load_giveaways, guild.id)
        except Exception:
            continue
        for gw in list(data.values()):
            if gw.get("ended") or gw.get("cancelled"):
                continue
            if gw.get("end_ts", 0) <= now:
                await _end_giveaway(guild, gw)


@giveaway_check_loop.error
async def giveaway_check_loop_error(error):
    print(f"⚠️ giveaway_check_loop crashed: {error}")
    traceback.print_exc()
...
@app_commands.describe(
    ...
    host="Who's hosting this giveaway (default: you)",
    requirement_role="Role required to enter (optional)",
    bonus_role="Role that gets bonus entries (optional)",
    bonus_entries="How many extra entries the bonus role gets (default: 1)",
    ping_role="Role to ping when the giveaway starts (optional)",
    ping_here="Also ping @here when the giveaway starts? (default: no)",
    attachment="Image or video shown on the giveaway (optional)",
    attachment2="A second image/video (optional)",
    attachment3="A third image/video (optional)",
)
async def giveaway_start(
    ...
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager can start giveaways.", ephemeral=True)
        return
    target_channel = channel or interaction.channel
    if not target_channel.permissions_for(interaction.guild.me).send_messages:
        await interaction.response.send_message(f"❌ I can't send messages in {target_channel.mention}.", ephemeral=True)
        return
...
async def _create_giveaway_and_post(
    interaction: discord.Interaction,
    ...
    """Shared by the /giveaway start modal (and reusable anywhere else a giveaway needs to
    be created): validates duration/winners, posts the giveaway embed with its entry
    button, and persists it. Sends its own ephemeral response either way."""
    seconds = _parse_duration(duration_str)
    if seconds is None or seconds < 10:
        await interaction.response.send_message(
            "❌ Invalid duration. Examples: `10m`, `1h`, `2h30m`, `1d`", ephemeral=True
        )
        return
    if winners < 1:
    ...
    for att in (attachments or [])[:3]:
        ...
        if content_type.startswith("image/") and image_url is None:
            # First image becomes the embed's big image; everything else (more images,
            # or any video) goes along as a regular message attachment below it.
            image_url = att.url
        else:
            try:
    ...
    gw = {
        ...
        "requirement_role_id": requirement_role.id if requirement_role else None,
    ...
    embed = _build_giveaway_embed(gw, interaction.guild)
    ...
    allowed_roles = []
    if ping_here:
        ping_mentions.append("@here")
    if ping_role:
        ping_mentions.append(ping_role.mention)
        allowed_roles.append(ping_role)
    if ping_mentions:
        send_kwargs["content"] = f"{' '.join(ping_mentions)} 🎉 New giveaway!"
        send_kwargs["allowed_mentions"] = discord.AllowedMentions(everyone=ping_here, roles=allowed_roles)
    msg = await target_channel.send(**send_kwargs)
    gw["message_id"] = msg.id
    view = GiveawayView(f"{interaction.guild.id}:{msg.id}")
    await msg.edit(view=view)
    bot.add_view(view, message_id=msg.id)

    data = load_giveaways(interaction.guild.id)
    data[str(msg.id)] = gw
    save_giveaways(interaction.guild.id, data)
    await interaction.response.send_message(f"✅ Giveaway started in {target_channel.mention}! ID: `{msg.id}`", ephemeral=True)


class GiveawayCreateModal(discord.ui.Modal, title="Create a Giveaway"):
    """Matches GiveawayBot's own creation form (Duration / Number of Winners / Prize /
    Description) — the advanced options (channel, host, roles, ping, attachments) are
    already picked via the /giveaway start slash command's own parameters before this
    modal opens, and are just carried through here to _create_giveaway_and_post."""

    duration = discord.ui.TextInput(label="Duration", placeholder="Ex: 10 minutes", required=True, max_length=50)
    winners_count = discord.ui.TextInput(label="Number of Winners", default="1", required=True, max_length=10)
    prize = discord.ui.TextInput(label="Prize", required=True, max_length=200)
    description = discord.ui.TextInput(
        label="Description", required=False, max_length=1000, style=discord.TextStyle.paragraph
    )

    def __init__(self, target_channel, host, requirement_role, bonus_role, bonus_entries, ping_role, ping_here, attachments):
        super().__init__()
        self.target_channel = target_channel
        self.host = host
        self.requirement_role = requirement_role
        self.bonus_role = bonus_role
        self.bonus_entries = bonus_entries
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await _create_giveaway_and_post(
            ...
            ping_here=self.ping_here,
            attachments=self.attachments,
        )


@giveaway_group.command(name="end", description="End a giveaway immediately and pick winner(s)")
@app_commands.describe(giveaway_id="The giveaway's message ID (see /giveaway list)")
async def giveaway_end(interaction: discord.Interaction, giveaway_id: str):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    data = load_giveaways(interaction.guild.id)
    ...
    await _end_giveaway(interaction.guild, gw)


@giveaway_group.command(name="reroll", description="Pick new winner(s) for an ended giveaway")
@app_commands.describe(giveaway_id="The giveaway's message ID")
async def giveaway_reroll(interaction: discord.Interaction, giveaway_id: str):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Only Staff/Manager.", ephemeral=True)
        return
    gw, winners = await _reroll_giveaway_by_key(interaction.guild.id, giveaway_id)
    if not gw:
        await interaction.response.send_message("⚠️ Giveaway not found.", ephemeral=True)
        return
    await interaction.response.send_message(
        f"🔁 New winner(s): {', '.join(f'<@{w}>' for w in winners) if winners else 'No valid entrants.'}"
    )
    channel = interaction.guild.get_channel(gw["channel_id"])
    if channel:
        try:
            msg = await channel.fetch_message(int(giveaway_id))
            await msg.edit(embed=_build_giveaway_embed(gw, interaction.guild, ended=True))
        except discord.HTTPException:
            pass


@giveaway_group.command(name="cancel", description="Cancel a giveaway (no winner will be picked)")
@app_commands.describe(giveaway_id="The giveaway's message ID")
async def giveaway_cancel(interaction: discord.Interaction, giveaway_id: str):
    ...
    if not gw or gw.get("ended"):
        await interaction.response.send_message("⚠️ Giveaway not found or already ended.", ephemeral=True)
        return
    await interaction.response.send_message("🚫 Cancelling giveaway...", ephemeral=True)
    await _end_giveaway(interaction.guild, gw, cancelled=True)


@giveaway_group.command(name="list", description="Show all active giveaways")
async def giveaway_list(interaction: discord.Interaction):
    data = load_giveaways(interaction.guild.id)
    active = {k: v for k, v in data.items() if not v.get("ended") and not v.get("cancelled")}
    if not active:
        await interaction.response.send_message("No giveaways are currently running.", ephemeral=True)
        return
    embed = discord.Embed(title="🎉 Active Giveaways", color=GIVEAWAY_COLOR)
    for key, gw in active.items():
        embed.add_field(
            name=f"{gw['prize']} (ID: {key})",
            value=f"<#{gw['channel_id']}> • ends <t:{int(gw['end_ts'])}:R> • {len(gw.get('entries', []))} entries",
            inline=False,
        )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
# ============================================================
# PARTNERSHIP TICKETS (paired with the link filter above)
# ============================================================
...
class TicketCloseView(discord.ui.View):
    ...
    @discord.ui.button(label="🔒 Close Ticket", style=discord.ButtonStyle.danger, custom_id="partnership_close_ticket")
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        tickets = load_tickets(interaction.guild.id)
        t = tickets.get(str(interaction.channel.id))
        is_opener = t and t.get("user_id") == interaction.user.id
        if not (is_opener or has_mod_permission(interaction.user)):
        ...
        try:
            await interaction.channel.delete(reason=f"Partnership ticket closed by {interaction.user}")
        except discord.HTTPException:
            pass
...
class PartnershipTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="📩 Open Partnership Ticket", style=discord.ButtonStyle.primary, custom_id="partnership_open_ticket")
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        config = load_guild_config(interaction.guild.id)
        category_id = config.get("partnership_category_id")
        category = interaction.guild.get_channel(category_id) if category_id else None
        if not category:
            await interaction.response.send_message(
                "⚠️ The partnership system hasn't been set up yet (Staff: set a role and ticket category on the dashboard's Settings page).", ephemeral=True
            )
            return
        tickets = load_tickets(interaction.guild.id)
        for ch_id, t in tickets.items():
            if t.get("user_id") == interaction.user.id and not t.get("closed"):
                await interaction.response.send_message(f"⚠️ You already have an open ticket: <#{ch_id}>", ephemeral=True)
                return
        ...
        for rid in config.get("staff_role_ids", []) + config.get("manager_role_ids", []):
            role = interaction.guild.get_role(rid)
            if role:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True)

        safe_name = re.sub(r"[^a-z0-9-]", "", interaction.user.display_name.lower().replace(" ", "-"))[:20] or "user"
        try:
            channel = await interaction.guild.create_text_channel(
                name=f"partnership-{safe_name}",
                category=category,
                overwrites=overwrites,
                reason=f"Partnership ticket for {interaction.user}",
            )
        except discord.Forbidden:
            await interaction.followup.send("❌ I'm missing permission to create a channel.", ephemeral=True)
            return

        tickets[str(channel.id)] = {
            "user_id": interaction.user.id, "type": "partnership", "created_ts": time.time(), "closed": False
        }
        save_tickets(interaction.guild.id, tickets)
        ...
        welcome = discord.Embed(
            ...
            description=(
                f"Hey {interaction.user.mention}! You can introduce your project/server here "
                f"and post links — only you and the team can write here.\n\n"
                f"A team member will get back to you soon."
            ),
            color=PARTNERSHIP_COLOR,
        )
        welcome.set_footer(text="Zexo • Partnership")
        await channel.send(content=interaction.user.mention, embed=welcome, view=TicketCloseView())
...
# ============================================================
# MOD-LOG (ban / kick / timeout) — mirrors the log-channel setup from the screenshot
# ============================================================
...
async def on_member_ban(guild: discord.Guild, user):
    actor, reason = await _audit_actor_and_reason(guild, discord.AuditLogAction.ban, user.id)
    embed = discord.Embed(title="🔨 Member Banned", color=MODLOG_BAN_COLOR, timestamp=datetime.now(timezone.utc))
    ...
    if avatar:
        embed.set_thumbnail(url=avatar.url)
    await _send_modlog(guild, embed, "ban")


@bot.event
async def on_member_remove(member: discord.Member):
    # on_member_remove fires for both kicks AND ordinary leaves — only a matching, FRESH
    # audit-log kick entry tells them apart, so a normal leave is silently ignored here.
    actor, reason = await _audit_actor_and_reason(member.guild, discord.AuditLogAction.kick, member.id)
    if not actor:
        return
    embed = discord.Embed(title="👢 Member Kicked", color=MODLOG_KICK_COLOR, timestamp=datetime.now(timezone.utc))
    embed.add_field(name="User", value=f"{member} (`{member.id}`)", inline=False)
    embed.add_field(name="By", value=actor.mention, inline=True)
    embed.add_field(name="Reason", value=reason or "No reason provided", inline=True)
    embed.set_thumbnail(url=member.display_avatar.url)
    await _send_modlog(member.guild, embed, "kick")
...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    if after.timed_out_until and after.timed_out_until > now:
        actor, reason = await _audit_actor_and_reason(after.guild, discord.AuditLogAction.member_update, after.id)
        embed = discord.Embed(title="🔇 Member Timed Out", color=MODLOG_TIMEOUT_COLOR, timestamp=now)
        embed.add_field(name="User", value=f"{after} (`{after.id}`)", inline=False)
        embed.add_field(name="By", value=actor.mention if actor else "Unknown", inline=True)
        embed.add_field(name="Until", value=f"<t:{int(after.timed_out_until.timestamp())}:F>", inline=True)
        embed.add_field(name="Reason", value=reason or "No reason provided", inline=True)
        embed.set_thumbnail(url=after.display_avatar.url)
        await _send_modlog(after.guild, embed, "timeout")
    elif before.timed_out_until and not after.timed_out_until:
        actor, _ = await _audit_actor_and_reason(after.guild, discord.AuditLogAction.member_update, after.id)
        embed = discord.Embed(title="🔊 Timeout Removed", color=MODLOG_UNTIMEOUT_COLOR, timestamp=now)
        embed.add_field(name="User", value=f"{after} (`{after.id}`)", inline=False)
        embed.add_field(name="By", value=actor.mention if actor else "Unknown", inline=True)
        embed.set_thumbnail(url=after.display_avatar.url)
        await _send_modlog(after.guild, embed, "timeout")
...
# ============================================================
# Standalone /timeout and /untimeout — logged automatically by on_member_update above
# ============================================================

@bot.tree.command(name="synccommands", description="Force-refresh Zexo's slash commands for this server (Admin only)")
async def slash_synccommands(interaction: discord.Interaction):
    """Manual, on-demand re-sync for THIS guild. Exists because the automatic sync only runs
    once in on_ready — if that one attempt hits a transient error (rate limit, brief network
    blip) or a new command got added after the bot was already running, staff previously had
    no way to fix it short of a full bot restart. This does the same copy_global_to + sync
    call on demand and reports back exactly what happened (success + count, or the real
    Discord error) instead of staff only seeing it in the console logs."""
    ...
    try:
        ...
        await interaction.followup.send(
            ...
            f"If a command *still* doesn't show up in the `/` picker after this, it's Discord's "
            f"client-side cache, not the bot — fully close and reopen the Discord app (or hard-"
            f"refresh the browser tab), don't just switch channels/servers.",
            ephemeral=True,
        )
    except discord.Forbidden:
        await interaction.followup.send(
            "❌ Discord refused the sync — Zexo is missing the `applications.commands` scope in "
            "this server. Re-invite the bot using an invite link that includes that scope.",
            ephemeral=True,
        )
    except discord.HTTPException as e:
        await interaction.followup.send(f"❌ Sync failed: `{e}`\n(If this mentions rate limits, wait a few minutes and try again.)", ephemeral=True)
...
def handle_dashboard_error(e):
    """Without this, ANY unhandled exception in a route (a bad guild id, Discord API hiccup,
    a stale session, whatever) shows a raw stack trace / blank page and looks like the whole
    site 'doesn't load' — even though the bot itself and every other page are fine. This logs
    the real error to the console (so it's still debuggable) but always hands the browser back
    a small, working page instead of dying."""
    traceback.print_exc()
    code = getattr(e, "code", 500) if hasattr(e, "code") and isinstance(getattr(e, "code"), int) else 500
    return (
        "<html><body style='font-family:sans-serif;background:#0d0d12;color:#eee;padding:40px;text-align:center;'>"
        "<h2>⚠️ Something went wrong loading this page.</h2>"
        "<p>It's been logged — try going back or refreshing. If it keeps happening, check the Render logs.</p>"
...
# Discord access tokens expire (usually after a few hours) and this app never implemented a
# refresh-token flow — so a token going stale used to fail SILENTLY: fetch_user_guilds just
# returned [], the dashboard rendered its normal "No mutual servers found" empty state, and
# there was no way to tell "you have no servers" apart from "your login died". This flag lets
# /servers tell the two apart and point the person at logging in again instead of guessing.
_user_guild_fetch_failed = set()  # user_id (str) — last fetch hit an auth error
...
def fetch_user_guilds(user_id: str, access_token: str, force_refresh: bool = False):
    ...
    if res.status_code == 401:
        # Expired/invalid token — nothing to retry, the cookie is stale. Drop any cached
        # guilds for this user (they're no longer trustworthy) and flag it for /servers.
        _user_guild_cache.pop(user_id, None)
        _user_guild_fetch_failed.add(user_id)
        return []
    ...
    _user_guild_fetch_failed.discard(user_id)
    guilds = [
...
# Bits below ADMINISTRATOR that are "trusted enough" to use the embed builder — Manage Server,
# Manage Channels, and Manage Messages. This is what lets staff who AREN'T full Administrators
# actually use the embed builder (previously it silently required full Administrator, which is
# why it looked "bugged" for people who genuinely had real permissions on the server).
MANAGE_GUILD_BIT = 0x20
MANAGE_CHANNELS_BIT = 0x10
MANAGE_MESSAGES_BIT = 0x2000
EMBED_BUILDER_BITS = ADMINISTRATOR_BIT | MANAGE_GUILD_BIT | MANAGE_CHANNELS_BIT | MANAGE_MESSAGES_BIT


def user_can_use_embed_builder(guild_id: int) -> bool:
    """True if the logged-in user is either a full Administrator OR has Manage Server / Manage
    Channels / Manage Messages on this guild — the embed builder doesn't need full admin, just
    someone trusted to post things."""
    user = current_user()
    if not user:
        return False
    for g in user_guilds(user):
        if int(g["id"]) == guild_id:
            try:
                return (int(g["permissions"]) & EMBED_BUILDER_BITS) != 0
            except (ValueError, TypeError):
                return False
    return False
    ...
    <body>
      ...
      <section id="features">
        ...
        <div class="grid">
          ...
          <div class="card reveal"><div class="icon-node">🎁</div><h3>Giveaways</h3>
            <p>A GiveawayBot-style popup form to launch one — duration, winners, prize, description — plus bonus entries, requirement roles, and reroll/cancel from Discord or the dashboard.</p></div>
          <div class="card reveal"><div class="icon-node">🚨</div><h3>Mod-log</h3>
            <p>Bans, kicks, timeouts, and edited/deleted messages (with attachments) get logged automatically, with moderator and reason.</p></div>
          <div class="card reveal"><div class="icon-node">🔗</div><h3>Link filter &amp; partnerships</h3>
            <p>Auto-removes links/promo from anyone without a partnership role, and opens a ticket instead — so partnership requests never get lost.</p></div>
          <div class="card reveal"><div class="icon-node">🔒</div><h3>Channel access control</h3>
            <p>Lock a specific user or role to only being able to post in certain channels — configurable per user or per role.</p></div>
          <div class="card reveal"><div class="icon-node">🧽</div><h3>Free subtitle remover</h3>
            <p>AI-removes burned-in subtitles/text from any video, upload or link — self-hosted, no per-run cost.</p></div>
        </div>
      </section>
...
<body>
  ...
  <div class="status-badge">
    ...
    <span class="pill">👤 {{ user.username }}
      ...
      <a href="/dashboard/{{ guild_id }}/giveaways" style="color:#a855f7; margin-left:10px;">🎉 Giveaways</a>{% endif %}
      {% if can_embed %}<a href="/dashboard/{{ guild_id }}/embed-builder" style="color:#c4b5fd; margin-left:10px;">🎨 Embeds</a>{% endif %}
...
<div class="wrap">
  ...
  {% if login_expired %}
  <div class="empty" style="margin-bottom:18px; border-color:rgba(248,113,113,0.4); color:#f87171;">
    ⚠️ Your Discord login expired, so no servers could be loaded.
    <a href="/login" style="color:#f87171; text-decoration:underline;">Log in again</a> to fix this.
  </div>
  {% endif %}
...
def my_servers():
    ...
    login_expired = str(user["id"]) in _user_guild_fetch_failed
...
def web_dashboard(guild_id):
    ...
    return render_template_string(
        ...
        can_embed=user_can_use_embed_builder(guild_id),
        guild_id=guild_id,
    )


SETTINGS_HTML = """
{% macro role_picker(field_name, selected_ids) -%}
<div class="role-picker" data-field="{{ field_name }}" data-selected="{{ selected_ids|join(',') }}">
  <div class="role-chips"></div>
  <input type="text" class="role-search" placeholder="Type to search roles…" autocomplete="off">
  <div class="role-suggestions"></div>
</div>
{%- endmacro %}
<!DOCTYPE html>
<html lang="en">
...
<style>
  * { box-sizing: border-box; }
  ...
  .role-chips:empty { margin-bottom:0; }
  .role-chip { display:inline-flex; align-items:center; gap:6px; background:rgba(139,92,246,0.18);
    border:1px solid rgba(196,181,253,0.4); color:#e9e4ff; padding:6px 8px 6px 12px; border-radius:999px;
    font-size:0.85rem; }
  .role-chip button { background:none; border:none; color:#e9e4ff; cursor:pointer; font-size:0.85rem;
    line-height:1; padding:2px 4px; opacity:0.7; }
  .role-chip button:hover { opacity:1; }
  .role-search { margin:0; }
  .role-suggestions { display:none; position:absolute; z-index:20; left:0; right:0; margin-top:4px;
    background:#16161f; border:1px solid rgba(255,255,255,0.14); border-radius:10px; max-height:220px;
    overflow-y:auto; box-shadow:0 10px 30px rgba(0,0,0,0.4); }
  .role-suggestion-item { padding:10px 14px; font-size:0.9rem; cursor:pointer; }
  .role-suggestion-item:hover, .role-suggestion-item.active { background:rgba(139,92,246,0.25); }
  .role-suggestion-empty { padding:10px 14px; font-size:0.85rem; color:#6b6b80; }
</style></head>
<body>
<div class="wrap">
  ...
  <a class="back" href="/dashboard/{{ guild_id }}/giveaways" style="margin-left:14px;">🎉 Giveaways →</a>
  ...
  <form method="POST">
    ...
    <div class="card">
      ...
      <select name="submit_channel_id">
        <option value="0" {% if not config.submit_channel_id %}selected{% endif %}>— none —</option>
      ...
      <select name="discussion_channel_id">
        <option value="0" {% if not config.discussion_channel_id %}selected{% endif %}>— none —</option>
      ...
      <select name="results_channel_id">
        <option value="0" {% if not config.results_channel_id %}selected{% endif %}>— none —</option>
      ...
      <select name="log_channel_id">
        <option value="0" {% if not config.log_channel_id %}selected{% endif %}>— none —</option>
      ...
      <label>Rankings channel</label>
      <select name="rankup_channel_id">
        <option value="0" {% if not config.rankup_channel_id %}selected{% endif %}>— none —</option>
        {% for c in channels %}<option value="{{ c.id }}" {% if c.id == config.rankup_channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
      </select>
      <div class="hint">💡 Posted here — with a ping — the moment someone's points cross into a new, higher rank (e.g. D → C).</div>
    </div>

    <div class="card">
      <h2>🛡️ Roles</h2>
      <div class="desc">Who can submit, who manages, and who gets pinged. Type to search, click to add — add as many as you want.</div>
      <label>Picker role(s) (must ping one of these to submit)</label>
      {{ role_picker('picker_role_ids', config.picker_role_ids) }}
      <div class="hint">💡 Anyone can post — but the bot only picks up a submission if one of these roles gets pinged in the message. Add as many as you want.</div>
      <label>Manager role(s)</label>
      {{ role_picker('manager_role_ids', config.manager_role_ids) }}
      <div class="hint">💡 Can use manual point-adjustment and moderation commands in Discord. Add as many as you want.</div>
      <label>Staff role(s)</label>
      {{ role_picker('staff_role_ids', config.staff_role_ids) }}
      <div class="hint">💡 A lighter permission tier — staff-only slash commands, below Manager. Add as many as you want.</div>
      <label>Top-Edits ping role</label>
      <select name="top_edits_role_id">
        <option value="0" {% if not config.top_edits_role_id %}selected{% endif %}>— none —</option>
    ...
    <div class="card">
      ...
      <select name="unranked_role_id">
        <option value="0" {% if not config.unranked_role_id %}selected{% endif %}>— none —</option>
    ...
    <div class="card">
      <h2>🎁 Bonus Points</h2>
      <div class="desc">Extra % on top of the daily win points for a specific role or user (e.g. Server Boosters). Multiple matching rows stack. A real Server Booster already gets a bonus automatically below — add rows here for anything extra.</div>
      <div id="bonusRows">
        {% for row in bonus_rows %}
        <div class="tier-edit bonus-row">
          <div class="row4">
            <div>
              <label>Type</label>
              <select name="bonus_type[]" class="bonus-type-select" onchange="bonusRowToggle(this)">
                <option value="role" {% if row.type == "role" %}selected{% endif %}>Role</option>
                <option value="user" {% if row.type == "user" %}selected{% endif %}>User</option>
              </select>
            </div>
            <div class="bonus-role-field" {% if row.type == "user" %}style="display:none"{% endif %}>
              <label>Role</label>
              <select name="bonus_target_role[]">
                <option value="">— pick a role —</option>
                {% for r in roles %}<option value="{{ r.id }}" {% if row.type == "role" and row.target_id == r.id|string %}selected{% endif %}>@{{ r.name }}</option>{% endfor %}
              </select>
            </div>
            <div class="bonus-user-field" {% if row.type != "user" %}style="display:none"{% endif %}>
              <label>User ID</label>
              <input type="text" name="bonus_target_user[]" value="{% if row.type == 'user' %}{{ row.target_id }}{% endif %}" placeholder="Paste user ID">
            </div>
            <div>
              <label>Bonus % <span style="font-weight:normal; opacity:0.6;">(optional)</span></label>
              <input type="number" name="bonus_percent[]" min="0" max="1000" value="{{ row.percent }}" placeholder="0">
            </div>
            <div style="display:flex; align-items:flex-end;">
              <button type="button" class="btn-remove-rank" onclick="this.closest('.bonus-row').remove()">✕ Remove</button>
            </div>
          </div>
        </div>
        {% endfor %}
      </div>
      <button type="button" class="btn" id="addBonusBtn">+ Add bonus rule</button>
      <template id="bonusRowTemplate">
        <div class="tier-edit bonus-row">
          <div class="row4">
            <div>
              <label>Type</label>
            ...
            <div class="bonus-user-field" style="display:none">
              <label>User ID</label>
              <input type="text" name="bonus_target_user[]" value="" placeholder="Paste user ID">
            </div>
            <div>
              <label>Bonus % <span style="font-weight:normal; opacity:0.6;">(optional)</span></label>
              <input type="number" name="bonus_percent[]" min="0" max="1000" value="" placeholder="0">
            </div>
            <div style="display:flex; align-items:flex-end;">
              <button type="button" class="btn-remove-rank" onclick="this.closest('.bonus-row').remove()">✕ Remove</button>
            </div>
      ...
      <script>
        document.getElementById('addBonusBtn').addEventListener('click', function () {
          const tpl = document.getElementById('bonusRowTemplate');
          document.getElementById('bonusRows').appendChild(tpl.content.cloneNode(true));
        });
        function bonusRowToggle(typeSelect) {
          const row = typeSelect.closest('.bonus-row');
          const roleField = row.querySelector('.bonus-role-field');
          const userField = row.querySelector('.bonus-user-field');
          const isRole = typeSelect.value === 'role';
          roleField.style.display = isRole ? '' : 'none';
          userField.style.display = isRole ? 'none' : '';
        }
      </script>
      <div class="hint">💡 Pick a role directly from the dropdown, or switch Type to User and paste a user ID (right-click a user → "Copy ID", Discord Developer Mode must be on). % can be left blank — a row with no % just doesn't do anything yet. Bonus stacks with every matching row plus the automatic booster bonus.</div>
      <label>Automatic Server Booster bonus %</label>
      <input type="number" name="booster_bonus_percent" min="0" max="1000" value="{{ config.booster_bonus_percent }}">
      <div class="hint">💡 Applied automatically whenever the daily winner is actively boosting the server — no role selection needed for this one.</div>
    </div>
    ...
    <div class="card">
      <h2>🚨 Mod-Log</h2>
      <div class="desc">Bans, kicks, and timeouts are logged here automatically (moderator + reason) — each to its own channel.</div>
      <label>Ban-log channel</label>
      <select name="banlog_channel_id">
        <option value="0" {% if not config.banlog_channel_id %}selected{% endif %}>— Off —</option>
        {% for c in channels %}<option value="{{ c.id }}" {% if c.id == config.banlog_channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
      </select>
      <label>Kick-log channel</label>
      <select name="kicklog_channel_id">
        <option value="0" {% if not config.kicklog_channel_id %}selected{% endif %}>— Off —</option>
        {% for c in channels %}<option value="{{ c.id }}" {% if c.id == config.kicklog_channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
      </select>
      <label>Timeout-log channel</label>
      <select name="timeoutlog_channel_id">
        <option value="0" {% if not config.timeoutlog_channel_id %}selected{% endif %}>— Off —</option>
        {% for c in channels %}<option value="{{ c.id }}" {% if c.id == config.timeoutlog_channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
      </select>
      <label>Message-log channel (edited / deleted)</label>
      <select name="msglog_channel_id">
        <option value="0" {% if not config.msglog_channel_id %}selected{% endif %}>— Off —</option>
        {% for c in channels %}<option value="{{ c.id }}" {% if c.id == config.msglog_channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
      </select>
      <div class="hint">💡 Logs every edited or deleted message — including images, videos, GIFs, and links — with author and content. You can point each log at a different channel, or the same one for all four.</div>
    </div>

    <div class="card">
      <h2>🔗 Link Filter &amp; Partnership</h2>
      <div class="desc">Automatically removes links/promo from anyone without a partnership role, and offers a ticket system for it instead.</div>
      <label style="text-transform:none; letter-spacing:normal; display:flex; align-items:center; gap:8px;">
        <input type="checkbox" name="linkfilter_enabled" {% if config.linkfilter_enabled %}checked{% endif %} style="width:auto;"> Link filter active
      </label>
      <label>Partnership Recruiter role(s) (allowed to post links)</label>
      {{ role_picker('partnership_role_ids', config.partnership_role_ids) }}
      <label>Ticket category (partnership tickets are created here)</label>
      <select name="partnership_category_id">
        <option value="0" {% if not config.partnership_category_id %}selected{% endif %}>— none —</option>
        {% for cat in categories %}<option value="{{ cat.id }}" {% if cat.id == config.partnership_category_id %}selected{% endif %}>{{ cat.name }}</option>{% endfor %}
      </select>
      <label>Additional bypass role(s) (allowed to post links anyway)</label>
      {{ role_picker('linkfilter_bypass_role_ids', config.linkfilter_bypass_role_ids) }}
      <div class="hint">💡 Staff/Manager and any partnership role can always post links — the link filter still removes links from everyone else automatically. Giveaways are managed on their own <a href="/dashboard/{{ guild_id }}/giveaways" style="color:#c4b5fd;">🎉 Giveaways page</a>.</div>
    </div>

    <div class="card">
      <h2>🧽 Subtitle Remover (/removesubs)</h2>
      <div class="desc">Free, self-hosted AI removal of burned-in subtitles/text from videos (runs on the bot's own CPU).</div>
      <label style="text-transform:none; letter-spacing:normal; display:flex; align-items:center; gap:8px;">
        <input type="checkbox" name="removesubs_enabled" {% if config.removesubs_enabled %}checked{% endif %} style="width:auto;"> /removesubs active
      </label>
      <label>Default method</label>
      <select name="removesubs_default_method">
        {% for val, label in [("hybrid","Hybrid (recommended)"), ("inpaint","Fast inpaint"), ("inpaint_ns","Smooth gradients (Navier-Stokes)"), ("blur","Blur"), ("black","Black box"), ("background","Background color fill")] %}
        <option value="{{ val }}" {% if config.removesubs_default_method == val %}selected{% endif %}>{{ label }}</option>
        {% endfor %}
      </select>
      <label>Default quality</label>
      <select name="removesubs_default_quality">
        {% for val, label in [("360p","Fast (360p)"), ("480p","Balanced (480p)"), ("720p","High (720p)"), ("1080p","Highest (1080p, slow)")] %}
        <option value="{{ val }}" {% if config.removesubs_default_quality == val %}selected{% endif %}>{{ label }}</option>
        {% endfor %}
      </select>
      <label>Max video length (seconds)</label>
      <input type="number" name="removesubs_max_seconds" min="10" max="600" value="{{ config.removesubs_max_seconds }}">
      <div class="hint">💡 Runs entirely locally on CPU (no paid API) — the longer the video and the higher the quality, the longer it takes.</div>
    </div>

    <div class="card">
      <h2>🔒 Channel Access</h2>
      <div class="desc">Locks a specific user or role so they can ONLY write in the channels selected here. Multiple rows with the same target = multiple allowed channels.</div>
      <div id="accessRows">
        {% for row in access_rows %}
        <div class="tier-edit access-row">
          <div class="row4">
            <div>
              <label>Type</label>
            ...
            <div>
              ...
              <input type="text" name="restrict_target[]" value="{{ row.target_id }}" placeholder="Paste ID">
            </div>
            <div>
              <label>Allowed channel</label>
              <select name="restrict_channel[]">
                {% for c in channels %}<option value="{{ c.id }}" {% if c.id == row.channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
              </select>
            </div>
            <div style="display:flex; align-items:flex-end;">
              <button type="button" class="btn-remove-rank" onclick="this.closest('.access-row').remove()">✕ Remove</button>
            </div>
          </div>
        </div>
        {% endfor %}
      </div>
      <button type="button" class="btn" id="addAccessBtn">+ Add rule</button>
      <template id="accessRowTemplate">
        <div class="tier-edit access-row">
          <div class="row4">
            <div>
              <label>Type</label>
            ...
            <div>
              <label>User/Role ID</label>
              <input type="text" name="restrict_target[]" value="" placeholder="Paste ID">
            </div>
            <div>
              <label>Allowed channel</label>
              <select name="restrict_channel[]">
      ...
      <script>
        document.getElementById('addAccessBtn').addEventListener('click', function () {
          const tpl = document.getElementById('accessRowTemplate');
          document.getElementById('accessRows').appendChild(tpl.content.cloneNode(true));
        });
      </script>
      <div class="hint">💡 Right-click a user/role → "Copy ID" (Discord Developer Mode must be on). Leave a row empty or remove it to delete a rule.</div>
    </div>
...
<script>
  const ALL_ROLES = {{ roles_json|safe }};
  function initRolePicker(container) {
    const field = container.dataset.field;
    const selected = (container.dataset.selected || '').split(',').filter(Boolean);
    const chosen = new Set(selected);
    const chipsEl = container.querySelector('.role-chips');
    const searchEl = container.querySelector('.role-search');
    const suggEl = container.querySelector('.role-suggestions');

    function roleName(id) {
      const r = ALL_ROLES.find(function (r) { return r.id === id; });
      return r ? r.name : ('unknown role ' + id);
    }
    ...
    function render() {
      ...
      chosen.forEach(function (id) {
        ...
        const label = document.createTextNode('@' + roleName(id) + ' ');
        chip.appendChild(label);
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.setAttribute('aria-label', 'Remove');
        btn.textContent = '✕';
        btn.addEventListener('click', function () { chosen.delete(id); render(); });
    ...
    function showSuggestions(query) {
      ...
      const pool = ALL_ROLES.filter(function (r) { return !chosen.has(r.id); });
      const matches = (q ? pool.filter(function (r) { return r.name.toLowerCase().includes(q); }) : pool).slice(0, 8);
      if (!matches.length) {
        const empty = document.createElement('div');
        empty.className = 'role-suggestion-empty';
        empty.textContent = q ? 'No matching roles' : 'No more roles to add';
      ...
      matches.forEach(function (r) {
        ...
        item.className = 'role-suggestion-item';
    ...
    searchEl.addEventListener('blur', function () { setTimeout(function () { suggEl.style.display = 'none'; }, 150); });
    render();
  }
  document.querySelectorAll('.role-picker').forEach(initRolePicker);
</script>
</body></html>
"""
...
def web_settings(guild_id):
    ...
    if request.method == 'POST':
        ...
        updates = {}
        for field in ("submit_channel_id", "discussion_channel_id", "results_channel_id", "log_channel_id", "rankup_channel_id"):
            try:
        ...
        for field in ("top_edits_role_id", "unranked_role_id"):
            ...
            if val == 0:
                updates[field] = None
            elif val in role_ids:
                updates[field] = val
        for field in ("picker_role_ids", "manager_role_ids", "staff_role_ids", "linkfilter_bypass_role_ids", "partnership_role_ids"):
            picked = []
        ...
        category_ids = {c.id for c in guild.categories}
        for field in ("banlog_channel_id", "kicklog_channel_id", "timeoutlog_channel_id", "msglog_channel_id"):
            try:
                val = int(request.form.get(field, 0))
            except ValueError:
                val = 0
            updates[field] = val if val in channel_ids else None
        try:
            val = int(request.form.get("partnership_category_id", 0))
        except ValueError:
            val = 0
        updates["partnership_category_id"] = val if val in category_ids else None
        updates["linkfilter_enabled"] = request.form.get("linkfilter_enabled") == "on"

        updates["removesubs_enabled"] = request.form.get("removesubs_enabled") == "on"
        method_val = request.form.get("removesubs_default_method", "hybrid")
        updates["removesubs_default_method"] = method_val if method_val in (
            "hybrid", "inpaint", "inpaint_ns", "blur", "black", "background"
        ) else "hybrid"
        quality_val = request.form.get("removesubs_default_quality", "480p")
        updates["removesubs_default_quality"] = quality_val if quality_val in (
            "360p", "480p", "720p", "1080p"
        ) else "480p"
        try:
            val = int(request.form.get("removesubs_max_seconds", MAX_REMOVESUBS_SECONDS))
        except ValueError:
            val = MAX_REMOVESUBS_SECONDS
        updates["removesubs_max_seconds"] = max(10, min(600, val))

        restrict_types = request.form.getlist("restrict_type[]")
        restrict_targets = request.form.getlist("restrict_target[]")
        restrict_channels = request.form.getlist("restrict_channel[]")
        new_restricted = {}
        for i in range(len(restrict_types)):
            kind = (restrict_types[i] if i < len(restrict_types) else "").strip()
            raw_target = (restrict_targets[i] if i < len(restrict_targets) else "").strip()
            if kind not in ("user", "role") or not raw_target:
                continue
            digits = re.sub(r"[^0-9]", "", raw_target)
            if not digits:
                continue
            try:
                chan_val = int(restrict_channels[i]) if i < len(restrict_channels) else 0
            except (ValueError, IndexError):
                continue
            if chan_val not in channel_ids:
                continue
            key = f"{kind}:{digits}"
            chans = set(new_restricted.get(key, []))
            chans.add(chan_val)
            new_restricted[key] = list(chans)
        updates["restricted_access"] = new_restricted

        bonus_types = request.form.getlist("bonus_type[]")
        bonus_target_roles = request.form.getlist("bonus_target_role[]")
        bonus_target_users = request.form.getlist("bonus_target_user[]")
        bonus_percents = request.form.getlist("bonus_percent[]")
        new_bonuses = []
        for i in range(len(bonus_types)):
            kind = (bonus_types[i] if i < len(bonus_types) else "").strip()
            if kind == "role":
                raw_target = (bonus_target_roles[i] if i < len(bonus_target_roles) else "").strip()
            else:
                raw_target = (bonus_target_users[i] if i < len(bonus_target_users) else "").strip()
            if kind not in ("user", "role") or not raw_target:
                continue
            digits = re.sub(r"[^0-9]", "", raw_target)
            if not digits:
                continue
            try:
                percent_val = int(bonus_percents[i]) if i < len(bonus_percents) else 0
            except (ValueError, IndexError):
                percent_val = 0
            percent_val = max(0, min(1000, percent_val))
            if percent_val == 0:
                continue
            new_bonuses.append({"type": kind, "target_id": digits, "percent": percent_val})
        updates["point_bonuses"] = new_bonuses

        try:
            booster_percent_val = int(request.form.get("booster_bonus_percent", 50))
        except ValueError:
            booster_percent_val = 50
        updates["booster_bonus_percent"] = max(0, min(1000, booster_percent_val))
    ...
    # IDs as strings here, not ints: Discord snowflakes (18-19 digits) exceed JS's 53-bit
    # safe-integer range, so JSON.parse()-ing them as numbers in the role-picker's JS
    # silently rounds them to a different number. The rounded value then gets submitted
    # back on Save, fails the server-side "is this a real role id" check, and the whole
    # role picker looked like it never saved anything.
    roles_json = json.dumps([{"id": str(r["id"]), "name": r["name"]} for r in roles])
    categories = [{"id": c.id, "name": c.name} for c in guild.categories]
    access_rows = []
    for key, chans in config.get("restricted_access", {}).items():
        kind, _, raw_id = key.partition(":")
        for cid in chans:
            access_rows.append({"type": kind, "target_id": raw_id, "channel_id": cid})
    bonus_rows = [
        {"type": rule.get("type"), "target_id": rule.get("target_id"), "percent": rule.get("percent")}
        for rule in (config.get("point_bonuses") or [])
    ]
...
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Zexo Giveaways</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;600;700;800&display=swap" rel="stylesheet">
<style>
  :root { --bg:#07070d; --panel:rgba(20,20,31,0.7); --border:rgba(168,85,247,0.18); --accent:#a855f7; --muted:#9a9ab0; --danger:#f87171; }
  * { box-sizing:border-box; }
  body { margin:0; font-family:'Poppins',Arial,sans-serif; background:var(--bg); color:#f4f4f8; padding:30px 16px 70px; }
  .wrap { max-width:980px; margin:0 auto; }
  .topbar { display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:22px; }
  h1 { font-size:1.9rem; font-weight:800; margin:0; background:linear-gradient(90deg,#a855f7,#f59e0b);
       -webkit-background-clip:text; background-clip:text; color:transparent; }
  .tag { color:var(--muted); font-size:0.85rem; }
  .nav a { color:var(--muted); text-decoration:none; font-size:0.85rem; margin-left:16px; }
  .nav a:hover { color:var(--accent); }
  .flash { background:rgba(168,85,247,0.15); border:1px solid rgba(168,85,247,0.4); color:#d8b4fe;
           padding:10px 16px; border-radius:12px; margin-bottom:18px; }
  .gw-card { background:var(--panel); border:1px solid var(--border); border-radius:16px; padding:18px 22px;
             margin-bottom:16px; backdrop-filter:blur(14px); }
  .gw-card.is-ended { opacity:0.75; }
  .gw-title { font-weight:700; font-size:1.05rem; margin-bottom:4px; }
  .gw-meta { color:var(--muted); font-size:0.8rem; margin-bottom:12px; }
  .badge { display:inline-block; padding:3px 10px; border-radius:999px; font-size:0.7rem; font-weight:700; margin-left:8px; }
  .badge.active { background:rgba(34,197,94,0.18); color:#4ade80; }
  .badge.ended { background:rgba(107,114,128,0.25); color:#d1d5db; }
  .badge.cancelled { background:rgba(239,68,68,0.18); color:#f87171; }
  .actions { display:flex; gap:10px; flex-wrap:wrap; }
  button { padding:8px 16px; border-radius:9px; border:1px solid var(--accent); background:rgba(168,85,247,0.12);
    color:var(--accent); font-weight:700; font-size:0.8rem; cursor:pointer; font-family:inherit; }
...
<div class="wrap">
  <div class="topbar">
    <div><h1>🎉 Giveaways</h1><div class="tag">{{ server_name }} · manage running &amp; past giveaways</div></div>
    <div class="nav">
  ...
  {% if not rows %}
  <div class="empty">No giveaways yet. Start one with <code>/giveaway start</code> in Discord.</div>
  {% endif %}

  {% for row in rows %}
  <div class="gw-card {% if row.ended %}is-ended{% endif %}">
    <div class="gw-title">{{ row.prize }}
      {% if row.cancelled %}<span class="badge cancelled">Cancelled</span>
      {% elif row.ended %}<span class="badge ended">Ended</span>
      {% else %}<span class="badge active">Active</span>{% endif %}
    </div>
    <div class="gw-meta">
      #{{ row.channel_name }} · {{ row.entries }} entrie(s) · {{ row.winners_count }} winner(s) needed
      {% if row.ended and row.winners_labels %} · Winner(s): {{ row.winners_labels|join(", ") }}{% endif %}
      · ID: <code>{{ row.id }}</code>
    </div>
    <div class="actions">
      {% if not row.ended %}
      <form method="POST" style="display:inline;">
        <input type="hidden" name="giveaway_id" value="{{ row.id }}"><input type="hidden" name="action" value="end">
        <button type="submit">✅ End now</button>
      </form>
      <form method="POST" style="display:inline;" onsubmit="return confirm('Cancel for real? No winner will be picked.');">
        <input type="hidden" name="giveaway_id" value="{{ row.id }}"><input type="hidden" name="action" value="cancel">
        <button type="submit" class="danger">🚫 Cancel</button>
      </form>
      {% else %}
        {% if not row.cancelled %}
        <form method="POST" style="display:inline;">
          <input type="hidden" name="giveaway_id" value="{{ row.id }}"><input type="hidden" name="action" value="reroll">
          <button type="submit">🔁 Reroll</button>
        </form>
        {% endif %}
        <form method="POST" style="display:inline;" onsubmit="return confirm('Remove from the list? (does not delete the Discord message)');">
          <input type="hidden" name="giveaway_id" value="{{ row.id }}"><input type="hidden" name="action" value="delete">
          <button type="submit" class="danger">🗑️ Remove</button>
        </form>
      {% endif %}
    </div>
  </div>
...
async def _giveaway_action_reroll(guild: discord.Guild, key: str):
    """Reroll + re-render the giveaway message + announce — used by both the dashboard's
    Reroll button and (indirectly, via the same helpers) the in-Discord button/slash command."""
    gw, winners = await _reroll_giveaway_by_key(guild.id, key)
    if not gw:
        return None
    channel = guild.get_channel(gw["channel_id"])
    if channel:
        try:
            ...
            await msg.edit(embed=_build_giveaway_embed(gw, guild, ended=True))
        except discord.HTTPException:
            pass
        try:
            if winners:
                await channel.send(f"🔁 Reroll (via dashboard)! New winner(s): {', '.join(f'<@{w}>' for w in winners)} — **{gw['prize']}**")
            else:
                await channel.send("😢 No valid entrants for the reroll.")
        except discord.HTTPException:
            pass
    return gw


@app.route('/dashboard/<int:guild_id>/giveaways', methods=['GET', 'POST'])
def web_giveaways(guild_id):
    if not current_user():
        return redirect(url_for('login'))
    if not user_is_admin_of(guild_id):
        return "❌ You need Administrator permission on that server to view this page.", 403
    guild = bot.get_guild(guild_id)
    if not guild:
        return "Server not found, or Zexo isn't in that server.", 404
    ...
    if request.method == 'POST':
        ...
        if action == 'delete':
            if giveaway_id in data:
                ...
                save_giveaways(guild_id, data)
                message = "🗑️ Removed from the list."
        elif not gw:
            message = "⚠️ Giveaway not found."
        elif action == 'end' and not gw.get('ended'):
            run_on_bot_loop(_end_giveaway(guild, gw), timeout=30)
            message = "✅ Giveaway ended."
        elif action == 'cancel' and not gw.get('ended'):
            run_on_bot_loop(_end_giveaway(guild, gw, cancelled=True), timeout=30)
            message = "🚫 Giveaway cancelled."
        elif action == 'reroll' and gw.get('ended') and not gw.get('cancelled'):
            run_on_bot_loop(_giveaway_action_reroll(guild, giveaway_id), timeout=30)
            message = "🔁 Rerolled."

    data = load_giveaways(guild_id)
    rows = []
    for key, gw in sorted(data.items(), key=lambda kv: kv[1].get("created_ts", 0), reverse=True):
        ...
        rows.append({
            ...
            "cancelled": gw.get("cancelled", False),
            "winners_labels": [resolved_names.get(str(w), f"User {w}") for w in gw.get("winners", [])],
        })

    return render_template_string(
        GIVEAWAYS_HTML, guild_id=guild_id, server_name=guild.name, rows=rows, message=message
    )
...
def require_embed_builder_json(guild_id: int):
    """Same as require_admin_json, but for the embed builder specifically: Administrator OR
    Manage Server/Channels/Messages is enough — see user_can_use_embed_builder."""
    if not current_user():
        return {"ok": False, "error": "Not logged in."}, 401
    if not user_can_use_embed_builder(guild_id):
        return {"ok": False, "error": "You need Administrator, Manage Server, Manage Channels, or Manage Messages permission on that server."}, 403
    return None
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .embed-toggle { display:flex; align-items:center; gap:8px; text-transform:none; letter-spacing:normal;
    font-size:0.78rem; color: var(--muted); margin-top:8px; cursor:pointer; }
  .embed-toggle input { width:auto; margin:0; }
</style></head>
<body>
<div class="wrap">
  <div class="topbar">
    ...
    <div class="nav">
      ...
      <a href="/dashboard/{{ guild_id }}/giveaways">🎉 Giveaways</a>
      <a href="/logout">Log out</a>
    </div>
  </div>
  ...
  <div class="panel" id="moderation">
    <h2>🔇 Moderation</h2>
    <div class="desc">Time out a member for a set duration, or remove an active timeout, right from here. Logged to the timeout-log channel if one is set on the Settings page.</div>
    <div class="row3">
      <div class="field"><label>Discord User ID</label><input type="text" id="mod-user" placeholder="e.g. 123456789012345678"></div>
      <div class="field"><label>Duration</label><input type="text" id="mod-duration" placeholder="e.g. 10m, 1h, 1d (max 28d)"></div>
      <button onclick="doModeration('timeout')">Timeout</button>
    </div>
    <div class="field"><label>Reason (optional)</label><input type="text" id="mod-reason" placeholder="Reason shown in the log"></div>
    <div style="display:flex; gap:10px; margin-top:6px;">
      <button class="danger" onclick="doModeration('untimeout')">Remove timeout</button>
    </div>
    <div id="mod-result" class="hint"></div>
  </div>
  ...
  <div class="panel" id="texts">
    ...
    <div class="field">
      ...
      <label class="embed-toggle"><input type="checkbox" id="tx-winners-embed" {% if config.winner_use_embed %}checked{% endif %}> Send as embed (uncheck for plain text)</label>
    </div>
    <div class="field">
      <label>Rank-up announcement lines (one per line — a random one is picked each time; leave empty for the built-in 100-line pool). Use <code>{rank}</code> for the new rank's name.</label>
      <textarea id="tx-rankup" placeholder="⬆️ {member} just ranked up to {rank}!">{{ config.custom_rankup_messages|join('\\n') }}</textarea>
      <label class="embed-toggle"><input type="checkbox" id="tx-rankup-embed" {% if config.rankup_use_embed %}checked{% endif %}> Send as embed (uncheck for plain text)</label>
      <div class="hint">The channel rank-ups get posted to is set on the <a href="/dashboard/{{ guild_id }}/settings" style="color:#c4b5fd;">Settings page</a> ("Rank-up channel").</div>
    </div>
    <div class="field">
      ...
      <label class="embed-toggle"><input type="checkbox" id="tx-reminder-embed" {% if config.reminder_use_embed %}checked{% endif %}> Send as embed (uncheck for plain text)</label>
    </div>
    <div class="field">
...
async function doModeration(action) {
  const user_id = document.getElementById('mod-user').value.trim();
  const duration = document.getElementById('mod-duration').value.trim();
  const reason = document.getElementById('mod-reason').value.trim();
  if (!user_id) { toast('Enter a user ID first', false); return; }
  if (action === 'timeout' && !duration) { toast('Enter a duration first, e.g. 10m or 1h', false); return; }
  try {
    const data = await api(`/dashboard/${guildId}/api/moderation`, { action, user_id, duration, reason });
    document.getElementById('mod-result').textContent = data.message;
    toast(data.message);
  } catch (e) {}
}
...
async function saveTexts() {
  ...
  const custom_rankup_messages = document.getElementById('tx-rankup').value.split('\\n').map(s => s.trim()).filter(Boolean);
  ...
  const winner_use_embed = document.getElementById('tx-winners-embed').checked;
  const rankup_use_embed = document.getElementById('tx-rankup-embed').checked;
  const reminder_use_embed = document.getElementById('tx-reminder-embed').checked;
  try {
    await api(`/dashboard/${guildId}/api/messages`, {
      custom_winner_messages, custom_rankup_messages, custom_reminder_message, custom_poll_question,
      winner_use_embed, rankup_use_embed, reminder_use_embed,
    });
    toast('Texts saved ✓');
  } catch (e) {}
}
...
def web_commands(guild_id):
    ...
    badwords_html = "".join(
        f'<span class="chip" data-word="{html_lib.escape(w, quote=True)}"><span>{html_lib.escape(w)}</span>'
        f'<button onclick="removeBadword(this.parentElement.dataset.word, this.parentElement)">✕</button></span>'
        for w in words
...
@app.route('/dashboard/<int:guild_id>/api/moderation', methods=['POST'])
def api_moderation(guild_id):
    err = require_admin_json(guild_id)
    if err:
        return err
    guild = bot.get_guild(guild_id)
    ...
    try:
        ...
        return {"ok": False, "error": "Invalid user ID."}, 400
    reason = (data.get("reason") or "").strip() or f"Timeout set via dashboard by {current_user().get('username', 'staff')}"

    try:
        member = run_on_bot_loop(guild.fetch_member(user_id), timeout=20)
    except discord.NotFound:
        return {"ok": False, "error": "That user isn't a member of this server."}, 404
    except Exception:
        return {"ok": False, "error": "Couldn't look up that member."}, 500

    if action == "timeout":
        duration = (data.get("duration") or "").strip()
        seconds = _parse_duration(duration)
        if seconds is None or seconds < 10:
            return {"ok": False, "error": "Invalid duration. Examples: 10m, 1h, 1d"}, 400
        seconds = min(seconds, 28 * 24 * 3600)
        try:
            run_on_bot_loop(member.timeout(discord.utils.utcnow() + timedelta(seconds=seconds), reason=reason), timeout=20)
        except discord.Forbidden:
            return {"ok": False, "error": "Zexo is missing permission to time out that member (check role order)."}, 403
        except discord.HTTPException as ex:
            return {"ok": False, "error": f"Discord rejected that: {ex}"}, 500
        return {"ok": True, "message": f"{member.display_name} was timed out for {duration}."}

    if action == "untimeout":
        try:
            run_on_bot_loop(member.timeout(None, reason=reason), timeout=20)
        except discord.Forbidden:
            return {"ok": False, "error": "Zexo is missing permission for that."}, 403
        except discord.HTTPException as ex:
            return {"ok": False, "error": f"Discord rejected that: {ex}"}, 500
        return {"ok": True, "message": f"{member.display_name}'s timeout was removed."}
...
def api_messages(guild_id):
    ...
    if "custom_rankup_messages" in data and isinstance(data["custom_rankup_messages"], list):
        updates["custom_rankup_messages"] = [str(s)[:300] for s in data["custom_rankup_messages"]][:200]
    ...
    for field in ("winner_use_embed", "rankup_use_embed", "reminder_use_embed"):
        if field in data:
    ...
    <div>
      ...
      <div class="panel">
        ...
        <div class="field"><label>Mode</label>
          <select id="eb-mode" onchange="onModeChange()">
            <option value="embed">Rich embed (built above)</option>
            <option value="raw">Plain text — paste exactly as-is, no formatting/embed</option>
          </select>
        </div>
        <div id="eb-raw-wrap" class="field" style="display:none;">
          <label>Raw text</label>
          <textarea id="eb-raw" placeholder="Paste anything here — sent word-for-word, no embed, no changes. Longer than 2000 characters gets split into multiple messages automatically." style="min-height:180px;"></textarea>
          <div class="hint">Nothing here gets touched — no markdown cleanup, no link previews suppressed, no trimming beyond Discord's own 2000-char-per-message limit (which just splits it into more messages).</div>
        </div>
        <div class="field"><label>Channel</label>
          <select id="eb-channel" onchange="onChannelChange()">
            {% for c in channels %}<option value="{{ c.id }}" data-type="{{ c.type }}">{{ '🗂️' if c.type == 'forum' else '#' }}{{ c.name }}</option>{% endfor %}
          </select>
        </div>
        <div id="eb-forum-post-wrap" class="field" style="display:none;">
          <label>Forum post</label>
          <select id="eb-forum-post" onchange="onForumPostChange()">
            <option value="__new__">🆕 New post</option>
          </select>
          <div class="hint" id="eb-forum-post-hint">This channel is a forum — pick an existing post to send into it, or create a new one.</div>
        </div>
        <div id="eb-forum-title-wrap" class="field" style="display:none;">
          <label>Forum post title</label>
          <input type="text" id="eb-forum-title" placeholder="Title for the new forum post" maxlength="100">
          <div class="hint">This channel is a forum — sending creates a new post (thread) with this title.</div>
        </div>
        <button class="wide" id="eb-send-btn" onclick="sendEmbed()">Send</button>
        <button class="wide ghost" id="eb-cancel-edit-btn" style="display:none;" onclick="cancelEdit()">Cancel edit / start a new embed</button>
        <div id="eb-result" class="hint"></div>
      </div>

      <div class="panel">
        <h2>📌 Sent embeds</h2>
        <div class="desc">Embeds you've sent from here — reopen one to edit it in place, or delete it. Anything deleted on Discord itself drops off this list automatically.</div>
        <div id="eb-sent-list"><span class="empty">Loading…</span></div>
      </div>
    </div>
...
const guildId = "{{ guild_id }}";
const guildEmojis = [
  {% for em in emojis %}{ name: {{ em.name|tojson }}, id: {{ em.id|string|tojson }}, animated: {{ em.animated|tojson }}, url: {{ em.url|tojson }} },
  {% endfor %}
];
let fieldCount = 0;
...
async function sendEmbed() {
  if (editingMessageId) {
    ...
    const payload = { message_id: editingMessageId, embed, content: embed.content };
    try {
      const res = await fetch(`/dashboard/${guildId}/api/embed-builder/edit`, {
  ...
  const channelOpt = document.querySelector(`#eb-channel option[value="${channel_id}"]`);
  const isForum = channelOpt && channelOpt.dataset.type === 'forum';
  const mode = document.getElementById('eb-mode').value;
  const thread_id = isForum ? document.getElementById('eb-forum-post').value : null;
  const sendingToExisting = isForum && thread_id && thread_id !== '__new__';
  const forum_title = document.getElementById('eb-forum-title').value.trim();
  if (isForum && !sendingToExisting && !forum_title) { toast('This is a forum channel — give the new post a title, or pick an existing post above', false); return; }

  const payload = { channel_id, mode, thread_id, forum_title: (isForum && !sendingToExisting) ? forum_title : null };
  if (mode === 'raw') {
    const raw_text = document.getElementById('eb-raw').value;
    if (!raw_text.trim()) { toast('Paste some text first', false); return; }
  ...
  try {
    ...
    document.getElementById('eb-result').textContent = data.message;
    toast(data.message);
    if (mode !== 'raw') loadSentEmbeds();
  } catch (e) { toast('Network error', false); }
}
function clearForm() {
  document.getElementById('eb-content').value = '';
  document.getElementById('eb-author').value = '';
  document.getElementById('eb-author-icon').value = '';
  document.getElementById('eb-title').value = '';
  document.getElementById('eb-desc').value = '';
  document.getElementById('eb-color').value = '#22ffb0';
  document.getElementById('eb-timestamp').value = '0';
  document.getElementById('eb-thumb').value = '';
  document.getElementById('eb-image').value = '';
  document.getElementById('eb-footer').value = '';
  document.getElementById('eb-footer-icon').value = '';
...
function startEditMode(messageId) {
  ...
  document.getElementById('eb-send-btn').textContent = 'Save changes';
  document.getElementById('eb-cancel-edit-btn').style.display = 'block';
  document.getElementById('eb-result').textContent = '';
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
function cancelEdit() {
  editingMessageId = null;
  document.getElementById('eb-send-btn').textContent = 'Send';
  document.getElementById('eb-cancel-edit-btn').style.display = 'none';
  clearForm();
}
function editSentEmbed(messageId) {
  const item = sentEmbedsCache.find(i => i.message_id === messageId);
  if (!item) return;
  const e = item.embed || {};
  document.getElementById('eb-content').value = item.content || '';
  document.getElementById('eb-author').value = e.author_name || '';
  document.getElementById('eb-author-icon').value = e.author_icon_url || '';
  document.getElementById('eb-title').value = e.title || '';
  document.getElementById('eb-desc').value = e.description || '';
  document.getElementById('eb-color').value = e.color || '#22ffb0';
  document.getElementById('eb-timestamp').value = e.timestamp ? '1' : '0';
  document.getElementById('eb-thumb').value = e.thumbnail_url || '';
  document.getElementById('eb-image').value = e.image_url || '';
  document.getElementById('eb-footer').value = e.footer_text || '';
  document.getElementById('eb-footer-icon').value = e.footer_icon_url || '';
  document.getElementById('eb-fields').innerHTML = '';
  fieldCount = 0;
  (e.fields || []).forEach(f => addField(f.name, f.value, f.inline));
  if (!(e.fields || []).length) addField();
  renderPreview();
  startEditMode(messageId);
}
async function deleteSentEmbed(messageId) {
  if (!confirm('Delete this embed from Discord? This cannot be undone.')) return;
  try {
    const res = await fetch(`/dashboard/${guildId}/api/embed-builder/delete`, {
      method: 'POST', headers: {'Content-Type':'application/json'},
...
function renderSentEmbedsList(items) {
  const wrap = document.getElementById('eb-sent-list');
  if (!items.length) { wrap.innerHTML = '<span class="empty">Nothing sent from here yet.</span>'; return; }
  wrap.innerHTML = items.map(i => {
    const e = i.embed || {};
    const label = e.title || (e.description ? e.description.slice(0, 60) : '(no title)');
    const where = i.is_forum ? `🗂️ ${i.channel_name}` : `#${i.channel_name}`;
    return `<div class="fieldrow" style="grid-template-columns:1fr auto auto;">
      <div><strong>${escapeHtml(label)}</strong><div class="hint" style="margin-top:2px;">${escapeHtml(where)}</div></div>
      <button class="ghost" onclick="editSentEmbed('${i.message_id}')">✏️ Edit</button>
      <button class="ghost" onclick="deleteSentEmbed('${i.message_id}')">🗑️</button>
    </div>`;
  }).join('');
}
async function loadSentEmbeds() {
  try {
    const res = await fetch(`/dashboard/${guildId}/api/embed-builder/sent`);
    const data = await res.json();
    if (res.ok && data.ok) {
      sentEmbedsCache = data.items;
      renderSentEmbedsList(sentEmbedsCache);
    } else {
      document.getElementById('eb-sent-list').innerHTML = '<span class="empty">Could not load sent embeds.</span>';
    }
  } catch (e) {
    document.getElementById('eb-sent-list').innerHTML = '<span class="empty">Could not load sent embeds.</span>';
  }
...
function onModeChange() {
  const raw = document.getElementById('eb-mode').value === 'raw';
  document.getElementById('eb-raw-wrap').style.display = raw ? 'block' : 'none';
}
async function onChannelChange() {
  const channel_id = document.getElementById('eb-channel').value;
  const opt = document.querySelector(`#eb-channel option[value="${channel_id}"]`);
  const isForum = opt && opt.dataset.type === 'forum';
  document.getElementById('eb-forum-post-wrap').style.display = isForum ? 'block' : 'none';
  document.getElementById('eb-forum-title-wrap').style.display = isForum ? 'block' : 'none';
  if (!isForum) return;

  const postSelect = document.getElementById('eb-forum-post');
  postSelect.innerHTML = '<option value="__new__">🆕 New post</option>';
  const hint = document.getElementById('eb-forum-post-hint');
  hint.textContent = 'Loading existing posts…';
  try {
    const res = await fetch(`/dashboard/${guildId}/api/embed-builder/forum-posts?channel_id=${channel_id}`);
    const data = await res.json();
    if (res.ok && data.ok) {
      data.posts.forEach(p => {
        const o = document.createElement('option');
        o.value = p.id;
        o.textContent = `${p.archived ? '📦 ' : '💬 '}${p.name}${p.locked ? ' (locked)' : ''}`;
        postSelect.appendChild(o);
      });
      hint.textContent = 'Pick an existing post to send into it, or leave "New post" to create one.';
    } else {
      hint.textContent = 'Could not load existing posts — you can still create a new one.';
    }
  } catch (e) {
    hint.textContent = 'Could not load existing posts — you can still create a new one.';
  }
...
function onForumPostChange() {
  const isForum = document.getElementById('eb-forum-post-wrap').style.display === 'block';
  const thread_id = document.getElementById('eb-forum-post').value;
  const sendingToExisting = isForum && thread_id && thread_id !== '__new__';
  document.getElementById('eb-forum-title-wrap').style.display = (isForum && !sendingToExisting) ? 'block' : 'none';
}
onModeChange();
onChannelChange();
addField();
...
def web_embed_builder(guild_id):
    if not current_user():
        return redirect(url_for('login'))
    if not user_can_use_embed_builder(guild_id):
        return "❌ You need Administrator, Manage Server, Manage Channels, or Manage Messages permission on that server to use the embed builder.", 403
    ...
    channels = (
        [{"id": c.id, "name": c.name, "type": "text"} for c in guild.text_channels]
        + [{"id": c.id, "name": c.name, "type": "forum"} for c in guild.forums]
    )
    emojis = [
        {
            "name": e.name,
...
def chunk_text(text: str, limit: int = 2000):
    """Split text into <=limit character chunks for Discord's per-message cap, breaking on
    line boundaries where possible instead of mid-word. Never rewrites the content itself."""
    text = text.replace("\r\n", "\n")
    if len(text) <= limit:
        return [text]
    chunks = []
    ...
    for line in text.split("\n"):
        # A single line longer than the whole limit has to be hard-split — still verbatim,
        # just spread across messages.
...
async def _fetch_forum_posts(channel: discord.ForumChannel, limit: int = 100):
    """Active + archived posts (threads) in a forum channel, newest first. Used to populate the
    'existing post' dropdown in the embed builder so an embed can be sent into a post that's
    already there, instead of only ever creating a brand new one."""
    posts = {}
    for t in channel.threads:  # active threads, already cached
        posts[t.id] = t
    try:
        ...
        pass
    ordered = sorted(posts.values(), key=lambda t: t.created_at or discord.utils.utcnow(), reverse=True)
    # id sent as a STRING, not a JSON number — Discord snowflakes are 18-19 digits, well past
    # JavaScript's safe integer range (~16 digits), so a raw number silently gets rounded to a
    # different ID once it reaches the browser. That corrupted ID was actually what broke the
    # "you need admin perms" check on Send — see the guildId fix below for the same root cause.
    return [{"id": str(t.id), "name": t.name, "archived": t.archived, "locked": t.locked} for t in ordered]


@app.route('/dashboard/<int:guild_id>/api/embed-builder/forum-posts')
def api_embed_builder_forum_posts(guild_id):
    err = require_embed_builder_json(guild_id)
    if err:
        return err
    guild = bot.get_guild(guild_id)
    if not guild:
    ...
    if not channel or not isinstance(channel, discord.ForumChannel):
        return {"ok": False, "error": "That's not a forum channel."}, 400

    try:
        posts = run_on_bot_loop(_fetch_forum_posts(channel), timeout=30)
    except discord.Forbidden:
        return {"ok": False, "error": "Zexo doesn't have permission to read that forum's posts."}, 403
    except Exception as ex:
        print(f"⚠️ Embed builder forum-posts fetch failed: {ex}")
        return {"ok": False, "error": "Failed to load that forum's posts."}, 500
...
def api_embed_builder_send(guild_id):
    ...
    forum_title = (data.get("forum_title") or "").strip()[:100]

    # thread_id set (and not the "__new__" sentinel) -> send into an EXISTING forum post
    # instead of creating a new one.
    raw_thread_id = data.get("thread_id")
    thread_id = None
    if is_forum and raw_thread_id and raw_thread_id != "__new__":
        try:
            thread_id = int(raw_thread_id)
        except (TypeError, ValueError):
            return {"ok": False, "error": "Invalid post selected."}, 400
    ...
    if is_forum and thread_id:
        ...
        if thread is None:
            try:
                thread = run_on_bot_loop(guild.fetch_channel(thread_id), timeout=30)
            except (discord.NotFound, discord.Forbidden):
                thread = None
        if thread is None or thread.parent_id != channel.id:
            return {"ok": False, "error": "That post couldn't be found anymore — refresh the post list and try again."}, 404
        if thread.locked:
            return {"ok": False, "error": "That post is locked — pick another one."}, 400
        target = thread
    elif is_forum and not forum_title:
        return {"ok": False, "error": "Give the forum post a title, or pick an existing post to send into instead."}, 400

    mode = data.get("mode") or "embed"
    sent_message = None
    sent_target_id = None
    embed_payload = None
    embed_content = None
    ...
    try:
        if mode == "raw":
            ...
            if not raw_text.strip():
                return {"ok": False, "error": "Nothing to send — paste some text first."}, 400
            parts = chunk_text(raw_text, 2000)
            if is_forum and not thread_id:
                thread, first_msg = run_on_bot_loop(
                    channel.create_thread(name=forum_title, content=parts[0]), timeout=30
                )
                for part in parts[1:]:
                    run_on_bot_loop(thread.send(content=part), timeout=30)
            else:
                for part in parts:
                    run_on_bot_loop(target.send(content=part), timeout=30)
        else:
            e = data.get("embed") or {}
            ...
            if is_forum and not thread_id:
                new_thread, sent_message = run_on_bot_loop(
                    channel.create_thread(name=forum_title, content=content, embed=embed), timeout=30
                )
                sent_target_id = new_thread.id
            else:
                sent_message = run_on_bot_loop(target.send(content=content, embed=embed), timeout=30)
                sent_target_id = target.id
            # Kept as-received (not the discord.Embed object) so it can be dropped straight back
            # into the dashboard's form fields the next time someone reopens it to edit.
            embed_payload = e
            embed_content = content
    except discord.Forbidden:
        return {"ok": False, "error": "Zexo doesn't have permission to post there."}, 403
    except discord.HTTPException as ex:
        msg = str(ex)
        if is_forum and "tag" in msg.lower():
            return {"ok": False, "error": "That forum requires a tag to be picked — remove the requirement in Discord's forum settings, or post manually and pick a tag."}, 400
    ...
    if sent_message is not None:
        record_sent_embed(
            guild_id, sent_message.id, sent_target_id, is_forum,
            embed_payload, embed_content,
            forum_title=(forum_title if (is_forum and not thread_id) else None),
        )

    if is_forum and thread_id:
        where = f"the existing post \"{target.name}\" in {channel.name}"
    elif is_forum:
        where = f"a new post in {channel.name}"
    else:
        where = f"#{channel.name}"
    return {"ok": True, "message": f"Sent to {where} ✅"}


async def _resolve_target_channel(guild: discord.Guild, target_id: int):
    """Text channel or thread — whichever the embed was originally sent into."""
    target = guild.get_channel(target_id) or guild.get_thread(target_id)
    if target is None:
        try:
            target = await guild.fetch_channel(target_id)
        except (discord.NotFound, discord.Forbidden):
            target = None
    return target


@app.route('/dashboard/<int:guild_id>/api/embed-builder/sent')
def api_embed_builder_sent(guild_id):
    """Lists every embed the dashboard has sent for this server that's still editable, newest
    first, along with enough channel/thread context for the UI to show where each one lives."""
    err = require_embed_builder_json(guild_id)
    if err:
        return err
    guild = bot.get_guild(guild_id)
    ...
    stored = load_sent_embeds(guild_id)
    items = []
    for message_id, entry in stored.items():
        target = guild.get_channel(entry.get("target_id")) or guild.get_thread(entry.get("target_id"))
        channel_name = getattr(target, "name", None) or "unknown channel"
        items.append({
            "message_id": message_id,
            "target_id": entry.get("target_id"),
            "is_forum": bool(entry.get("is_forum")),
            "channel_name": channel_name,
            "embed": entry.get("embed") or {},
...
@app.route('/dashboard/<int:guild_id>/api/embed-builder/edit', methods=['POST'])
def api_embed_builder_edit(guild_id):
    """Re-builds the embed from the submitted fields and edits the original Discord message
    in place, then updates what's stored so the next edit starts from the latest version."""
    err = require_embed_builder_json(guild_id)
    if err:
        return err
    guild = bot.get_guild(guild_id)
    ...
    try:
        ...
        return {"ok": False, "error": "Invalid message."}, 400

    stored = load_sent_embeds(guild_id)
    entry = stored.get(str(message_id))
    if not entry:
        return {"ok": False, "error": "That embed isn't in the saved list anymore — it may already have been deleted."}, 404

    target = run_on_bot_loop(_resolve_target_channel(guild, entry.get("target_id")), timeout=30)
    if target is None:
        forget_sent_embed(guild_id, message_id)
        return {"ok": False, "error": "The channel or post that embed was sent to no longer exists — removed it from the list."}, 404

    try:
        message = run_on_bot_loop(target.fetch_message(message_id), timeout=30)
    except discord.NotFound:
        forget_sent_embed(guild_id, message_id)
        return {"ok": False, "error": "That message was already deleted on Discord — removed it from the list."}, 404
    except discord.Forbidden:
        return {"ok": False, "error": "Zexo doesn't have permission to read messages there anymore."}, 403
    ...
    try:
        run_on_bot_loop(message.edit(content=content, embed=embed), timeout=30)
    except discord.Forbidden:
        return {"ok": False, "error": "Zexo doesn't have permission to edit that message."}, 403
    except discord.HTTPException as ex:
        print(f"⚠️ Embed builder edit failed: {ex}")
        return {"ok": False, "error": "Failed to save — check the URLs (image/thumbnail/icon) are valid direct links."}, 500

    record_sent_embed(
        guild_id, message_id, entry.get("target_id"), bool(entry.get("is_forum")),
        e, content, forum_title=entry.get("forum_title"),
    )
    return {"ok": True, "message": "Embed updated ✅"}


@app.route('/dashboard/<int:guild_id>/api/embed-builder/delete', methods=['POST'])
def api_embed_builder_delete(guild_id):
    """Deletes the actual Discord message and drops it from the saved list. If the message was
    already gone on Discord's side, it still gets removed from the list rather than erroring."""
    err = require_embed_builder_json(guild_id)
    if err:
        return err
    guild = bot.get_guild(guild_id)
    ...
    entry = stored.get(str(message_id))
    if not entry:
        return {"ok": False, "error": "That embed isn't in the saved list anymore."}, 404

    target = run_on_bot_loop(_resolve_target_channel(guild, entry.get("target_id")), timeout=30)
    if target is not None:
        try:
            message = run_on_bot_loop(target.fetch_message(message_id), timeout=30)
            run_on_bot_loop(message.delete(), timeout=30)
        except discord.NotFound:
            pass
        except discord.Forbidden:
            return {"ok": False, "error": "Zexo doesn't have permission to delete that message."}, 403

    forget_sent_embed(guild_id, message_id)
    return {"ok": True, "message": "Embed deleted ✅"}


def run_web():
    port = int(os.environ.get("PORT", 10000))
    # threaded=True is critical here: without it, Flask's dev server handles ONE request at a
    # time. Any request that briefly blocks (the 30s dashboard auto-refresh, a Discord OAuth
    # guilds lookup, or an /api/trigger call that waits on run_on_bot_loop) then queues up every
    # other page load behind it — which is exactly what "Control Room -> Dashboard hangs/loads
    # forever" looks like from the browser. threaded=True lets requests run concurrently instead.
