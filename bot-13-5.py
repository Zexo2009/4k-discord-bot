...
async def on_message(message: discord.Message):
    ...
    if message.guild and not message.author.bot:
        ...
        if submit_channel and picker_role and message.channel.id == submit_channel.id:
            ...
            # 1) Picker-ping reminder goes first — pinging is required for the edit to count
            # at all, so that's the more urgent thing to tell someone about.
            if sources and not pinged:
                ...
                if message.author.id not in reminded:
                    ...
                    reminder_embed = discord.Embed(
                        title="📌 Don't forget to ping!",
                        description=reminder_text,
                        color=discord.Color.from_rgb(245, 158, 11),  # amber
                    )
                    try:
                        await message.reply(embed=reminder_embed, mention_author=False)
                    except discord.HTTPException:
                        pass

            # 2) AI-rating hint second — and only once this person has actually pinged the
            # Picker role successfully at least once. Before that, the ping reminder above is
            # the only thing that matters; showing the AI hint too early just dilutes it.
            ...
            if sources and message.author.id in load_has_pinged_users(message.guild.id):
                ...
                if message.author.id not in hint_key_users:
                    ...
                    hint_embed = discord.Embed(
                        title="✨ Want AI feedback on this edit?",
                        description="Run `.airating` for a score, software guess, and tips.",
                        color=discord.Color.from_rgb(34, 255, 176),  # brand teal/green
                    )
                    try:
...
def _yt_dlp_download(url: str, out_path: str, max_bytes: int):
    ydl_opts = {
        ...
        # you're not a bot" wall and TikTok's own bot-checks when using yt-dlp's default web
        # client. Forcing the Android/iOS player clients for YouTube and a real browser UA for
        # everything else sidesteps most of that without needing cookies.
        "extractor_args": {
