...
def default_guild_config() -> dict:
    ...
    return {
        ...
        "modlog_channel_id": None,           # ban/kick/timeout logs go here if set
        ...
        "partnership_role_id": None,         # role required to post links ("Partnership Recruiter")
    ...
    if log_channel:
        try:
            await log_channel.send(embed=discord.Embed(
                ...
                description=f"**{len(payload.message_ids)}** Nachrichten wurden in <#{payload.channel_id}> gelöscht (z.B. durch Purge).",
...
async def on_message_edit(before: discord.Message, after: discord.Message):
    ...
    if not log_channel:
        return
    old_content = before.content or "*(kein Text)*"
    new_content = after.content or "*(kein Text)*"
    if len(old_content) > 1000:
        old_content = old_content[:1000] + "…"
    if len(new_content) > 1000:
        new_content = new_content[:1000] + "…"
    embed = discord.Embed(title="✏️ Nachricht bearbeitet", color=discord.Color.from_rgb(234, 179, 8), timestamp=datetime.now(timezone.utc))
    ...
    embed.add_field(name="🔗 Springe hin", value=f"[Klick hier]({after.jump_url})", inline=True)
    ...
    if links_before or links_after:
        if links_before == links_after:
            embed.add_field(name="🔗 Erkannte Links", value=(links_before or links_after)[:1024], inline=False)
        else:
            if links_before:
                embed.add_field(name="🔗 Links (vorher)", value=links_before[:1024], inline=False)
            if links_after:
                embed.add_field(name="🔗 Links (nachher)", value=links_after[:1024], inline=False)
        ...
        if restricted and not message.author.guild_permissions.administrator:
            ...
            if matched and message.channel.id not in allowed_channels:
                try:
                    ...
                    pass
                try:
                    allowed_label = ", ".join(f"<#{c}>" for c in allowed_channels) or "keinen Kanälen"
                    await message.channel.send(
                        f"🔒 {message.author.mention}, du darfst nur in {allowed_label} schreiben.",
                        delete_after=8,
                    )
                except discord.HTTPException:
                    pass
    ...
    if message.guild and not message.author.bot and message.content:
        ...
        if not is_ticket_channel and _guild_link_filter_check(message, config):
            try:
                ...
                pass
            try:
                if config.get("partnership_role_id"):
                    desc = (
                        f"{message.author.mention}, Links/Promo sind hier nur mit der "
                        f"<@&{config['partnership_role_id']}> Rolle erlaubt.\n\n"
                        f"Möchtest du für dein Projekt werben? Eröffne unten ein Partnership-Ticket!"
                    )
                else:
                    desc = f"{message.author.mention}, Links/Promo sind hier nicht erlaubt."
                link_embed = discord.Embed(title="🔗 Link entfernt", description=desc, color=PARTNERSHIP_COLOR)
                link_embed.set_footer(text="Zexo • Partnership-Schutz")
...
HELP_CATEGORIES = {
    ...
    "moderation": {
        ...
        "description": (
            ...
            "**`/giveaway start <prize> <duration> [winners] [channel] [requirement_role] "
            "[bonus_role] [bonus_entries] [attachment] [ping_here]`**\n"
            "Starts a giveaway with a button to enter. Attach an image or video — it shows up "
            ...
            "Ban, kick, and timeout get logged here automatically (moderator + reason).\n\n"
            ...
            "**`/modlog off [mod|messages]`**\n"
            "Turns one of the two logs back off.\n\n"
...
async def _log_deleted_message(guild_id: int, channel_id: int, cached_message):
    ...
    embed = discord.Embed(title="🗑️ Nachricht gelöscht", color=discord.Color.from_rgb(239, 68, 68), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Kanal", value=f"<#{channel_id}>", inline=True)
    if cached_message is not None:
        embed.add_field(name="Autor", value=f"{cached_message.author.mention} (`{cached_message.author.id}`)", inline=True)
        content = cached_message.content or "*(kein Text — nur Anhang/Embed)*"
        if len(content) > 1000:
        ...
        if cached_message.attachments:
            ...
            embed.add_field(name="Anhänge (Bild/Video/GIF)", value=files_list, inline=False)
        link_field = _extract_link_field(cached_message.content)
        if link_field:
            embed.add_field(name="🔗 Erkannte Links", value=link_field[:1024], inline=False)
        avatar = getattr(cached_message.author, "display_avatar", None)
        if avatar:
            embed.set_thumbnail(url=avatar.url)
    else:
        embed.description = "*(Nachrichteninhalt war nicht im Cache — keine weiteren Details verfügbar)*"
    try:
        await log_channel.send(embed=embed)
    except discord.HTTPException:
        pass
...
def _guild_link_filter_check(message: discord.Message, config: dict) -> bool:
    ...
    if _member_has_any_role(member, config.get("manager_role_ids", []), MANAGER_ROLE_NAME):
        return False
    partnership_role_id = config.get("partnership_role_id")
    if partnership_role_id and _member_has_role(member, partnership_role_id, "Partnership Recruiter"):
        return False
...
def _build_giveaway_embed(gw: dict, guild: discord.Guild, ended: bool = False) -> discord.Embed:
    ...
    if ended:
        ...
        if gw.get("cancelled"):
            desc = f"**Preis:** {gw['prize']}\n\n🚫 **Abgebrochen — kein Gewinner.**"
        elif winners:
            desc = f"**Preis:** {gw['prize']}\n\n🏆 **Gewinner:** " + ", ".join(f"<@{w}>" for w in winners)
        else:
            desc = f"**Preis:** {gw['prize']}\n\n😢 **Keine gültigen Teilnahmen — kein Gewinner.**"
    ...
    embed.add_field(name="🏆 Gewinner", value=str(gw["winners_count"]), inline=True)
    embed.add_field(name="👥 Teilnahmen", value=str(entries_count), inline=True)
    if not ended:
        embed.add_field(name="⏰ Endet", value=f"<t:{int(gw['end_ts'])}:R>", inline=True)
    if gw.get("requirement_role_id"):
        embed.add_field(name="🔒 Voraussetzung", value=f"<@&{gw['requirement_role_id']}>", inline=True)
    if gw.get("bonus_role_id"):
        embed.add_field(
            name="✨ Bonus-Lose",
            value=f"<@&{gw['bonus_role_id']}> bekommt +{gw.get('bonus_entries', 1)} Lose",
            inline=True,
        )
...
class GiveawayView(discord.ui.View):
    ...
    async def _on_enter(self, interaction: discord.Interaction):
        ...
        if not gw or gw.get("ended") or gw.get("cancelled"):
            await interaction.response.send_message("⚠️ Dieses Giveaway ist nicht mehr aktiv.", ephemeral=True)
            return
        member = interaction.user
        if gw.get("requirement_role_id") and not any(r.id == gw["requirement_role_id"] for r in member.roles):
            await interaction.response.send_message(
                f"❌ Du brauchst die Rolle <@&{gw['requirement_role_id']}>, um teilzunehmen.", ephemeral=True
            )
            return
        ...
        if uid in entries:
            ...
            await interaction.response.send_message("↩️ Du hast deine Teilnahme zurückgezogen.", ephemeral=True)
        else:
            entries.append(uid)
            weight = 1
            ...
            save_giveaways(guild_id, data)
            await interaction.response.send_message(f"✅ Du nimmst jetzt teil! (Lose: {weight})", ephemeral=True)
        try:
...
class GiveawayEndedView(discord.ui.View):
    ...
    async def _on_reroll(self, interaction: discord.Interaction):
        if not has_mod_permission(interaction.user):
            await interaction.response.send_message("❌ Nur Staff/Manager können rerollen.", ephemeral=True)
            return
        guild_id_str, msg_id_str = self.giveaway_key.split(":")
        gw, winners = await _reroll_giveaway_by_key(int(guild_id_str), msg_id_str)
        if not gw:
            await interaction.response.send_message("⚠️ Giveaway nicht gefunden.", ephemeral=True)
            return
        await interaction.response.edit_message(embed=_build_giveaway_embed(gw, interaction.guild, ended=True))
        if winners:
            await interaction.channel.send(
                f"🔁 Reroll! Neue(r) Gewinner: {', '.join(f'<@{w}>' for w in winners)} — **{gw['prize']}**"
            )
        else:
            await interaction.channel.send("😢 Keine gültigen Teilnehmer für den Reroll.")
...
async def _end_giveaway(guild: discord.Guild, gw: dict, cancelled: bool = False):
    ...
    try:
        if cancelled:
            await channel.send("🚫 Das Giveaway wurde abgebrochen — kein Gewinner.")
        elif winners:
            await channel.send(
                f"🎉 Glückwunsch {', '.join(f'<@{w}>' for w in winners)}! Ihr habt **{gw['prize']}** gewonnen!"
            )
        else:
            await channel.send("😢 Niemand hat gültig teilgenommen — kein Gewinner.")
    except discord.HTTPException:
        pass
...
@giveaway_group.command(name="start", description="Starte ein neues Giveaway")
@app_commands.describe(
    prize="Was wird verlost?",
    duration="Dauer, z.B. 10m, 1h, 2h30m, 1d",
    winners="Anzahl Gewinner (Standard: 1)",
    channel="Kanal (Standard: aktueller Kanal)",
    requirement_role="Rolle die zur Teilnahme benötigt wird (optional)",
    bonus_role="Rolle die Bonus-Lose bekommt (optional)",
    bonus_entries="Wie viele Extra-Lose die Bonus-Rolle bekommt (Standard: 1)",
    attachment="Bild oder Video das im Giveaway gezeigt wird (optional)",
    ping_here="@here pingen wenn das Giveaway startet? (Standard: nein)",
)
async def giveaway_start(
    ...
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Nur Staff/Manager können Giveaways starten.", ephemeral=True)
        return
    seconds = _parse_duration(duration)
    if seconds is None or seconds < 10:
        await interaction.response.send_message(
            "❌ Ungültige Dauer. Beispiele: `10m`, `1h`, `2h30m`, `1d`", ephemeral=True
        )
        return
    if winners < 1:
        await interaction.response.send_message("❌ Mindestens 1 Gewinner.", ephemeral=True)
        return
    target_channel = channel or interaction.channel
    if not target_channel.permissions_for(interaction.guild.me).send_messages:
        await interaction.response.send_message(f"❌ Ich kann in {target_channel.mention} nicht schreiben.", ephemeral=True)
        return
    ...
    if ping_here:
        send_kwargs["content"] = "@here 🎉 Neues Giveaway!"
    ...
    await interaction.followup.send(f"✅ Giveaway gestartet in {target_channel.mention}! ID: `{msg.id}`", ephemeral=True)


@giveaway_group.command(name="end", description="Beende ein Giveaway sofort und lose Gewinner aus")
@app_commands.describe(giveaway_id="Die Nachrichten-ID des Giveaways (siehe /giveaway list)")
async def giveaway_end(interaction: discord.Interaction, giveaway_id: str):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Nur Staff/Manager.", ephemeral=True)
        return
    data = load_giveaways(interaction.guild.id)
    gw = data.get(giveaway_id)
    if not gw or gw.get("ended"):
        await interaction.response.send_message("⚠️ Giveaway nicht gefunden oder bereits beendet.", ephemeral=True)
        return
    await interaction.response.send_message("✅ Giveaway wird beendet...", ephemeral=True)
    await _end_giveaway(interaction.guild, gw)


@giveaway_group.command(name="reroll", description="Neue Gewinner für ein beendetes Giveaway auslosen")
@app_commands.describe(giveaway_id="Die Nachrichten-ID des Giveaways")
async def giveaway_reroll(interaction: discord.Interaction, giveaway_id: str):
    ...
    if not gw:
        ...
        return
    await interaction.response.send_message(
        f"🔁 Neue Gewinner: {', '.join(f'<@{w}>' for w in winners) if winners else 'Keine gültigen Teilnehmer.'}"
    )
    channel = interaction.guild.get_channel(gw["channel_id"])
    if channel:
        try:
...
@giveaway_group.command(name="cancel", description="Giveaway abbrechen (kein Gewinner wird gezogen)")
@app_commands.describe(giveaway_id="Die Nachrichten-ID des Giveaways")
async def giveaway_cancel(interaction: discord.Interaction, giveaway_id: str):
    ...
    if not gw or gw.get("ended"):
        await interaction.response.send_message("⚠️ Giveaway nicht gefunden oder bereits beendet.", ephemeral=True)
        return
    await interaction.response.send_message("🚫 Giveaway wird abgebrochen...", ephemeral=True)
    await _end_giveaway(interaction.guild, gw, cancelled=True)


@giveaway_group.command(name="list", description="Zeige alle aktiven Giveaways")
async def giveaway_list(interaction: discord.Interaction):
    ...
    if not active:
        await interaction.response.send_message("Es laufen gerade keine Giveaways.", ephemeral=True)
        return
    embed = discord.Embed(title="🎉 Aktive Giveaways", color=GIVEAWAY_COLOR)
    for key, gw in active.items():
        embed.add_field(
            name=f"{gw['prize']} (ID: {key})",
            value=f"<#{gw['channel_id']}> • endet <t:{int(gw['end_ts'])}:R> • {len(gw.get('entries', []))} Teilnahmen",
            inline=False,
        )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
class TicketCloseView(discord.ui.View):
    ...
    @discord.ui.button(label="🔒 Ticket schließen", style=discord.ButtonStyle.danger, custom_id="partnership_close_ticket")
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not (is_opener or has_mod_permission(interaction.user)):
            await interaction.response.send_message("❌ Nur der Ersteller oder Staff kann dieses Ticket schließen.", ephemeral=True)
            return
        ...
        try:
            await interaction.channel.delete(reason=f"Partnership-Ticket geschlossen von {interaction.user}")
        except discord.HTTPException:
            pass
...
class PartnershipTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="📩 Partnership-Ticket öffnen", style=discord.ButtonStyle.primary, custom_id="partnership_open_ticket")
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not category:
            await interaction.response.send_message(
                "⚠️ Das Partnership-System wurde noch nicht eingerichtet (Staff: `/partnership setup`).", ephemeral=True
            )
            return
        tickets = load_tickets(interaction.guild.id)
        for ch_id, t in tickets.items():
            if t.get("user_id") == interaction.user.id and not t.get("closed"):
                await interaction.response.send_message(f"⚠️ Du hast bereits ein offenes Ticket: <#{ch_id}>", ephemeral=True)
                return
        ...
        try:
            channel = await interaction.guild.create_text_channel(
                ...
                reason=f"Partnership-Ticket für {interaction.user}",
            )
        except discord.Forbidden:
            await interaction.followup.send("❌ Mir fehlt die Berechtigung, einen Kanal zu erstellen.", ephemeral=True)
            return
        ...
        welcome = discord.Embed(
            ...
            description=(
                f"Hey {interaction.user.mention}! Hier kannst du dein Projekt/deinen Server vorstellen "
                f"und Links posten — nur du und das Team können hier schreiben.\n\n"
                f"Ein Teammitglied meldet sich bald bei dir."
            ),
...
@partnership_group.command(name="setup", description="Partnership-System einrichten (Rolle + Ticket-Kategorie)")
@app_commands.describe(
    role="Die Rolle die Promo/Links posten darf (z.B. Partnership Recruiter)",
    category="Kategorie in der Ticket-Kanäle erstellt werden",
)
async def partnership_setup(interaction: discord.Interaction, role: discord.Role, category: discord.CategoryChannel):
    ...
    save_guild_config(interaction.guild.id, {
        "partnership_role_id": role.id,
        "partnership_category_id": category.id,
        "linkfilter_enabled": True,
    })
    await interaction.response.send_message(
        f"✅ Partnership-System eingerichtet!\n• Rolle: {role.mention}\n• Ticket-Kategorie: {category.mention}\n"
        f"• Link-Filter wurde automatisch aktiviert.",
        ephemeral=True,
    )
...
@linkfilter_group.command(name="toggle", description="Link-Filter an/ausschalten")
...
async def linkfilter_toggle(interaction: discord.Interaction, enabled: bool):
    ...
    await interaction.response.send_message(f"✅ Link-Filter ist jetzt **{'AN' if enabled else 'AUS'}**.", ephemeral=True)


@linkfilter_group.command(name="bypass", description="Rolle hinzufügen, die trotzdem Links posten darf")
...
async def linkfilter_bypass(interaction: discord.Interaction, role: discord.Role):
    ...
    await interaction.response.send_message(f"✅ {role.mention} darf jetzt Links posten.", ephemeral=True)
...
channelaccess_group = app_commands.Group(name="channelaccess", description="Beschränke User/Rollen auf bestimmte Kanäle")


@channelaccess_group.command(name="add", description="Erlaube einem User/einer Rolle nur bestimmte Kanäle")
@app_commands.describe(channel="Kanal der erlaubt werden soll", member="User (optional)", role="Rolle (optional)")
async def channelaccess_add(
    ...
    if not member and not role:
        await interaction.response.send_message("❌ Gib entweder einen User oder eine Rolle an.", ephemeral=True)
        return
    config = load_guild_config(interaction.guild.id)
    ...
    await interaction.response.send_message(
        f"✅ {target_label} kann jetzt NUR NOCH in den freigegebenen Kanälen schreiben (zuletzt hinzugefügt: {channel.mention}).",
        ephemeral=True,
    )


@channelaccess_group.command(name="remove", description="Entferne einen Kanal aus der Erlaubnisliste")
@app_commands.describe(channel="Kanal", member="User (optional)", role="Rolle (optional)")
...
@channelaccess_group.command(name="clear", description="Entferne die komplette Beschränkung für ein Ziel")
@app_commands.describe(member="User (optional)", role="Rolle (optional)")
async def channelaccess_clear(interaction: discord.Interaction, member: discord.Member = None, role: discord.Role = None):
    ...
    restricted.pop(key, None)
    save_guild_config(interaction.guild.id, {"restricted_access": restricted})
    await interaction.response.send_message("✅ Beschränkung entfernt.", ephemeral=True)


@channelaccess_group.command(name="list", description="Zeige alle Kanal-Beschränkungen")
async def channelaccess_list(interaction: discord.Interaction):
    config = load_guild_config(interaction.guild.id)
    restricted = config.get("restricted_access", {})
    if not restricted:
        await interaction.response.send_message("Keine Beschränkungen aktiv.", ephemeral=True)
        return
    lines = []
    ...
    embed = discord.Embed(title="🔒 Kanal-Beschränkungen", description="\n".join(lines), color=discord.Color.from_rgb(99, 102, 241))
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
modlog_group = app_commands.Group(name="modlog", description="Mod-Log Einstellungen (Ban/Kick/Timeout)")


@modlog_group.command(name="channel", description="Setze den Kanal für Ban/Kick/Timeout-Logs")
...
async def modlog_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Nur Staff/Manager.", ephemeral=True)
        return
    save_guild_config(interaction.guild.id, {"modlog_channel_id": channel.id})
    await interaction.response.send_message(f"✅ Mod-Log-Kanal gesetzt: {channel.mention}", ephemeral=True)


@modlog_group.command(name="messages", description="Setze den Kanal für bearbeitete/gelöschte Nachrichten")
@app_commands.describe(channel="Log-Kanal")
async def modlog_messages(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await interaction.response.send_message(f"✅ Nachrichten-Log-Kanal gesetzt: {channel.mention}", ephemeral=True)


@modlog_group.command(name="off", description="Mod-Log deaktivieren")
@app_commands.describe(which="Welches Log ausschalten (Standard: Ban/Kick/Timeout)")
@app_commands.choices(which=[
    app_commands.Choice(name="Ban/Kick/Timeout", value="mod"),
    app_commands.Choice(name="Nachrichten (bearbeitet/gelöscht)", value="messages"),
])
async def modlog_off(interaction: discord.Interaction, which: app_commands.Choice[str] = None):
    if not has_mod_permission(interaction.user):
        await interaction.response.send_message("❌ Nur Staff/Manager.", ephemeral=True)
        return
    if which and which.value == "messages":
        save_guild_config(interaction.guild.id, {"msglog_channel_id": None})
        await interaction.response.send_message("✅ Nachrichten-Log deaktiviert.", ephemeral=True)
    else:
        save_guild_config(interaction.guild.id, {"modlog_channel_id": None})
        await interaction.response.send_message("✅ Ban/Kick/Timeout-Log deaktiviert.", ephemeral=True)
...
async def on_member_ban(guild: discord.Guild, user):
    ...
    embed = discord.Embed(title="🔨 Mitglied gebannt", color=MODLOG_BAN_COLOR, timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Nutzer", value=f"{user} (`{user.id}`)", inline=False)
    embed.add_field(name="Von", value=actor.mention if actor else "Unbekannt", inline=True)
    embed.add_field(name="Grund", value=reason or "Kein Grund angegeben", inline=True)
...
async def on_member_remove(member: discord.Member):
    ...
    if not actor:
        return
    embed = discord.Embed(title="👢 Mitglied gekickt", color=MODLOG_KICK_COLOR, timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Nutzer", value=f"{member} (`{member.id}`)", inline=False)
    embed.add_field(name="Von", value=actor.mention, inline=True)
    embed.add_field(name="Grund", value=reason or "Kein Grund angegeben", inline=True)
    embed.set_thumbnail(url=member.display_avatar.url)
    await _send_modlog(member.guild, embed)
...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    if after.timed_out_until and after.timed_out_until > now:
        ...
        embed = discord.Embed(title="🔇 Mitglied getimeoutet", color=MODLOG_TIMEOUT_COLOR, timestamp=now)
        embed.add_field(name="Nutzer", value=f"{after} (`{after.id}`)", inline=False)
        embed.add_field(name="Von", value=actor.mention if actor else "Unbekannt", inline=True)
        embed.add_field(name="Bis", value=f"<t:{int(after.timed_out_until.timestamp())}:F>", inline=True)
        embed.add_field(name="Grund", value=reason or "Kein Grund angegeben", inline=True)
        ...
        embed = discord.Embed(title="🔊 Timeout entfernt", color=MODLOG_UNTIMEOUT_COLOR, timestamp=now)
        embed.add_field(name="Nutzer", value=f"{after} (`{after.id}`)", inline=False)
        embed.add_field(name="Von", value=actor.mention if actor else "Unbekannt", inline=True)
        embed.set_thumbnail(url=after.display_avatar.url)
        await _send_modlog(after.guild, embed)
...
@bot.tree.command(name="timeout", description="Timeout für ein Mitglied setzen (Staff/Manager)")
@app_commands.describe(member="Mitglied", duration="Dauer, z.B. 10m, 1h, 1d (max. 28 Tage)", reason="Grund (optional)")
async def slash_timeout(interaction: discord.Interaction, member: discord.Member, duration: str, reason: str = None):
    ...
    seconds = _parse_duration(duration)
    if seconds is None or seconds < 10:
        await interaction.response.send_message("❌ Ungültige Dauer. Beispiele: `10m`, `1h`, `1d`", ephemeral=True)
        return
    seconds = min(seconds, 28 * 24 * 3600)
    try:
        await member.timeout(discord.utils.utcnow() + timedelta(seconds=seconds), reason=reason or f"Timeout von {interaction.user}")
    except discord.Forbidden:
        await interaction.response.send_message("❌ Mir fehlt die Berechtigung dafür.", ephemeral=True)
        return
    await interaction.response.send_message(
        f"🔇 {member.mention} wurde für **{duration}** getimeoutet.\nGrund: {reason or 'Kein Grund angegeben'}"
    )


@bot.tree.command(name="untimeout", description="Timeout eines Mitglieds entfernen (Staff/Manager)")
...
<div class="wrap">
  ...
  <form method="POST">
    ...
    <div class="card">
      <h2>🚨 Mod-Log</h2>
      <div class="desc">Ban, Kick und Timeout werden hier automatisch geloggt (Moderator + Grund).</div>
      <label>Mod-Log Kanal (Ban/Kick/Timeout)</label>
      <select name="modlog_channel_id">
        <option value="0" {% if not config.modlog_channel_id %}selected{% endif %}>— Aus —</option>
        {% for c in channels %}<option value="{{ c.id }}" {% if c.id == config.modlog_channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
      </select>
      <label>Nachrichten-Log Kanal (bearbeitet / gelöscht)</label>
      <select name="msglog_channel_id">
        <option value="0" {% if not config.msglog_channel_id %}selected{% endif %}>— Aus —</option>
        {% for c in channels %}<option value="{{ c.id }}" {% if c.id == config.msglog_channel_id %}selected{% endif %}>#{{ c.name }}</option>{% endfor %}
      </select>
      <div class="hint">💡 Loggt jede bearbeitete oder gelöschte Nachricht — inklusive Bildern, Videos, GIFs und Links — mit Autor und Inhalt.</div>
    </div>

    <div class="card">
      <h2>🔗 Link-Filter &amp; Partnership</h2>
      <div class="desc">Entfernt automatisch Links/Promo von jedem ohne Partnership-Rolle und bietet ein Ticket-System dafür an.</div>
      <label style="text-transform:none; letter-spacing:normal; display:flex; align-items:center; gap:8px;">
        <input type="checkbox" name="linkfilter_enabled" {% if config.linkfilter_enabled %}checked{% endif %} style="width:auto;"> Link-Filter aktiv
      </label>
      <label>Partnership-Recruiter-Rolle (darf Links posten)</label>
      <select name="partnership_role_id">
        <option value="0" {% if not config.partnership_role_id %}selected{% endif %}>— keine —</option>
        {% for r in roles %}<option value="{{ r.id }}" {% if r.id == config.partnership_role_id %}selected{% endif %}>@{{ r.name }}</option>{% endfor %}
      </select>
      <label>Ticket-Kategorie (Partnership-Tickets werden hier erstellt)</label>
      <select name="partnership_category_id">
        <option value="0" {% if not config.partnership_category_id %}selected{% endif %}>— keine —</option>
        {% for cat in categories %}<option value="{{ cat.id }}" {% if cat.id == config.partnership_category_id %}selected{% endif %}>{{ cat.name }}</option>{% endfor %}
      </select>
      <label>Zusätzliche Bypass-Rolle(n) (dürfen trotzdem Links posten)</label>
      <select name="linkfilter_bypass_role_ids" multiple size="5">
        {% for r in roles %}<option value="{{ r.id }}" {% if r.id in config.linkfilter_bypass_role_ids %}selected{% endif %}>@{{ r.name }}</option>{% endfor %}
      </select>
      <div class="hint">💡 Staff/Manager und die Partnership-Rolle dürfen immer Links posten. Giveaways werden auf der eigenen <a href="/dashboard/{{ guild_id }}/giveaways" style="color:#c4b5fd;">🎉 Giveaways-Seite</a> verwaltet.</div>
    </div>

    <div class="card">
      <h2>🧽 Untertitel-Entferner (/removesubs)</h2>
      <div class="desc">Kostenlose, selbst gehostete KI-Entfernung von eingebrannten Untertiteln/Text aus Videos (läuft auf der CPU des Bots).</div>
      <label style="text-transform:none; letter-spacing:normal; display:flex; align-items:center; gap:8px;">
        <input type="checkbox" name="removesubs_enabled" {% if config.removesubs_enabled %}checked{% endif %} style="width:auto;"> /removesubs aktiv
      </label>
      <label>Standard-Methode</label>
      <select name="removesubs_default_method">
        {% for val, label in [("hybrid","Hybrid (empfohlen)"), ("inpaint","Schnelles Inpaint"), ("inpaint_ns","Weiche Verläufe (Navier-Stokes)"), ("blur","Weichzeichnen"), ("black","Schwarzer Balken"), ("background","Hintergrundfarbe füllen")] %}
      ...
      <select name="removesubs_default_quality">
        {% for val, label in [("360p","Schnell (360p)"), ("480p","Ausgewogen (480p)"), ("720p","Hoch (720p)"), ("1080p","Höchste (1080p, langsam)")] %}
        <option value="{{ val }}" {% if config.removesubs_default_quality == val %}selected{% endif %}>{{ label }}</option>
        {% endfor %}
      </select>
      <label>Max. Videolänge (Sekunden)</label>
      <input type="number" name="removesubs_max_seconds" min="10" max="600" value="{{ config.removesubs_max_seconds }}">
      <div class="hint">💡 Läuft komplett lokal auf CPU (kein bezahlter API-Dienst) — je länger das Video und je höher die Qualität, desto länger dauert's.</div>
    </div>

    <div class="card">
      <h2>🔒 Kanal-Zugriff</h2>
      <div class="desc">Sperrt einen bestimmten User oder eine Rolle darauf, NUR in den hier ausgewählten Kanälen schreiben zu können. Mehrere Zeilen mit demselben Ziel = mehrere erlaubte Kanäle.</div>
      <div id="accessRows">
          ...
          <div class="row4">
            <div>
              ...
              <select name="restrict_type[]">
                ...
                <option value="role" {% if row.type == "role" %}selected{% endif %}>Rolle</option>
              </select>
            </div>
            <div>
              <label>User-/Rollen-ID</label>
              <input type="text" name="restrict_target[]" value="{{ row.target_id }}" placeholder="ID einfügen">
            </div>
            <div>
              <label>Erlaubter Kanal</label>
            ...
            <div style="display:flex; align-items:flex-end;">
              <button type="button" class="btn-remove-rank" onclick="this.closest('.access-row').remove()">✕ Entfernen</button>
            </div>
          </div>
        </div>
        {% endfor %}
      </div>
      <button type="button" class="btn" id="addAccessBtn">+ Regel hinzufügen</button>
      <template id="accessRowTemplate">
        <div class="tier-edit access-row">
          <div class="row4">
            ...
            <div>
              <label>User-/Rollen-ID</label>
              <input type="text" name="restrict_target[]" value="" placeholder="ID einfügen">
            </div>
            <div>
              <label>Erlaubter Kanal</label>
              <select name="restrict_channel[]">
      ...
      <div class="hint">💡 Rechtsklick auf einen User/eine Rolle → "ID kopieren" (Discord-Entwicklermodus muss an sein). Eine Zeile komplett leer lassen bzw. entfernen, um eine Regel zu löschen.</div>
    </div>
...
def web_settings(guild_id):
    ...
    if request.method == 'POST':
        ...
        for field in ("modlog_channel_id", "msglog_channel_id"):
            try:
                val = int(request.form.get(field, 0))
            except ValueError:
                val = 0
        ...
        try:
            val = int(request.form.get("partnership_role_id", 0))
        except ValueError:
            val = 0
        updates["partnership_role_id"] = val if val in role_ids else None
        try:
        ...
        bypass_picked = []
        for raw_val in request.form.getlist("linkfilter_bypass_role_ids"):
            try:
                v = int(raw_val)
            except ValueError:
                continue
            if v in role_ids:
                bypass_picked.append(v)
        updates["linkfilter_bypass_role_ids"] = bypass_picked
...
<div class="wrap">
  <div class="topbar">
    <div><h1>🎉 Giveaways</h1><div class="tag">{{ server_name }} · laufende & vergangene Giveaways verwalten</div></div>
    <div class="nav">
  ...
  {% if not rows %}
  <div class="empty">Noch keine Giveaways. Starte eins mit <code>/giveaway start</code> in Discord.</div>
  {% endif %}

  {% for row in rows %}
  <div class="gw-card {% if row.ended %}is-ended{% endif %}">
    <div class="gw-title">{{ row.prize }}
      {% if row.cancelled %}<span class="badge cancelled">Abgebrochen</span>
      {% elif row.ended %}<span class="badge ended">Beendet</span>
      {% else %}<span class="badge active">Aktiv</span>{% endif %}
    </div>
    <div class="gw-meta">
      #{{ row.channel_name }} · {{ row.entries }} Teilnahme(n) · {{ row.winners_count }} Gewinner gesucht
      {% if row.ended and row.winners_labels %} · Gewinner: {{ row.winners_labels|join(", ") }}{% endif %}
      · ID: <code>{{ row.id }}</code>
    </div>
    <div class="actions">
      {% if not row.ended %}
      <form method="POST" style="display:inline;">
        ...
        <button type="submit">✅ Jetzt beenden</button>
      </form>
      <form method="POST" style="display:inline;" onsubmit="return confirm('Wirklich abbrechen? Es wird kein Gewinner gezogen.');">
      ...
      {% else %}
        ...
        {% endif %}
        <form method="POST" style="display:inline;" onsubmit="return confirm('Aus der Liste entfernen? (löscht nicht die Discord-Nachricht)');">
          <input type="hidden" name="giveaway_id" value="{{ row.id }}"><input type="hidden" name="action" value="delete">
          <button type="submit" class="danger">🗑️ Entfernen</button>
        </form>
      {% endif %}
    </div>
  </div>
...
async def _giveaway_action_reroll(guild: discord.Guild, key: str):
    ...
    if channel:
        ...
        try:
            if winners:
                await channel.send(f"🔁 Reroll (über das Dashboard)! Neue Gewinner: {', '.join(f'<@{w}>' for w in winners)} — **{gw['prize']}**")
            else:
                await channel.send("😢 Keine gültigen Teilnehmer für den Reroll.")
        except discord.HTTPException:
            pass
    return gw
...
def web_giveaways(guild_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'delete':
            if giveaway_id in data:
                ...
                save_giveaways(guild_id, data)
                message = "🗑️ Aus der Liste entfernt."
        elif not gw:
            message = "⚠️ Giveaway nicht gefunden."
        elif action == 'end' and not gw.get('ended'):
            run_on_bot_loop(_end_giveaway(guild, gw), timeout=30)
            message = "✅ Giveaway beendet."
    ...
    for key, gw in sorted(data.items(), key=lambda kv: kv[1].get("created_ts", 0), reverse=True):
        ...
        rows.append({
            "id": key,
            "prize": gw.get("prize", "?"),
            "channel_name": channel.name if channel else "gelöschter Kanal",
