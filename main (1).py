    ...
    <style>
      * { box-sizing:border-box; }
      body { margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center; overflow:hidden;
             background:#07070d; font-family:'Poppins',Arial,sans-serif; color:#f4f4f8; position:relative; }
      .orb { position:absolute; border-radius:50%; filter:blur(90px); opacity:0.55; animation:float 12s ease-in-out infinite; }
      .orb1 { width:420px; height:420px; background:#8b5cf6; top:-120px; left:-100px; }
      .orb2 { width:380px; height:380px; background:#f59e0b; bottom:-140px; right:-100px; animation-delay:3s; }
      .orb3 { width:280px; height:280px; background:#22d3ee; top:40%; left:60%; animation-delay:6s; }
      @keyframes float { 0%,100%{ transform:translate(0,0) scale(1); } 50%{ transform:translate(30px,-30px) scale(1.08); } }
      .box { text-align:center; z-index:1; }
      ...
      h1 { font-size:3rem; font-weight:800; margin:0; letter-spacing:-0.02em;
      ...
      p.tag { color:#9a9ab0; margin:10px 0 26px; }
      a { display:inline-block; padding:13px 30px; border-radius:999px;
      ...
      a:hover { transform:translateY(-3px) scale(1.03); box-shadow:0 12px 40px rgba(139,92,246,0.6); }
    </style></head>
    <body>
      ...
      <div class="box">
        ...
        <h1>⚡ ZEXO</h1>
        <p class="tag">Top Edit voting, points &amp; leaderboards for the server</p>
...
<body>
  ...
  <div class="footer">Zexo · <span class="pulse">●</span> auto-refreshing every 30s</div>
  <script>
...
def web_dashboard(guild_id):
    ...
    for i, (uid, score) in enumerate(sorted_entries):
        ...
        _next_label, _needed, low, high = next_rank_progress(score)
    ...
    collect_dt = now.replace(hour=config["collect_hour"], minute=0, second=0, microsecond=0)
    results_dt = now.replace(hour=config["results_hour"], minute=0, second=0, microsecond=0)
...
current_polls = {}  # guild_id -> {"message_id": ..., "channel_id": ..., "entries": [...]}


def get_poll_state(guild_id: int) -> dict:
    return current_polls.setdefault(guild_id, {"message_id": None, "channel_id": None, "entries": []})
...
async def apply_point_roles(guild: discord.Guild, member_id: int, total_points: int):
    ...
    target_label = current_rank_label(total_points)
...
# YouTube/TikTok/Instagram links get downloaded and re-posted as a real video attachment
# (instead of a bare link) so they preview/play properly in Discord like a native upload.
SOCIAL_VIDEO_DOMAINS = (
    "youtube.com", "youtu.be", "m.youtube.com",
    "tiktok.com", "vm.tiktok.com", "vt.tiktok.com",
    "instagram.com", "instagr.am",
)
MAX_DISCORD_UPLOAD_BYTES = 24 * 1024 * 1024  # stay safely under Discord's 25MB non-boosted limit
...
def is_social_video_link(url: str) -> bool:
    try:
        ...
        return False
    return any(host == d or host.endswith("." + d) for d in SOCIAL_VIDEO_DOMAINS)


def _yt_dlp_download(url: str, out_path: str):
    ydl_opts = {
        "format": "mp4/bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[filesize<24M]/best",
        "outtmpl": out_path,
        "quiet": True,
        "no_warnings": True,
        "noplaylist": True,
        "max_filesize": MAX_DISCORD_UPLOAD_BYTES,
...
async def download_social_video(url: str) -> str | None:
    """Downloads a YouTube/TikTok/Instagram link as an actual mp4 via yt-dlp, so it can be
    re-uploaded as a real Discord video attachment. Returns a local file path on success,
    or None if the download fails / is too large — callers should fall back to the raw link."""
    out_path = f"/tmp/dl_{random.randint(0, 10**9)}.mp4"
    loop = asyncio.get_running_loop()
    try:
        await loop.run_in_executor(None, _yt_dlp_download, url, out_path)
    except Exception as e:
    ...
    if not os.path.exists(out_path) or os.path.getsize(out_path) == 0:
        return None
    if os.path.getsize(out_path) > MAX_DISCORD_UPLOAD_BYTES:
        os.remove(out_path)
        print(f"⚠️ Downloaded video too large for Discord upload, falling back to link: {url}")
        return None
    return out_path
...
def extract_video_sources(msg: discord.Message):
    ...
    missed and never counted at all."""
    sources = []
    ...
    async for msg in submit_channel.history(after=since, limit=200):
        ...
        for video_source in extract_video_sources(msg):
            if is_video_recently_used(video_source, video_history):
                continue
            entries.append({"author_id": msg.author.id, "author_name": msg.author.display_name, "video_url": video_source})
...
async def run_collect_job():
    """Runs independently for every server the bot is in, each with its own channels/roles,
    its own dedupe history, and its own poll state — one server's vote can never overwrite
    another's."""
    for guild in bot.guilds:
        poll_state = get_poll_state(guild.id)
        poll_state["entries"] = []
        poll_state["message_id"] = None
        poll_state["channel_id"] = None
        ...
        for entry in entries:
            ...
            if is_social_video_link(entry["video_url"]):
                downloaded_path = await download_social_video(entry["video_url"])
        ...
        poll_state["message_id"] = sent.id
        poll_state["channel_id"] = discussion_channel.id
...
async def run_results_job():
    """Runs independently for every server: each resolves its own poll and announces its
    own winner. One server's result can never leak into another's."""
    for guild in bot.guilds:
        poll_state = get_poll_state(guild.id)
        if not poll_state["message_id"]:
            print(f"No active poll to resolve today in {guild.name}.")
            continue

        config = load_guild_config(guild.id)
        channel = bot.get_channel(poll_state["channel_id"])
        if not channel:
            continue

        try:
            msg = await channel.fetch_message(poll_state["message_id"])
        except discord.NotFound:
        ...
        if winner_index >= len(poll_state["entries"]):
            continue

        best_entry = poll_state["entries"][winner_index]
        ...
        if results_channel:
            ...
            embed = discord.Embed(
                ...
                description=win_line,
                color=discord.Color.gold(),
            )
            embed.add_field(name="Winner", value=f"<@{best_entry['author_id']}>", inline=True)
            embed.add_field(name="Votes", value=f"**{best_votes}**", inline=True)
            ...
            if is_social_video_link(best_entry["video_url"]):
                downloaded_path = await download_social_video(best_entry["video_url"])
            ...
            if downloaded_path:
                ...
                # The video link stays in the plain content (not the embed) so Discord auto-plays it.
                content = f"{ping} {best_entry['video_url']}".strip()
...
def build_tutorial_embed(guild: discord.Guild):
    ...
    embed.add_field(
        ...
        value=(
            f"Every day at **{config['collect_hour']}:00 (Europe/Berlin)**, the bot scans that channel for new "
            f"submissions and picks up to **{MAX_POLL_ANSWERS}** of them. Once a video has been used in a "
            "vote, it can **never** be picked again — no matter how long ago that was."
        ),
        inline=False,
    )
    embed.add_field(
        ...
        value=(
            ...
            f"**@{PICKER_ROLE_NAME}**. Voting stays open for **{POLL_DURATION_HOURS} hours**, closing at "
            "**{}:00**. Use `/previewvotes` any time to privately see what's currently in the vote.".format(config['results_hour'])
        ),
        inline=False,
    )
    embed.add_field(
        ...
        value=(
            ...
            f"The winner earns **+{WINNER_POINTS} point(s)**. Ties are broken with a coin-flip bracket."
        ),
        inline=False,
    )
...
async def previewvotes(ctx):
    ...
    if not entries:
        try:
            await ctx.author.send("📭 No edits queued yet — post one in the submissions channel and ping the Picker role.")
        except discord.Forbidden:
            await ctx.send("📭 No edits queued yet. *(couldn't DM you — check your privacy settings)*")
            return
    else:
        ...
        embed.set_footer(text=f"{len(entries)} edit(s) queued so far")
        try:
...
async def slash_previewvotes(interaction: discord.Interaction):
    ...
    if entries is None:
        ...
        return
    if not entries:
        await interaction.followup.send("📭 No edits queued yet — post one in the submissions channel and ping the Picker role.", ephemeral=True)
        return
    ...
    embed.set_footer(text=f"{len(entries)} edit(s) queued so far · Only visible to you")
